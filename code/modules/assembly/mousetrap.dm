...
/obj/item/device/assembly/mousetrap/examine(mob/user)
	..()
	if(armed)
		user.text2tab("It looks like it's armed.")
		...
		if(!armed)
			if(ishuman(usr))
				...
				if((user.getBrainLoss() >= 60) || user.disabilities & CLUMSY && prob(50))
					user.text2tab("<span class='warning'>Your hand slips, setting off the trigger!</span>")
					pulse(0)
		update_icon()
		if(usr)
...
/obj/item/device/assembly/mousetrap/attack_self(mob/living/carbon/human/user)
	if(!armed)
		user.text2tab("<span class='notice'>You arm [src].</span>")
	else
		if(((user.getBrainLoss() >= 60) || user.disabilities & CLUMSY) && prob(50))
			...
			return
		user.text2tab("<span class='notice'>You disarm [src].</span>")
	armed = !armed
	update_icon()
