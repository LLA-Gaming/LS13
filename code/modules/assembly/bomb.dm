...
/obj/item/device/onetankbomb/attackby(obj/item/weapon/W, mob/user, params)
	...
	if((istype(W, /obj/item/weapon/weldingtool) && W:welding))
		if(!status)
			...
			user.text2tab("<span class='notice'>A pressure hole has been bored to [bombtank] valve. \The [bombtank] can now be ignited.</span>")
		else
			status = 0
			bombers += "[key_name(user)] unwelded a single tank bomb. Temp: [bombtank.air_contents.temperature-T0C]"
			user.text2tab("<span class='notice'>The hole has been closed.</span>")
