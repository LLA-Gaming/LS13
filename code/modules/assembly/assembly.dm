...
/obj/item/device/assembly/proc/is_secured(mob/user)
	if(!secured)
		user.text2tab("<span class='warning'>The [name] is unsecured!</span>")
		return 0
	return 1
...
/obj/item/device/assembly/attackby(obj/item/weapon/W, mob/user, params)
	if(isassembly(W))
		...
		if((!A.secured) && (!secured))
			...
			user.text2tab("<span class='notice'>You attach and secure \the [A] to \the [src]!</span>")
		else
			user.text2tab("<span class='warning'>Both devices must be in attachable mode to be attached together.</span>")
		return
	if(istype(W, /obj/item/weapon/screwdriver))
		if(toggle_secure())
			user.text2tab("<span class='notice'>\The [src] is ready!</span>")
		else
			user.text2tab("<span class='notice'>\The [src] can now be attached!</span>")
		return
	..()
...
/obj/item/device/assembly/examine(mob/user)
	..()
	if(secured)
		user.text2tab("\The [src] is secured and ready to be used.")
	else
		user.text2tab("\The [src] can be attached to other things.")
