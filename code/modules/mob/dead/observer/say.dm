/mob/dead/observer/say(message)
	...
	if(jobban_isbanned(src, "OOC"))
		src.text2tab("<span class='danger'>You have been banned from deadchat.</span>")
		return

	if (src.client)
		if(src.client.prefs.muted & MUTE_DEADCHAT)
			src.text2tab("<span class='danger'>You cannot talk in deadchat (muted).</span>")
			return
