...
/mob/living/simple_animal/bot/proc/call_bot(caller, turf/waypoint, message=TRUE)
	...
	if(client) //Player bots instead get a location command from the AI
		src.text2tab("<span class='noticebig'>Priority waypoint set by \icon[caller] <b>[caller]</b>. Proceed to <b>[end_area.name]<\b>.")

	//For giving the bot temporary all-access.
	var/obj/item/weapon/card/id/all_access = new /obj/item/weapon/card/id
	var/datum/job/manager/All = new/datum/job/manager
