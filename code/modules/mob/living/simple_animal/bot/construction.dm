...
/obj/item/weapon/bucket_sensor/attackby(obj/item/W, mob/user as mob, params)
	...
	if(istype(W, /obj/item/robot_parts/l_arm) || istype(W, /obj/item/robot_parts/r_arm))
		...
		A.name = created_name
		user.text2tab("<span class='notice'>You add the robot arm to the bucket and sensor assembly. Beep boop!</span>")
		user.unEquip(src, 1)
		qdel(src)
	...
	switch(build_step)
		if(0,1)
			if(istype(W, /obj/item/robot_parts/l_leg) || istype(W, /obj/item/robot_parts/r_leg))
				...
				qdel(W)
				build_step++
				user.text2tab("<span class='notice'>You add the robot leg to [src].</span>")
		...
		if(2)
			...
			if(newcolor || istype(W, /obj/item/clothing/suit/armor/vest))
				...
				qdel(W)
				build_step++
				user.text2tab("<span class='notice'>You add the armor to [src].</span>")
		...
		if(3)
			if(istype(W, /obj/item/weapon/weldingtool))
				...
				if(WT.remove_fuel(0,user))
					...
					user.text2tab("<span class='notice'>You weld the vest to [src].</span>")
		if(4)
			switch(lasercolor)
				if("b")
			...
			qdel(W)
			build_step++
			user.text2tab("<span class='notice'>You add the helmet to [src].</span>")
		...
		if(5)
			if(isprox(W))
				...
				qdel(W)
				build_step++
				user.text2tab("<span class='notice'>You add the prox sensor to [src].</span>")
		...
		if(6)
			if(istype(W, /obj/item/stack/cable_coil))
				...
				if(coil.get_amount() < 1)
					user.text2tab("<span class='warning'>You need one length of cable to wire the ED-209!</span>")
					return
				user.text2tab("<span class='notice'>You start to wire [src]...</span>")
				if(do_after(user, 40, target = src))
					if(coil.get_amount() >= 1 && build_step == 6)
						coil.use(1)
						build_step = 7
						user.text2tab("<span class='notice'>You wire the ED-209 assembly.</span>")
		...
		if(7)
			...
			name = newname
			build_step++
			user.text2tab("<span class='notice'>You add [W] to [src].</span>")
		...
		if(8)
			if(istype(W, /obj/item/weapon/screwdriver))
				...
				user.text2tab("<span class='notice'>You start attaching the gun to the frame...</span>")
				sleep(40)
				if(get_turf(user) == T)
					build_step++
					name = "armed [name]"
					user.text2tab("<span class='notice'>Taser gun attached.</span>")
		...
		if(9)
			if(istype(W, /obj/item/weapon/stock_parts/cell))
				if(!user.unEquip(W))
					return
				build_step++
				user.text2tab("<span class='notice'>You complete the ED-209.</span>")
...
/obj/item/weapon/storage/toolbox/mechanical/attackby(obj/item/stack/tile/plasteel/T, mob/user, params)
	if(!istype(T, /obj/item/stack/tile/plasteel))
		...
		return
	if(contents.len >= 1)
		user.text2tab("<span class='warning'>They won't fit in, as there is already stuff inside!</span>")
		return
	if(T.use(10))
		if(user.s_active)
		...
		user.put_in_hands(B)
		user.text2tab("<span class='notice'>You add the tiles into the empty toolbox. They protrude from the top.</span>")
		user.unEquip(src, 1)
		qdel(src)
	else
		user.text2tab("<span class='warning'>You need 10 floor tiles to start building a floorbot!</span>")
		return
...
/obj/item/weapon/toolbox_tiles/attackby(obj/item/W, mob/user, params)
	...
	if(isprox(W))
		...
		user.put_in_hands(B)
		user.text2tab("<span class='notice'>You add the sensor to the toolbox and tiles.</span>")
		user.unEquip(src, 1)
		qdel(src)
...
/obj/item/weapon/toolbox_tiles_sensor/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/robot_parts/l_arm) || istype(W, /obj/item/robot_parts/r_arm))
		...
		A.name = created_name
		user.text2tab("<span class='notice'>You add the robot arm to the odd looking toolbox assembly. Boop beep!</span>")
		user.unEquip(src, 1)
		qdel(src)
	else if(istype(W, /obj/item/weapon/pen))
		var/t = stripped_input(user, "Enter new robot name", name, created_name,MAX_NAME_LEN)
...
/obj/item/weapon/storage/firstaid/attackby(obj/item/robot_parts/S, mob/user, params)
	...
	//Making a medibot!
	if(contents.len >= 1)
		user.text2tab("<span class='warning'>You need to empty [src] out first!</span>")
		return
	...
	qdel(S)
	user.put_in_hands(A)
	user.text2tab("<span class='notice'>You add the robot arm to the first aid kit.</span>")
	user.unEquip(src, 1)
	qdel(src)
		...
		switch(build_step)
			if(0)
				if(istype(W, /obj/item/device/healthanalyzer))
					...
					qdel(W)
					build_step++
					user.text2tab("<span class='notice'>You add the health sensor to [src].</span>")
			...
			if(1)
				if(isprox(W))
					...
					qdel(W)
					build_step++
					user.text2tab("<span class='notice'>You complete the Medibot. Beep boop!</span>")
					var/turf/T = get_turf(src)
...
/obj/item/clothing/head/helmet/attackby(obj/item/device/assembly/signaler/S, mob/user, params)
	...
	if(F) //Has a flashlight. Player must remove it, else it will be lost forever.
		user.text2tab("<span class='warning'>The mounted flashlight is in the way, remove it first!</span>")
		return
	...
	if(S.secured)
		...
		user.put_in_hands(A)
		user.text2tab("<span class='notice'>You add the signaler to the helmet.</span>")
		user.unEquip(src, 1)
		qdel(src)
	else
		return
	...
	if(istype(I, /obj/item/weapon/weldingtool))
		if(!build_step)
			...
			if(WT.remove_fuel(0, user))
				build_step++
				overlays += "hs_hole"
				user.text2tab("<span class='notice'>You weld a hole in [src]!</span>")
		else if(build_step == 1)
			var/obj/item/weapon/weldingtool/WT = I
			if(WT.remove_fuel(0, user))
				build_step--
				overlays -= "hs_hole"
				user.text2tab("<span class='notice'>You weld the hole in [src] shut!</span>")
		...
		if(!user.unEquip(I))
			return
		build_step++
		user.text2tab("<span class='notice'>You add the robot arm to [src]!</span>")
		...
		if(!user.unEquip(I))
			return
		build_step++
		user.text2tab("<span class='notice'>You complete the Securitron! Beep boop.</span>")
		...
		if(!build_step)
			...
			user.text2tab("<span class='notice'>You disconnect the signaler from the helmet.</span>")
			qdel(src)

		else if(build_step == 2)
			overlays -= "hs_eye"
			new /obj/item/device/assembly/prox_sensor(get_turf(src))
			user.text2tab("<span class='notice'>You detach the proximity sensor from [src].</span>")
			build_step--

		else if(build_step == 3)
			overlays -= "hs_arm"
			new /obj/item/robot_parts/l_arm(get_turf(src))
			user.text2tab("<span class='notice'>You remove the robot arm from [src].</span>")
			build_step--
