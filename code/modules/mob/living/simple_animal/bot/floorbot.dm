...
/mob/living/simple_animal/bot/floorbot/New()
	..()
	update_icon()
	var/datum/job/roboticist/J = new/datum/job/roboticist
...
/mob/living/simple_animal/bot/floorbot/attackby(obj/item/W , mob/user, params)
	if(istype(W, /obj/item/stack/tile/plasteel))
		user.text2tab("<span class='notice'>The floorbot can produce normal tiles itself.</span>")
		return
	if(specialtiles && istype(W, /obj/item/stack/tile))
		var/obj/item/stack/tile/usedtile = W
		if(usedtile.type != tiletype)
			user.text2tab("<span class='warning'>Different custom tiles are already inside the floorbot.</span>")
			return
	if(istype(W, /obj/item/stack/tile))
		...
		specialtiles += loaded
		if(loaded > 0)
			user.text2tab("<span class='notice'>You load [loaded] tiles into the floorbot. It now contains [specialtiles] tiles.</span>")
		else
			user.text2tab("<span class='warning'>You need at least one floor tile to put into [src]!</span>")
	else
		..()
