...
/mob/living/simple_animal/slime/handle_environment(datum/gas_mixture/environment)
	...
	if(stat != DEAD)
		...
		if(stat == CONSCIOUS && stasis)
			src.text2tab("<span class='danger'>Nerve gas in the air has put you in stasis!</span>")
			stat = UNCONSCIOUS
			powerlevel = 0
			rabid = 0
			update_canmove()
			...
			src.text2tab("<span class='notice'>You wake up from the stasis.</span>")
			stat = CONSCIOUS
			update_canmove()
			regenerate_icons()
...
/mob/living/simple_animal/slime/proc/handle_feeding()
	...
	if(M.stat == DEAD) // our victim died
		if(!client)
			...
			src.text2tab("<i>This subject does not have a strong enough life energy anymore...</i>")
	...
	if(iscarbon(M))
		...
		if(prob(10) && C.client)
			C.text2tab("<span class='userdanger'>[pick("You can feel your body becoming weak!", \
