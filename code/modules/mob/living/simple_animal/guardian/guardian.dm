...
/mob/living/simple_animal/hostile/guardian/Login() //if we have a mind, set its name to ours when it logs in
	...
	if(mind)
		mind.name = "[real_name]"
	if(!summoner)
		src.text2tab("<span class='holoparasitebold'>For some reason, somehow, you have no summoner. Please report this bug immediately.</span>")
		return
	src.text2tab("<span class='holoparasite'>You are <font color=\"[namedatum.colour]\"><b>[real_name]</b></font>, bound to serve [summoner.real_name].</span>")
	src.text2tab("<span class='holoparasite'>You are capable of manifesting or recalling to your master with the buttons on your HUD. You will also find a button to communicate with them privately there.</span>")
	src.text2tab("<span class='holoparasite'>While personally invincible, you will die if [summoner.real_name] does, and any damage dealt to you will have a portion passed on to them as you feed upon them to sustain yourself.</span>")
...
/mob/living/simple_animal/hostile/guardian/Life() //Dies if the summoner dies
	...
	if(summoner)
		if(summoner.stat == DEAD)
			src.text2tab("<span class='danger'>Your summoner has died!</span>")
...
/mob/living/simple_animal/hostile/guardian/proc/snapback()
	if(summoner)
		if(get_dist(get_turf(summoner),get_turf(src)) <= range)
			...
			src.text2tab("<span class='holoparasite'>You moved out of range, and were pulled back! You can only move [range] meters from [summoner.real_name]!</span>")
...
/mob/living/simple_animal/hostile/guardian/AttackingTarget()
	if(src.loc == summoner)
		src.text2tab("<span class='danger'><B>You must be manifested to attack!</span></B>")
		return 0
	else
		..()
		return 1
...
/mob/living/simple_animal/hostile/guardian/death()
	..()
	summoner.text2tab("<span class='danger'><B>Your [name] died somehow!</span></B>")
	summoner.death()
...
/mob/living/simple_animal/hostile/guardian/adjustHealth(amount) //The spirit is invincible, but passes on damage to the summoner
	...
	if(summoner)
		...
		if(amount)
			summoner.text2tab("<span class='danger'><B>Your [name] is under attack! You take damage!</span></B>")
			summoner.visible_message("<span class='danger'><B>Blood sprays from [summoner] as [src] takes damage!</B></span>")
		if(summoner.stat == UNCONSCIOUS)
			summoner.text2tab("<span class='danger'><B>Your body can't take the strain of sustaining [src] in this condition, it begins to fall apart!</span></B>")
...
/mob/living/simple_animal/hostile/guardian/gib()
	if(summoner)
		summoner.text2tab("<span class='danger'><B>Your [src] was blown up!</span></B>")
		summoner.gib()
	ghostize()
	qdel(src)
...
/mob/living/simple_animal/hostile/guardian/proc/ToggleMode()
	src.text2tab("<span class='danger'><B>You don't have another mode!</span></B>")

/mob/living/simple_animal/hostile/guardian/proc/ToggleLight()
	if(!luminosity)
		src.text2tab("<span class='notice'>You activate your light.</span>")
		SetLuminosity(3)
	else
		src.text2tab("<span class='notice'>You deactivate your light.</span>")
		SetLuminosity(0)
...
/mob/living/simple_animal/hostile/guardian/proc/Communicate()
	if(summoner)
		...
		summoner.text2tab(my_message)
		var/list/guardians = summoner.hasparasites()
		for(var/mob/para in guardians)
			para.text2tab(my_message)
...
/mob/living/proc/guardian_comm()
	...
	src.text2tab(my_message)
	var/list/guardians = hasparasites()
	for(var/para in guardians)
		var/mob/living/simple_animal/hostile/guardian/G = para
		G.text2tab("<font color=\"[G.namedatum.colour]\"><b><i>[src]:</i></b></font> [preliminary_message]") //but for guardians, use their color for the source instead
	for(var/mob/M in dead_mob_list)
		var/link = FOLLOW_LINK(M, src)
		M.text2tab("[link] [my_message]")
...
/mob/living/proc/guardian_reset()
	...
	if(guardians.len)
		...
		if(G)
			src.text2tab("<span class='holoparasite'>You attempt to reset <font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font>'s personality...</span>")
			...
			if(candidates.len)
				...
				G.text2tab("<span class='holoparasite'>Your user reset you, and your body was taken over by a ghost. Looks like they weren't happy with your performance.</span>")
				src.text2tab("<span class='holoparasitebold'>Your <font color=\"[G.namedatum.colour]\">[G.real_name]</font> has been successfully reset.</span>")
				...
				switch(G.namedatum.theme)
					if("tech")
						src.text2tab("<span class='holoparasite'><font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font> is now online!</span>")
					if("magic")
						src.text2tab("<span class='holoparasite'><font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font> has been summoned!</span>")
				guardians -= G
				if(!guardians.len)
					verbs -= /mob/living/proc/guardian_reset
			else
				src.text2tab("<span class='holoparasite'>There were no ghosts willing to take control of <font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font>. Looks like you're stuck with it for now.</span>")
		else
			src.text2tab("<span class='holoparasite'>You decide not to reset [guardians.len > 1 ? "any of your guardians":"your guardian"].</span>")
	else
		verbs -= /mob/living/proc/guardian_reset
...
/obj/item/weapon/guardiancreator/attack_self(mob/living/user)
	...
	if(guardians.len && !allowmultiple)
		user.text2tab("<span class='holoparasite'>You already have a [mob_name]!</span>")
		return
	if(user.mind && user.mind.changeling && !allowling)
		user.text2tab("[ling_failure]")
		return
	if(used == TRUE)
		user.text2tab("[used_message]")
		return
	used = TRUE
	user.text2tab("[use_message]")
...
/obj/item/weapon/guardiancreator/proc/spawn_guardian(var/mob/living/user, var/key)
	...
	if(random)
		...
		if(!guardiantype)
			user.text2tab("[failure_message]") //they canceled? sure okay don't force them into it
			used = FALSE
			return
	...
	var/list/guardians = user.hasparasites()
	if(guardians.len && !allowmultiple)
		user.text2tab("<span class='holoparasite'>You already have a [mob_name]!</span>") //nice try, bucko
		used = FALSE
		return
	...
	switch(theme)
		if("tech")
			user.text2tab("[G.tech_fluff_string]")
			user.text2tab("<span class='holoparasite'><font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font> is now online!</span>")
		if("magic")
			user.text2tab("[G.magic_fluff_string]")
			user.text2tab("<span class='holoparasite'><font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font> has been summoned!</span>")
		if("carp")
			user.text2tab("[G.carp_fluff_string]")
			user.text2tab("<span class='holoparasite'><font color=\"[G.namedatum.colour]\"><b>[G.real_name]</b></font> has been caught!</span>")
