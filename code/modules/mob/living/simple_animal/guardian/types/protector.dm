...
/mob/living/simple_animal/hostile/guardian/protector/ToggleMode()
	...
	if(toggle)
		...
		src.text2tab("<span class='danger'><B>You switch to protection mode.</span></B>")
		toggle = TRUE
...
/mob/living/simple_animal/hostile/guardian/protector/snapback() //snap to what? snap to the guardian!
	if(summoner)
		if(get_dist(get_turf(summoner),get_turf(src)) <= range)
			...
			summoner.text2tab("<span class='holoparasite'>You moved out of range, and were pulled back! You can only move [range] meters from <font color=\"[namedatum.colour]\"><b>[real_name]</b></font>!</span>")
