...
/mob/living/simple_animal/hostile/guardian/ranged/ToggleMode()
	if(src.loc == summoner)
		if(toggle)
			...
			range = 255
			incorporeal_move = 1
			src.text2tab("<span class='danger'><B>You switch to scout mode.</span></B>")
			toggle = TRUE
	else
...
/mob/living/simple_animal/hostile/guardian/ranged/ToggleLight()
	if(see_invisible == SEE_INVISIBLE_MINIMUM)
		src.text2tab("<span class='notice'>You deactivate your night vision.</span>")
		see_invisible = SEE_INVISIBLE_LIVING
	else
		src.text2tab("<span class='notice'>You activate your night vision.</span>")
...
/mob/living/simple_animal/hostile/guardian/ranged/verb/Snare()
	...
	if(src.snares.len <6)
		...
		src.snares |= S
		src.text2tab("<span class='danger'><B>Surveillance snare deployed!</span></B>")
	else
		src.text2tab("<span class='danger'><B>You have too many snares deployed. Remove some first.</span></B>")
...
/mob/living/simple_animal/hostile/guardian/ranged/verb/DisarmSnare()
	...
	if(picked_snare)
		...
		qdel(picked_snare)
		src.text2tab("<span class='danger'><B>Snare disarmed.</span></B>")
...
/obj/effect/snare/Crossed(AM as mob|obj)
	if(isliving(AM) && spawner && spawner.summoner && AM != spawner && !spawner.hasmatchingsummoner(AM))
		spawner.summoner.text2tab("<span class='danger'><B>[AM] has crossed surveillance snare, [name].</span></B>")
		var/list/guardians = spawner.summoner.hasparasites()
		for(var/para in guardians)
			para:text2tab("<span class='danger'><B>[AM] has crossed surveillance snare, [name].</span></B>")
