...
/mob/living/simple_animal/hostile/guardian/healer/ToggleMode()
	if(src.loc == summoner)
		if(toggle)
			...
			melee_damage_lower = 15
			melee_damage_upper = 15
			src.text2tab("<span class='danger'><B>You switch to combat mode.</span></B>")
			toggle = FALSE
		else
			a_intent = "help"
			speed = 1
			...
			src.text2tab("<span class='danger'><B>You switch to healing mode.</span></B>")
			toggle = TRUE
	else
		src.text2tab("<span class='danger'><B>You have to be recalled to toggle modes!</span></B>")
...
/mob/living/simple_animal/hostile/guardian/healer/verb/Beacon()
	...
	if(beacon_cooldown >= world.time)
		src.text2tab("<span class='danger'><B>Your power is on cooldown. You must wait five minutes between placing beacons.</span></B>")
		return
	...
	src.text2tab("<span class='danger'><B>Beacon placed! You may now warp targets \
...
/mob/living/simple_animal/hostile/guardian/healer/AltClickOn(atom/movable/A)
	if(!istype(A))
		return
	if(src.loc == summoner)
		src.text2tab("<span class='danger'><B>You must be manifested to warp a \
			target!</span></B>")
		return
	if(!beacon)
		src.text2tab("<span class='danger'><B>You need a beacon placed to warp \
			things!</span></B>")
		return
	if(!Adjacent(A))
		src.text2tab("<span class='danger'><B>You must be adjacent to your \
			target!</span></B>")
		return
	if(A.anchored)
		src.text2tab("<span class='danger'><B>Your target cannot be \
			anchored!</span></B>")
		return

	var/turf/T = get_turf(A)
	if(beacon.z != T.z)
		src.text2tab("<span class='danger'><B>The beacon is too far away to warp \
			to!</span></B>")
		return

	src.text2tab("<span class='danger'><B>You begin to warp [A].</span></B>")
	...
	if(!do_mob(src, A, 60)) //now start the channel
		src.text2tab("<span class='danger'><B>You need to hold still!</span></B>")
		return
