...
/mob/living/simple_animal/pet/dog/corgi/attackby(obj/item/O, mob/user, params)
	if(inventory_head && inventory_back)
		...
		if( istype(inventory_head,/obj/item/clothing/head/helmet) && istype(inventory_back,/obj/item/clothing/suit/armor) )
			if( O.force )
				user.text2tab("<span class='warning'>[src] is wearing too much armor! You can't cause \him any damage.</span>")
				visible_message("<span class='danger'>[user] hits [src] with [O], however [src] is too armored.</span>")
			else
				user.text2tab("<span class='warning'>[src] is wearing too much armor! You can't reach \his skin.<span>")
	...
	if (istype(O, /obj/item/weapon/razor))
		if (shaved)
			user.text2tab("<span class='warning'>You can't shave this corgi, it's already been shaved!</span>")
			return
		if (nofur)
			user.text2tab("<span class='warning'> You can't shave this corgi, it doesn't have a fur coat!</span>")
			return
...
/mob/living/simple_animal/pet/dog/corgi/Topic(href, href_list)
		...
		switch(remove_from)
			if("head")
				if(inventory_head)
					...
					usr.text2tab("<span class='danger'>There is nothing to remove from its [remove_from].</span>")
					return
			if("back")
				if(inventory_back)
	...
	//Adding things to inventory
		...
		switch(add_to)
			...
			if("back")
				if(inventory_back)
					usr.text2tab("<span class='warning'>It's already wearing something!</span>")
					return
				else
					...
					if(!usr.drop_item())
						usr.text2tab("<span class='warning'>\The [item_to_add] is stuck to your hand, you cannot put it on [src]'s back!</span>")
						return
					...
					if(!allowed)
						usr.text2tab("<span class='warning'>You set [item_to_add] on [src]'s back, but it falls off!</span>")
						item_to_add.loc = loc
						if(prob(25))
...
/mob/living/simple_animal/pet/dog/corgi/proc/place_on_head(obj/item/item_to_add, mob/user)
	...
	if(inventory_head)
		if(user)
			user.text2tab("<span class='warning'>You can't put more than one hat on [src]!</span>")
		return
	if(!item_to_add)
	...
	if(user && !user.drop_item())
		user.text2tab("<span class='warning'>\The [item_to_add] is stuck to your hand, you cannot put it on [src]'s head!</span>")
		return 0
	...
	if(valid)
		if(health <= 0)
			user.text2tab("<span class ='notice'>There is merely a dull, lifeless look in [real_name]'s eyes as you put the [item_to_add] on \him.</span>")
		else if(user)
		...
		user.text2tab("<span class='warning'>You set [item_to_add] on [src]'s head, but it falls off!</span>")
		item_to_add.loc = loc
		if(prob(25))
			step_rand(item_to_add)
		for(var/i in list(1,2,4,8,4,8,4,dir))
...
/mob/living/simple_animal/pet/dog/corgi/Ian
	...
	gender = MALE
	desc = "It's the HR's beloved corgi."
...
/mob/living/simple_animal/pet/dog/corgi/Ian/New()
	...
	if(age == 0)
		...
		P.real_name = "Ian"
		P.gender = MALE
		P.desc = "It's the HR's beloved corgi puppy."
		Write_Memory(0)
		qdel(src)
		...
		desc = "At a ripe old age of [record_age] Ian's not as spry as he used to be, but he'll always be the HR's beloved corgi." //RIP
		turns_per_move = 20
	..()
...
/mob/living/simple_animal/pet/dog/corgi/puppy/Topic(href, href_list)
	if(href_list["remove_inv"] || href_list["add_inv"])
		usr.text2tab("<span class='warning'>You can't fit this on [src]!</span>")
		return
	..()
...
/mob/living/simple_animal/pet/dog/corgi/Lisa/Topic(href, href_list)
	if(href_list["remove_inv"] || href_list["add_inv"])
		usr.text2tab("<span class='danger'>[src] already has a cute bow!</span>")
		return
	..()
