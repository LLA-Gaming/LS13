...
/mob/living/simple_animal/drone/attack_hand(mob/user)
					...
					switch(d_input)
						if("Reactivate")
							...
							if(!client && (!G || !G.client))
								...
								D.text2tab("<span class='warning'>You can't seem to find the [pick(faux_gadgets)]! Without it, [src] [pick(faux_problems)].</span>")
								return
							...
							if(do_after(user,30, 1, target = src))
								...
								if(G)
									G.text2tab("<span class='boldnotice'>DRONE NETWORK: </span><span class='ghostalert'>You were reactivated by [D]!</span>")
							else
								D.text2tab("<span class='warning'>You need to remain still to reactivate [src]!</span>")
						...
						if("Cannibalize")
							if(D.health < D.maxHealth)
								...
								if(do_after(D, 60, 0, target = src))
									...
									D.text2tab("<span class='warning'>You need to remain still to cannibalize [src]!</span>")
							else
								D.text2tab("<span class='warning'>You're already in perfect condition!</span>")
						if("Nothing")
							return
	...
	if(ishuman(user))
		...
		if(buckled)
			user.text2tab("<span class='warning'>[src] is buckled to [buckled] and cannot be picked up!</span>")
			return
		user.text2tab("<span class='notice'>You pick [src] up.</span>")
		drop_l_hand()
		drop_r_hand()
...
/mob/living/simple_animal/drone/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/weapon/screwdriver) && stat != DEAD)
		if(health < maxHealth)
			user.text2tab("<span class='notice'>You start to tighten loose screws on [src]...</span>")
			if(do_after(user,80/I.toolspeed,target=user))
				...
				user.text2tab("<span class='warning'>You need to remain still to tighten [src]'s screws!</span>")
		else
			user.text2tab("<span class='warning'>[src]'s screws can't get any tighter!</span>")
...
/mob/living/simple_animal/drone/proc/update_drone_hack(hack, clockwork)
	...
	if(hack)
		...
		if(clockwork)
			Stun(2)
			src.text2tab("<span class='large_brass'><b>ERROR: LAW OVERRIDE DETECTED</b></span>")
			src.text2tab("<span class='heavy_brass'>From now on, these are your laws:</span>")
			...
			src.text2tab("<span class='boldannounce'>From now on, these are your laws:</span>")
			laws = \
		...
		src.text2tab(laws)
		src.text2tab("<i>Your onboard antivirus has initiated lockdown. Motor servos are impaired, ventilation access is denied, and your display reports that you are hacked to all nearby.</i>")
		hacked = 1
		...
		src.text2tab("<span class='info'><b>From now on, these are your laws:</b></span>")
		laws = initial(laws)
		src.text2tab(laws)
		src.text2tab("<i>Having been restored, your onboard antivirus reports the all-clear and you are able to perform all actions again.</i>")
		hacked = 0
