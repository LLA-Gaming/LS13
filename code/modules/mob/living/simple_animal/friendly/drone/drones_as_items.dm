...
/obj/item/drone_shell/attack_ghost(mob/user)
	...
	if(config.use_age_restriction_for_jobs)
		...
		if(user.client.player_age < DRONE_MINIMUM_AGE)
			user.text2tab("<span class='danger'>You're too new to play as a drone! Please try again in [DRONE_MINIMUM_AGE - user.client.player_age] days.</span>")
			return
	if(!ticker.mode)
		user.text2tab("Can't become a drone before the game has started.")
		return
...
/obj/item/clothing/head/drone_holder/proc/uncurl()
	...
	if(istype(loc, /mob/living))
		...
		L.text2tab("<span class='warning'>[drone] is trying to escape!</span>")
