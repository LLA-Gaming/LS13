...
/mob/living/simple_animal/hostile/poison/giant_spider/proc/humanize_spider(mob/user)
	...
	if(spider_ask == "No" || !src || qdeleted(src))
		return 1
	if(key)
		user.text2tab("<span class='notice'>Someone else already took this spider.</span>")
		return 1
	key = user.key
	return 1
...
/mob/living/simple_animal/hostile/poison/giant_spider/nurse/verb/LayEggs()
	...
	if(stat == DEAD)
		return
	if(E)
		src.text2tab("<span class='warning'>There is already a cluster of eggs here!</span>")
	else if(!fed)
		src.text2tab("<span class='warning'>You are too hungry to do this!</span>")
