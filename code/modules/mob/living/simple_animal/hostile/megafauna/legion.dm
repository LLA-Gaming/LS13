...
/obj/item/weapon/staff_of_storms/attack_self(mob/user)
	if(storm_cooldown > world.time)
		user.text2tab("<span class='warning'>The staff is still recharging!</span>")
		return
	if(user.z != ZLEVEL_LAVALAND)
		user.text2tab("<span class='warning'>You can't seem to control the weather here!</span>")
		return
	...
	for(var/V in SSweather.existing_weather)
		...
		if(W.name == "ash storm")
			...
			break
	if(!A)
		user.text2tab("<span class='warning'>How odd! The planet seems to have lost its atmosphere!</span>")
		return

	if(A.stage != END_STAGE)
		if(A.stage == WIND_DOWN_STAGE)
			user.text2tab("<span class='warning'>The storm is already ending! It would be a waste to use the staff now.</span>")
			return
