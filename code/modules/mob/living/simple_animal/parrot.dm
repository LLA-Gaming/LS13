...
/mob/living/simple_animal/parrot/examine(mob/user)
	..()
	if(stat)
		user.text2tab(pick("This parrot is no more", "This is a late parrot", "This is an ex-parrot"))
	...
	if(ishuman(usr) || ismonkey(usr) || isrobot(usr) ||  isalienadult(usr))
			...
			switch(remove_from)
				if("ears")
					if(ears)
						...
						usr.text2tab("<span class='warning'>There is nothing to remove from its [remove_from]!</span>")
						return
		...
		//Adding things to inventory
			...
			if(!usr.get_active_hand())
				usr.text2tab("<span class='warning'>You have nothing in your hand to put on its [add_to]!</span>")
				return
			switch(add_to)
				if("ears")
					if(ears)
						...
						if( !istype(item_to_add,  /obj/item/device/radio/headset) )
							usr.text2tab("<span class='warning'>This object won't fit!</span>")
							return
						...
						usr.text2tab("<span class='notice'>You fit the headset onto [src].</span>")
...
/mob/living/simple_animal/parrot/attackby(obj/item/O, mob/living/user, params)
	if(!stat && !client && !istype(O, /obj/item/stack/medical) && !istype(O,/obj/item/weapon/reagent_containers/food/snacks/cracker))
		...
		user.text2tab("<span class='notice'>[src] eagerly devours the cracker.</span>")
	..()
	return
...
/mob/living/simple_animal/parrot/proc/steal_from_ground()
	...
	if(held_item)
		src.text2tab("<span class='warning'>You are already holding [held_item]!</span>")
		return 1
	...
	src.text2tab("<span class='warning'>There is nothing of interest to take!</span>")
	return 0
...
/mob/living/simple_animal/parrot/proc/steal_from_mob()
	...
	src.text2tab("<span class='warning'>There is nothing of interest to take!</spawn>")
	return 0
...
/mob/living/simple_animal/parrot/proc/drop_held_item(drop_gently = 1)
	...
	if(!held_item)
		if(src == usr) //So that other mobs wont make this message appear when they're bludgeoning you.
			src.text2tab("<span class='danger'>You have nothing to drop!</span>")
		return 0
...
//parrots will eat crackers instead of dropping them
	...
	if(!drop_gently)
		if(istype(held_item, /obj/item/weapon/grenade))
			...
			G.loc = src.loc
			G.prime()
			src.text2tab("You let go of [held_item]!")
			held_item = null
			return 1

	src.text2tab("You drop [held_item].")
		...
		for(var/atom/movable/AM in view(src,1))
			for(var/perch_path in desired_perches)
				if(istype(AM, perch_path))
					...
					icon_state = "parrot_sit"
					return
	src.text2tab("<span class='warning'>There is no perch nearby to sit on!</span>")
	return
...
/mob/living/simple_animal/parrot/proc/perch_mob_player()
	...
	if(icon_state == "parrot_fly")
		for(var/mob/living/carbon/human/H in view(src,1))
			...
			perch_on_human(H)
			return
		src.text2tab("<span class='warning'>There is nobody nearby that you can sit on!</span>")
	else
		icon_state = "parrot_fly"
		parrot_state = PARROT_WANDER
		if(buckled)
			src.text2tab("<span class='notice'>You are no longer sitting on [buckled]'s shoulder.</span>")
			buckled.unbuckle_mob(src,force=1)
		buckled = null
		pixel_x = initial(pixel_x)
		pixel_y = initial(pixel_y)
...
/mob/living/simple_animal/parrot/proc/perch_on_human(mob/living/carbon/human/H)
	...
	icon_state = "parrot_sit"
	parrot_state = PARROT_PERCH
	src.text2tab("<span class='notice'>You sit on [H]'s shoulder.</span>")
...
/mob/living/simple_animal/parrot/proc/toggle_mode()
	...
	if(melee_damage_upper)
		...
		melee_damage_upper = parrot_damage_upper
		a_intent = "harm"
	src.text2tab("You will now [a_intent] others...")
	return
