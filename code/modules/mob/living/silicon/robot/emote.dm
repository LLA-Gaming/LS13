	...
	switch(act)//01000001011011000111000001101000011000010110001001100101011101000110100101111010011001010110010000100001 (Seriously please keep it that way.)
		...
		if ("custom")
			if(jobban_isbanned(src, "emote"))
				src.text2tab("You cannot send custom emotes (banned)")
				return
			if(src.client)
...
/mob/living/silicon/robot/verb/powerwarn()
	...
	if(!cell || !cell.charge)
		...
		src.text2tab("<span class='warning'>You can only use this emote when you're out of charge.</span>")
