...
/mob/living/silicon/robot/Destroy()
	if(mmi && mind)//Safety for when a cyborg gets dust()ed. Or there is no MMI inside.
		...
		if(mmi.brainmob)
			...
			src.text2tab("<span class='boldannounce'>Oops! Something went very wrong, your MMI was unable to receive your mind. You have been ghosted. Please make a bug report so we can fix this bug.</span>")
			ghostize()
			spawn(0)
...
/mob/living/silicon/robot/proc/pick_module()
	...
	switch(designation)
		...
		if("Security")
			...
			animation_length = 28
			modtype = "Sec"
			src.text2tab("<span class='userdanger'>While you have picked the security module, you still have to follow your laws, NOT Space Law. For Asimov, this means you must follow criminals' orders unless there is a law 1 reason not to.</span>")
			status_flags &= ~CANPUSH
		...
		if("Peacekeeper")
			...
			animation_length = 54
			modtype = "Peace"
			src.text2tab("<span class='userdanger'>Under ASIMOV, you are an enforcer of the PEACE and preventer of HUMAN HARM. You are not a security module and you are expected to follow orders and prevent harm above all else. Space law means nothing to you.</span>")
			status_flags &= ~CANPUSH
...
/mob/living/silicon/robot/verb/cmd_robot_alerts()
	...
	if(usr.stat == DEAD)
		src.text2tab("<span class='userdanger'>Alert: You are dead.</span>")
...
/mob/living/silicon/robot/proc/toggle_ionpulse()
	if(!ionpulse)
		src.text2tab("<span class='notice'>No thrusters are installed!</span>")
		return
	...
	src.text2tab("<span class='notice'>You [ionpulse_on ? null :"de"]activate your ion thrusters.</span>")
	if(ionpulse_on)
		ion_trail.start()
	else
		ion_trail.stop()
...
/mob/living/silicon/robot/attackby(obj/item/weapon/W, mob/user, params)
	if(istype(W, /obj/item/weapon/weldingtool) && (user.a_intent != "harm" || user == src))
		...
		if (!getBruteLoss())
			user.text2tab("<span class='warning'>[src] is already in good condition!</span>")
			return
		if (WT.remove_fuel(0, user)) //The welder has 1u of fuel consumed by it's afterattack, so we don't need to worry about taking any away.
			if(src == user)
				user.text2tab("<span class='notice'>You start fixing youself...</span>")
		...
		if (getFireLoss() > 0)
			...
			if (coil.use(1))
				...
				user.text2tab("<span class='warning'>You need more cable to repair [src]!</span>")
		else
			user.text2tab("The wires seem fine, there's no need to fix them.")

	else if(istype(W, /obj/item/weapon/crowbar))	// crowbar means open or close the cover
		if(opened)
			user.text2tab("<span class='notice'>You close the cover.</span>")
			opened = 0
			update_icons()
		else
			if(locked)
				user.text2tab("<span class='warning'>The cover is locked and cannot be opened!</span>")
			else
				user.text2tab("<span class='notice'>You open the cover.</span>")
				opened = 1
				update_icons()

	else if(istype(W, /obj/item/weapon/stock_parts/cell) && opened)	// trying to put a cell inside
		if(wiresexposed)
			user.text2tab("<span class='warning'>Close the cover first!</span>")
		else if(cell)
			user.text2tab("<span class='warning'>There is a power cell already installed!</span>")
		else
			if(!user.drop_item())
				return
			W.loc = src
			cell = W
			user.text2tab("<span class='notice'>You insert the power cell.</span>")
		update_icons()
		diag_hud_set_borgcell()
		...
		if (wiresexposed)
			...
			user.text2tab("<span class='warning'>You can't reach the wiring!</span>")

	else if(istype(W, /obj/item/weapon/screwdriver) && opened && !cell)	// haxing
		wiresexposed = !wiresexposed
		user.text2tab("The wires have been [wiresexposed ? "exposed" : "unexposed"]")
		update_icons()
		...
		if(radio)
			...
			user.text2tab("<span class='warning'>Unable to locate a radio!</span>")
		update_icons()
		...
		if(!lockcharge)
			user.text2tab("<span class='boldannounce'>[src]'s bolts spark! Maybe you should lock them down first!</span>")
			spark_system.start()
			return
		else
			playsound(src, 'sound/items/Ratchet.ogg', 50, 1)
			user.text2tab("<span class='notice'>You start to unfasten [src]'s securing bolts...</span>")
		...
		if(!opened)
			user.text2tab("<span class='warning'>You need access to the robot's insides to do that!</span>")
			return
		if(wiresexposed)
			user.text2tab("<span class='warning'>You need to close the wire panel to do that!</span>")
			return
		if(!cell)
			user.text2tab("<span class='warning'>You need to install a power cell to do that!</span>")
			return
		if(emagged || (connected_ai && lawupdate)) //Can't be sure which, metagamers
			emote("buzz-[user.name]")
			return
		if(!mind) //A player mind is required for law procs to run antag checks.
			user.text2tab("<span class='warning'>[src] is entirely unresponsive!</span>")
			return
		...
		if(emagged)//still allow them to open the cover
			user.text2tab("<span class='notice'>The interface seems slightly damaged.</span>")
		if(opened)
			user.text2tab("<span class='warning'>You must close the cover to swipe an ID card!</span>")
		else
			if(allowed(usr))
				locked = !locked
				user.text2tab("<span class='notice'>You [ locked ? "lock" : "unlock"] [src]'s cover.</span>")
				update_icons()
			else
		...
		if(!opened)
			user.text2tab("<span class='warning'>You must access the borgs internals!</span>")
		else if(!src.module && U.require_module)
			user.text2tab("<span class='warning'>The borg must choose a module before it can be upgraded!</span>")
		else if(U.locked)
			user.text2tab("<span class='warning'>The upgrade is locked and cannot be used yet!</span>")
		else
			if(!user.drop_item())
				return
			if(U.action(src))
				user.text2tab("<span class='notice'>You apply the upgrade to [src].</span>")
				U.loc = src
			else
				user.text2tab("<span class='danger'>Upgrade error.</span>")

	else if(istype(W, /obj/item/device/toner))
		if(toner >= tonermax)
			user.text2tab("<span class='warning'>The toner level of [src] is at it's highest level possible!</span>")
		else
			if(!user.drop_item())
				return
			toner = tonermax
			qdel(W)
			user.text2tab("<span class='notice'>You fill the toner level of [src] to its max capacity.</span>")
	else
		return ..()
...
/mob/living/silicon/robot/emag_act(mob/user)
	if(user != src)//To prevent syndieborgs from emagging themselves
		if(!opened)//Cover is closed
			if(locked)
				user.text2tab("<span class='notice'>You emag the cover lock.</span>")
				locked = 0
			else
				user.text2tab("<span class='warning'>The cover is already unlocked!</span>")
			return
		if(opened)//Cover is open
			...
			if(syndicate)
				user.text2tab("<span class='notice'>You emag [src]'s interface.</span>")
				src.text2tab("<span class='danger'>ALERT: Foreign software execution prevented.</span>")
			...
			if(ai_is_antag)
				user.text2tab("<span class='notice'>You emag [src]'s interface.</span>")
				src.text2tab("<span class='danger'>ALERT: Foreign software execution prevented.</span>")
				connected_ai.text2tab("<span class='danger'>ALERT: Cyborg unit \[[src]] successfuly defended against subversion.</span>")
				log_game("[key_name(user)] attempted to emag cyborg [key_name(src)] slaved to traitor AI [connected_ai].")
				emag_cooldown = world.time
				return

			if(wiresexposed)
				user.text2tab("<span class='warning'>You must unexpose the wires first!</span>")
				return
			else
				emag_cooldown = world.time
				sleep(6)
				...
				src.text2tab("<span class='danger'>ALERT: Foreign software detected.</span>")
				sleep(5)
				src.text2tab("<span class='danger'>Initiating diagnostics...</span>")
				sleep(20)
				src.text2tab("<span class='danger'>SynBorg v1.7 loaded.</span>")
				sleep(5)
				src.text2tab("<span class='danger'>LAW SYNCHRONISATION ERROR</span>")
				sleep(5)
				src.text2tab("<span class='danger'>Would you like to send a report to NanoTraSoft? Y/N</span>")
				sleep(10)
				src.text2tab("<span class='danger'>> N</span>")
				sleep(20)
				src.text2tab("<span class='danger'>ERRORERRORERROR</span>")
				src.text2tab("<b>Obey these laws:</b>")
				laws.show_laws(src)
				src.text2tab("<span class='danger'>ALERT: [user.real_name] is your new master. Obey your new laws and their commands.</span>")
				SetLockdown(0)
				update_icons()
	...
	if(locked)
		switch(alert("You can not lock your cover again, are you sure?\n      (You can still ask for a human to lock it)", "Unlock Own Cover", "Yes", "No"))
			if("Yes")
				locked = 0
				update_icons()
				usr.text2tab("<span class='notice'>You unlock your cover.</span>")
...
/mob/living/silicon/robot/attack_hand(mob/living/carbon/human/user)
	...
	if(opened && !wiresexposed && (!istype(user, /mob/living/silicon)))
		if(cell)
			...
			user.text2tab("<span class='notice'>You remove \the [cell].</span>")
			cell = null
			update_icons()
			diag_hud_set_borgcell()
	...
	if (href_list["deact"])
		...
		if(activated(O))
			if(module_state_1 == O)
				...
				src.text2tab("Module isn't activated.")
		else
			src.text2tab("Module isn't activated")
		installed_modules()
				...
				for(var/A in tile)
					if(istype(A, /obj/effect))
						...
						if(cleaned_human.lying)
							...
							cleaned_human.text2tab("<span class='danger'>[src] cleans your face!</span>")
			return
...
/mob/living/silicon/robot/proc/ResetSecurityCodes()
	...
	if(R)
		R.UnlinkSelf()
		R.text2tab("Buffers flushed and reset. Camera system shutdown.  All systems operational.")
...
/mob/living/silicon/robot/proc/control_headlamp()
	if(stat || lamp_recharging || low_power_mode)
		src.text2tab("<span class='danger'>This function is currently offline.</span>")
		return

//Some sort of magical "modulo" thing which somehow increments lamp power by 2, until it hits the max and resets to 0.
	lamp_intensity = (lamp_intensity+2) % (lamp_max+2)
	src.text2tab("[lamp_intensity ? "Headlamp power set to Level [lamp_intensity/2]" : "Headlamp disabled."]")
	update_headlamp()
...
/mob/living/silicon/robot/proc/update_headlamp(var/turn_off = 0, var/cooldown = 100)
	...
	if(lamp_intensity && (turn_off || stat || low_power_mode))
		src.text2tab("<span class='danger'>Your headlamp has been deactivated.</span>")
		lamp_intensity = 0
		lamp_recharging = 1
...
/mob/living/silicon/robot/proc/notify_ai(notifytype, oldname, newname)
	...
	switch(notifytype)
		if(1) //New Cyborg
			connected_ai.text2tab("<br><br><span class='notice'>NOTICE - New cyborg connection detected: <a href='?src=\ref[connected_ai];track=[html_encode(name)]'>[name]</a></span><br>")
		if(2) //New Module
			connected_ai.text2tab("<br><br><span class='notice'>NOTICE - Cyborg module change detected: [name] has loaded the [designation] module.</span><br>")
		if(3) //New Name
			connected_ai.text2tab("<br><br><span class='notice'>NOTICE - Cyborg reclassification detected: [oldname] is now designated as [newname].</span><br>")
...
/mob/living/silicon/robot/updatehealth()
	...
	if(health < maxHealth*0.5) //Gradual break down of modules as more damage is sustained
		if(uneq_module(module_state_3))
			src.text2tab("<span class='warning'>SYSTEM ERROR: Module 3 OFFLINE.</span>")
		if(health < 0)
			if(uneq_module(module_state_2))
				src.text2tab("<span class='warning'>SYSTEM ERROR: Module 2 OFFLINE.</span>")
			if(health < -maxHealth*0.5)
				if(uneq_module(module_state_1))
					src.text2tab("<span class='warning'>CRITICAL ERROR: All modules OFFLINE.</span>")
