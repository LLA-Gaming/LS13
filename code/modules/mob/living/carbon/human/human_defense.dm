...
/mob/living/carbon/human/emp_act(severity)
	...
	for(var/obj/item/bodypart/L in src.bodyparts)
		if(L.status == ORGAN_ROBOTIC)
			if(!informed)
				src.text2tab("<span class='userdanger'>You feel a sharp pain as your robotic limbs overload.</span>")
				informed = 1
			switch(severity)
				if(1)
					L.take_damage(0,10)
...
/mob/living/carbon/human/acid_act(acidpwr, toxpwr, acid_volume)
	...
	if(head_clothes)
		if(!head_clothes.unacidable)
			...
			src.text2tab("<span class='notice'>Your [head_clothes.name] protects your head and face from the acid!</span>")
	else
		. = get_bodypart("head")
		if(.)
			damaged += .
	...
	if(chest_clothes)
		if(!chest_clothes.unacidable)
			...
			src.text2tab("<span class='notice'>Your [chest_clothes.name] protects your body from the acid!</span>")
	else
		. = get_bodypart("chest")
		if(.)
			damaged += .
	...
	if(arm_clothes)
		if(!arm_clothes.unacidable)
			...
			src.text2tab("<span class='notice'>Your [arm_clothes.name] protects your arms and hands from the acid!</span>")
	else
		. = get_bodypart("r_arm")
		if(.)
			damaged += .
	...
	if(leg_clothes)
		if(!leg_clothes.unacidable)
			...
			src.text2tab("<span class='notice'>Your [leg_clothes.name] protects your legs and feet from the acid!</span>")
	else
		. = get_bodypart("r_leg")
		if(.)
			damaged += .
