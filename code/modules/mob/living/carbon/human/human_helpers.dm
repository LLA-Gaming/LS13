...
/mob/living/carbon/human/can_use_guns(var/obj/item/weapon/gun/G)
	...
	if(G.trigger_guard == TRIGGER_GUARD_NORMAL)
		if(src.dna.check_mutation(HULK))
			src.text2tab("<span class='warning'>Your meaty finger is much too large for the trigger guard!</span>")
			return 0
		if(NOGUNS in src.dna.species.specflags)
			src.text2tab("<span class='warning'>Your fingers don't fit in the trigger guard!</span>")
			return 0

	if(martial_art && martial_art.name == "The Sleeping Carp") //great dishonor to famiry
		src.text2tab("<span class='warning'>Use of ranged weaponry would bring dishonor to the clan.</span>")
		return 0
