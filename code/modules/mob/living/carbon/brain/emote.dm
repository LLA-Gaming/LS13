/mob/living/carbon/brain/emote(act,m_type=1,message = null)
	...
	switch(act)
		if ("alarm")
			src.text2tab("You sound an alarm.")
			message = "<B>[src]</B> sounds an alarm."
			m_type = 2

		if ("alert")
			src.text2tab("You let out a distressed noise.")
			message = "<B>[src]</B> lets out a distressed noise."
			m_type = 2

		if ("beep","beeps")
			src.text2tab("You beep.")
		...
		if ("notice")
			src.text2tab("You play a loud tone.")
			message = "<B>[src]</B> plays a loud tone."
			m_type = 2

		if ("whistle","whistles")
			src.text2tab("You whistle.")
			message = "<B>[src]</B> whistles."
			m_type = 2

		if ("help")
			src.text2tab("Help for MMI emotes. You can use these emotes with say \"*emote\":\nalarm, alert, beep, blink, boop, flash, notice, whistle")
	...
	if (message)
		...
		for(var/mob/M in dead_mob_list)
			...
			if(M.stat == DEAD && !M.IsInVR() && (M.client && (M.client.prefs.chat_toggles & CHAT_GHOSTSIGHT)) && !(M in viewers(src,null)))
