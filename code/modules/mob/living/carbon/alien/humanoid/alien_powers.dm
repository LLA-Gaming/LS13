...
/obj/effect/proc_holder/alien/proc/cost_check(check_turf=0,mob/living/carbon/user,silent = 0)
	if(user.stat)
		if(!silent)
			user.text2tab("<span class='noticealien'>You must be conscious to do this.</span>")
		return 0
	if(user.getPlasma() < plasma_cost)
		if(!silent)
			user.text2tab("<span class='noticealien'>Not enough plasma stored.</span>")
		return 0
	if(check_turf && (!isturf(user.loc) || istype(user.loc, /turf/open/space)))
		if(!silent)
			user.text2tab("<span class='noticealien'>Bad place for a garden!</span>")
		return 0
	return 1
...
/obj/effect/proc_holder/alien/plant/fire(mob/living/carbon/user)
	if(locate(/obj/structure/alien/weeds/node) in get_turf(user))
		user.text2tab("There's already a weed node here.")
		return 0
...
/obj/effect/proc_holder/alien/whisper/fire(mob/living/carbon/user)
	...
	if(msg)
		...
		M.text2tab("<span class='noticealien'>You hear a strange, alien voice in your head...</span>[msg]")
		user.text2tab("<span class='noticealien'>You said: \"[msg]\" to [M]</span>")
		for(var/mob/ded in dead_mob_list)
			if(!isobserver(ded))
				continue
			var/follow_link_user = FOLLOW_LINK(ded, user)
			var/follow_link_whispee = FOLLOW_LINK(ded, M)
			ded.text2tab("[follow_link_user] \
...
/obj/effect/proc_holder/alien/transfer/fire(mob/living/carbon/user)
	...
	if (amount)
		...
		if (get_dist(user,M) <= 1)
			...
			M.text2tab("<span class='noticealien'>[user] has transfered [amount] plasma to you.</span>")
			user.text2tab("<span class='noticealien'>You trasfer [amount] plasma to [M]</span>")
		else
			user.text2tab("<span class='noticealien'>You need to be closer!</span>")
	return
	...
	if(target in oview(1,user))
		...
		if(isobj(target))
			...
			if(I.unacidable)	//So the aliens don't destroy energy fields/singularies/other aliens/etc with their acid.
				user.text2tab("<span class='noticealien'>You cannot dissolve this object.</span>")
				return 0
		// TURF CHECK
		...
		user.text2tab("<span class='noticealien'>Target is too far away.</span>")
		return 0
...
/obj/effect/proc_holder/alien/neurotoxin/fire(mob/living/carbon/user)
	if(active)
		...
		user.text2tab("<span class='notice'>You empty your neurotoxin gland.</span>")
		active = 0
	else if(user.ranged_ability && user.ranged_ability != src)
		user.text2tab("<span class='warning'>You already have another aimed ability readied! Cancel it first.")
		return
	else
		user.ranged_ability = src
		active = 1
		user.text2tab("<span class='notice'>You prepare your neurotoxin gland. <B>Left-click to fire at a target!</B></span>")
...
/obj/effect/proc_holder/alien/neurotoxin/InterceptClickOn(mob/living/carbon/user, params, atom/target)
	...
	if(user.getPlasma() < p_cost)
		user.text2tab("<span class='warning'>You need at least [p_cost] plasma to spit.</span>")
		return
...
/obj/effect/proc_holder/alien/resin/fire(mob/living/carbon/user)
	if(locate(/obj/structure/alien/resin) in user.loc)
		user.text2tab("<span class='danger'>There is already a resin structure there.</span>")
		return 0
	...
	if (!cost_check(check_turf,user))
		return 0
	user.text2tab("<span class='notice'>You shape a [choice].</span>")
...
/obj/effect/proc_holder/alien/sneak/fire(mob/living/carbon/alien/humanoid/user)
	if(!active)
		...
		user.sneaking = 1
		active = 1
		user.text2tab("<span class='noticealien'>You blend into the shadows...</span>")
	else
		user.alpha = initial(user.alpha)
		user.sneaking = 0
		active = 0
		user.text2tab("<span class='noticealien'>You reveal yourself!</span>")
