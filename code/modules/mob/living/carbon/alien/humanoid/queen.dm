...
/obj/effect/proc_holder/alien/lay_egg/fire(mob/living/carbon/user)
	if(locate(/obj/structure/alien/egg) in get_turf(user))
		user.text2tab("There's already an egg here.")
		return 0
...
/obj/effect/proc_holder/alien/royal/queen/promote/fire(mob/living/carbon/alien/user)
	...
	if(alien_type_present(/mob/living/carbon/alien/humanoid/royal/praetorian/))
		user.text2tab("<span class='noticealien'>You already have a Praetorian!</span>")
		return 0
	else
		for(prom in user)
			user.text2tab("<span class='noticealien'>You discard [prom].</span>")
			qdel(prom)
			return 0

		prom = new (user.loc)
		if(!user.put_in_active_hand(prom, 1))
			user.text2tab("<span class='warning'>You must empty your hands before preparing the parasite.</span>")
			return 0
		else //Just in case telling the player only once is not enough!
			user.text2tab("<span class='noticealien'>Use the royal parasite on one of your children to promote her to Praetorian!</span>")
	return 0
...
/obj/item/queenpromote/attack(mob/living/M, mob/living/carbon/alien/humanoid/user)
	if(!isalienadult(M) || istype(M, /mob/living/carbon/alien/humanoid/royal))
		user.text2tab("<span class='noticealien'>You may only use this with your adult, non-royal children!</span>")
		return
	if(alien_type_present(/mob/living/carbon/alien/humanoid/royal/praetorian/))
		user.text2tab("<span class='noticealien'>You already have a Praetorian!</span>")
		return
	...
	if(A.stat == CONSCIOUS && A.mind && A.key)
		if(!user.usePlasma(500))
			user.text2tab("<span class='noticealien'>You must have 500 plasma stored to use this!</span>")
			return

		A.text2tab("<span class='noticealien'>The queen has granted you a promotion to Praetorian!</span>")
		...
		user.text2tab("<span class='warning'>This child must be alert and responsive to become a Praetorian!</span>")

/obj/item/queenpromote/attack_self(mob/user)
	user.text2tab("<span class='noticealien'>You discard [src].</span>")
	qdel(src)
