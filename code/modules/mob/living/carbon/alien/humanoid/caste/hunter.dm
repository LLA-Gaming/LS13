...
/mob/living/carbon/alien/humanoid/hunter/proc/toggle_leap(message = 1)
	...
	update_icons()
	if(message)
		src.text2tab("<span class='noticealien'>You will now [leap_on_click ? "leap at":"slash at"] enemies!</span>")
	else
		return
...
/mob/living/carbon/alien/humanoid/hunter/proc/leap_at(atom/A)
	if(pounce_cooldown)
		src.text2tab("<span class='alertalien'>You are too fatigued to pounce right now!</span>")
		return
	...
	if(!has_gravity(src) || !has_gravity(A))
		src.text2tab("<span class='alertalien'>It is unsafe to leap without gravity!</span>")
