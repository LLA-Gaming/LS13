...
/obj/item/organ/body_egg/alien_embryo/on_find(mob/living/finder)
	..()
	if(stage < 4)
		finder.text2tab("It's small and weak, barely the size of a foetus.")
	else
		finder.text2tab("It's grown quite large, and writhes slightly as you look at it.")
		if(prob(10))
			AttemptGrow(0)
	...
	switch(stage)
		if(2, 3)
			...
			if(prob(2))
				owner.emote("cough")
			if(prob(2))
				owner.text2tab("<span class='danger'>Your throat feels sore.</span>")
			if(prob(2))
				owner.text2tab("<span class='danger'>Mucous runs down the back of your throat.</span>")
		if(4)
			if(prob(2))
				owner.emote("sneeze")
			if(prob(2))
				owner.emote("cough")
			if(prob(4))
				owner.text2tab("<span class='danger'>Your muscles ache.</span>")
				if(prob(20))
					owner.take_organ_damage(1)
			if(prob(4))
				owner.text2tab("<span class='danger'>Your stomach hurts.</span>")
				if(prob(20))
					owner.adjustToxLoss(1)
		if(5)
			owner.text2tab("<span class='danger'>You feel something tearing its way out of your stomach...</span>")
