...
/obj/effect/proc_holder/alien/larva_evolve/fire(mob/living/carbon/alien/user)
	...
	if(L.handcuffed || L.legcuffed) // Cuffing larvas ? Eh ?
		user.text2tab("<span class='danger'>You cannot evolve when you are cuffed.</span>")

	if(L.amount_grown >= L.max_grown)	//TODO ~Carn
		L.text2tab("<span class='name'>You are growing into a beautiful alien! It is time to choose a caste.</span>")
		L.text2tab("<span class='info'>There are three to choose from:")
		L.text2tab("<span class='name'>Hunters</span> <span class='info'>are the most agile caste tasked with hunting for hosts. They are faster than a human and can even pounce, but are not much tougher than a drone.</span>")
		L.text2tab("<span class='name'>Sentinels</span> <span class='info'>are tasked with protecting the hive. With their ranged spit, invisibility, and high health, they make formidable guardians and acceptable secondhand hunters.</span>")
		L.text2tab("<span class='name'>Drones</span> <span class='info'>are the weakest and slowest of the castes, but can grow into the queen if there is none, and are vital to maintaining a hive with their resin secretion abilities.</span>")
		...
		user.text2tab("<span class='danger'>You are not fully grown.</span>")
		return 0
