	...
	if(item_in_hand) //this segment checks if the item in your hand is twohanded.
		if(istype(item_in_hand,/obj/item/weapon/twohanded))
			if(item_in_hand:wielded == 1)
				usr.text2tab("<span class='warning'>Your other hand is too busy holding the [item_in_hand.name]</span>")
				return
	src.hand = !( src.hand )
...
/mob/living/carbon/proc/help_shake_act(mob/living/carbon/M)
	if(on_fire)
		M.text2tab("<span class='warning'>You can't put them out with just your bare hands!")
		return
...
/mob/living/carbon/flash_eyes(intensity = 1, override_blindness_check = 0, affect_silicon = 0, visual = 0)
	...
	if(.) // we've been flashed
		...
		if (damage == 1)
			src.text2tab("<span class='warning'>Your eyes sting a little.</span>")
			if(prob(40))
				adjust_eye_damage(1)

		else if (damage == 2)
			src.text2tab("<span class='warning'>Your eyes burn.</span>")
			adjust_eye_damage(rand(2, 4))

		else if( damage > 3)
			src.text2tab("<span class='warning'>Your eyes itch and burn severely!</span>")
			...
			if(eye_damage > 20)
				if(prob(eye_damage - 20))
					if(become_nearsighted())
						src.text2tab("<span class='warning'>Your eyes start to burn badly!</span>")
				else if(prob(eye_damage - 25))
					if(become_blind())
						src.text2tab("<span class='warning'>You can't see anything!</span>")
			else
				src.text2tab("<span class='warning'>Your eyes are really starting to hurt. This can't be good for you!</span>")
		if(has_bane(BANE_LIGHT))
			mind.disrupt_spells(-500)
		return 1
	else if(damage == 0) // just enough protection
		if(prob(20))
			src.text2tab("<span class='notice'>Something bright flashes in the corner of your vision!</span>")
		if(has_bane(BANE_LIGHT))
...
/mob/living/carbon/resist_buckle()
	if(restrained())
		...
		if(do_after(src, 600, 0, target = src))
			...
			if(src && buckled)
				src.text2tab("<span class='warning'>You fail to unbuckle yourself!</span>")
	else
		buckled.user_unbuckle_mob(src,src)
...
/mob/living/carbon/proc/cuff_resist(obj/item/I, breakouttime = 600, cuff_break = 0)
	...
	if(!cuff_break)
		...
		src.text2tab("<span class='notice'>You attempt to remove [I]... (This will take around [displaytime] minutes and you need to stand still.)</span>")
		if(do_after(src, breakouttime, 0, target = src))
			clear_cuffs(I, cuff_break)
		else
			src.text2tab("<span class='warning'>You fail to remove [I]!</span>")

	else if(cuff_break == FAST_CUFFBREAK)
		breakouttime = 50
		visible_message("<span class='warning'>[src] is trying to break [I]!</span>")
		src.text2tab("<span class='notice'>You attempt to break [I]... (This will take around 5 seconds and you need to stand still.)</span>")
		if(do_after(src, breakouttime, 0, target = src))
			clear_cuffs(I, cuff_break)
		else
			src.text2tab("<span class='warning'>You fail to break [I]!</span>")
...
/mob/living/carbon/proc/clear_cuffs(obj/item/I, cuff_break)
	...
	src.text2tab("<span class='notice'>You successfully [cuff_break ? "break" : "remove"] [I].</span>")
...
/mob/living/carbon/harvest(mob/living/user)
	...
	for(var/obj/item/organ/O in internal_organs)
		if(prob(50))
			...
			O.loc = get_turf(src)
	if(organs_amt)
		user.text2tab("<span class='notice'>You retrieve some of [src]\'s internal organs!</span>")
