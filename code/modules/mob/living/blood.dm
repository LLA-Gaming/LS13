	...
	if(bleedsuppress)
		...
		spawn(amount)
			...
			if(stat != DEAD && bleed_rate)
				src.text2tab("<span class='warning'>The blood soaks through your bandage.</span>")
	...
	if(bodytemperature >= 225 && !(disabilities & NOCLONE)) //cryosleep or husked people do not pump the blood.
		...
		switch(blood_volume)
			if(BLOOD_VOLUME_OKAY to BLOOD_VOLUME_SAFE)
				if(prob(5))
					src.text2tab("<span class='warning'>You feel [pick("dizzy","woozy","faint")].</span>")
			...
			if(BLOOD_VOLUME_BAD to BLOOD_VOLUME_OKAY)
				...
				if(prob(5))
					...
					src.text2tab("<span class='warning'>You feel very [word].</span>")
			if(BLOOD_VOLUME_SURVIVE to BLOOD_VOLUME_BAD)
				adjustOxyLoss(5)
				if(prob(15))
					Paralyse(rand(1,3))
					var/word = pick("dizzy","woozy","faint")
					src.text2tab("<span class='warning'>You feel extremely [word].</span>")
