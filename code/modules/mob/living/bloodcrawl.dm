...
/mob/living/proc/phaseout(obj/effect/decal/cleanable/B)
	if(iscarbon(src))
		...
		if(C.l_hand || C.r_hand)
			...
			C.text2tab("<span class='warning'>You may not hold items while blood crawling!</span>")
			return 0
...
/mob/living/proc/bloodpool_sink(obj/effect/decal/cleanable/B)
	...
	if(kidnapped)
		...
		if(!success)
			src.text2tab("<span class='danger'>You happily devour... nothing? Your meal vanished at some point!</span>")
	return 1

/mob/living/proc/bloodcrawl_consume(mob/living/victim)
	src.text2tab("<span class='danger'>You begin to feast on [victim]. You can not move while you are doing this.</span>")
	...
	if(victim.reagents && victim.reagents.has_reagent("devilskiss"))
		src.text2tab("<span class='warning'><b>AAH! THEIR FLESH! IT BURNS!</b></span>")
	...
	src.text2tab("<span class='danger'>You devour [victim]. Your health is fully restored.</span>")
...
/mob/living/proc/phasein(obj/effect/decal/cleanable/B)
	if(src.notransform)
		src.text2tab("<span class='warning'>Finish eating first!</span>")
		return 0
