...
/mob/living/emote(act, m_type=1, message = null)
 ...
 // Maybe some people are okay with that.
		...
		for(var/mob/M in dead_mob_list)
			...
			var/T = get_turf(src)
			if(M.stat == DEAD && !M.IsInVR() && M.client && (M.client.prefs.chat_toggles & CHAT_GHOSTSIGHT) && !(M in viewers(T,null)))
