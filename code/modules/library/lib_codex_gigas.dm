...
/obj/item/weapon/book/codex_gigas/attack_self(mob/user)
	...
	if(ismonkey(user))
		...
		return
	if(inUse)
		user.text2tab("<span class='notice'>Someone else is reading it.</span>")
	if(ishuman(user))
		var/mob/living/carbon/human/U = user
		if(U.check_acedia())
			user.text2tab("<span class='notice'>None of this matters, why are you reading this?  You put the [title] down.</span>")
			return
		inUse = 1
		...
		if(U.job in list("Librarian")) // the librarian is both faster, and more accurate than normal crew members at research
			...
			correctness = 100
			willpower = 100
		if(U.job in list("Station Manager", "Security Officer", "Head of Security", "Detective", "Warden"))
			willpower = 99
