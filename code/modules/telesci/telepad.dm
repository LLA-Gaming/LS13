...
/obj/machinery/telepad/New()
	..()
	wires = new /datum/wires/telepad(src)
...
/obj/machinery/telepad/attackby(obj/item/I, mob/user, params)
	...
	if(panel_open)
		if(istype(I, /obj/item/device/multitool) && !cant_link)
			...
			M.buffer = src
			user.text2tab("<span class='caution'>You save the data in the [I.name]'s buffer.</span>")
			return 1
...
/obj/machinery/telepad_cargo/attackby(obj/item/weapon/W, mob/user, params)
	if(istype(W, /obj/item/weapon/wrench))
		...
		if(anchored)
			anchored = 0
			user.text2tab("<span class='caution'>\The [src] can now be moved.</span>")
		else if(!anchored)
			anchored = 1
			user.text2tab("<span class='caution'>\The [src] is now secured.</span>")
	else if(istype(W, /obj/item/weapon/screwdriver))
		if(stage == 0)
			playsound(src, 'sound/items/Screwdriver.ogg', 50, 1)
			user.text2tab("<span class='caution'>You unscrew the telepad's tracking beacon.</span>")
			stage = 1
		else if(stage == 1)
			playsound(src, 'sound/items/Screwdriver.ogg', 50, 1)
			user.text2tab("<span class='caution'>You screw in the telepad's tracking beacon.</span>")
			stage = 0
		...
		if(WT.remove_fuel(0,user))
			...
			if(do_after(user,20/WT.toolspeed, target = src))
				if(!WT.isOn())
					return
				user.text2tab("<span class='notice'>You disassemble [src].</span>")
...
/obj/item/device/telepad_beacon/attack_self(mob/user)
	if(user)
		user.text2tab("<span class='caution'>Locked In</span>")
...
/obj/item/weapon/rcs/examine(mob/user)
	..()
	user.text2tab("There are [rcharges] charge\s left.")
...
/obj/item/weapon/rcs/attack_self(mob/user)
	if(emagged)
		if(mode == 0)
			...
			user.text2tab("<span class='caution'>The telepad locator has become uncalibrated.</span>")
		else
			mode = 0
			playsound(src.loc, 'sound/effects/pop.ogg', 50, 0)
			user.text2tab("<span class='caution'>You calibrate the telepad locator.</span>")
...
/obj/item/weapon/rcs/emag_act(mob/user)
	if(!emagged)
		...
		s.set_up(5, 1, src)
		s.start()
		user.text2tab("<span class='caution'>You emag the RCS. Click on it to toggle between modes.</span>")
