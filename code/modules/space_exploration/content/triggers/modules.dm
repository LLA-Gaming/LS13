/obj/effect/trigger_modules
	...
	conditional/
		...
		spoken_phrase/
			...
			Hear(var/msg, var/mob/living/M as mob)
				if(_exact)
					if(msg == _phrase)
						triggered = 1
					return 0
	...
	completion/
		...
		spawn_effect/
			...
			OnCompleted()
				var/datum/effect_system/system

				switch(_effect_type)
					if("smoke")
						system = new /datum/effect_system/smoke_spread()
						system.set_up(_amount, 0, get_turf(src))
						system.start()

					if("spark")
						system = new/datum/effect_system/spark_spread()
						system.set_up(_amount, 0, get_turf(src))
						system.start()

					if("lightning")
						system = new /datum/effect_system/lightning_spread()
						system.set_up(_amount, 0, get_turf(src))
						system.start()

					if("foam")
						system = new /datum/effect_system/foam_spread()
						system.set_up(_amount, get_turf(src))
						system.start()

					if("steam")
						system = new /datum/effect_system/steam_spread()
