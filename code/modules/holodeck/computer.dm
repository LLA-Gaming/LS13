...
/obj/machinery/computer/holodeck/emag_act(mob/user as mob)
	if(!emagged)
		if(!emag_programs.len)
			user.text2tab("[src] does not seem to have a card swipe port.  It must be an inferior model.")
			return
		playsound(loc, 'sound/effects/sparks4.ogg', 75, 1)
		emagged = 1
		user.text2tab("<span class='warning'>You vastly increase projector power and override the safety and security protocols.</span>")
		user.text2tab("Warning.  Automatic shutoff and derezing protocols have been corrupted.  Please call Nanotrasen maintenance and do not use the simulator.")
