...
/obj/item/weapon/dice/d20/fate/diceroll(mob/user)
	...
	if(!used)
		if(!ishuman(user) || !user.mind || (user.mind in ticker.mode.wizards))
			user.text2tab("<span class='warning'>You feel the magic of the dice is restricted to ordinary humans!</span>")
			return
		if(rigged > 0)
			effect(user,rigged)
		else
...
/obj/item/weapon/dice/d20/fate/equipped(mob/user, slot)
	if(!ishuman(user) || !user.mind || (user.mind in ticker.mode.wizards))
		user.text2tab("<span class='warning'>You feel the magic of the dice is restricted to ordinary humans! You should leave it alone.</span>")
		user.drop_item()
