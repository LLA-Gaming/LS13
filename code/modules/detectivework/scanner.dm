...
/obj/item/device/detective_scanner/attack_self(mob/user)
	if(log.len && !scanning)
		scanning = 1
		user.text2tab("<span class='notice'>Printing report, please wait...</span>")
		...
		spawn(100)
			...
			if(ismob(loc))
				var/mob/M = loc
				M.put_in_hands(P)
				M.text2tab("<span class='notice'>Report printed. Log cleared.<span>")

			// Clear the logs
			log = list()
			scanning = 0
	else
		user.text2tab("<span class='notice'>The scanner has no logs or is in use.</span>")
...
/obj/item/device/detective_scanner/proc/scan(atom/A, mob/user)
	...
	if(!scanning)
		...
		user.text2tab("<span class='notice'>You scan \the [A]. The scanner is now analysing the results...</span>")
		...
		spawn(0)
			...
			if(!found_something)
				...
				if(holder)
					holder.text2tab("<span class='warning'>Unable to locate any fingerprints, materials, fibers, or blood on \the [target_name]!</span>")
			else
				if(holder)
					holder.text2tab("<span class='notice'>You finish scanning \the [target_name].</span>")
