...
/area/virtual_reality
	...
	var/list/forbidden_perseus_items = list(/obj/item/weapon/grenade/plastic/c4/breach)
	...
	Exited(var/atom/movable/A, var/atom/new_loc)
		if(isliving(A))
			...
			if(!istype(get_area(L), /area/virtual_reality))
				...
				spawn(0)	L.loc = safepick(get_area_turfs(SSvirtual_reality.crew_destination))
