...
/datum/surgery_step/manipulate_organs/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(isorgan(tool))
		...
		if(target_zone != I.zone || target.getorganslot(I.slot))
			user.text2tab("<span class='notice'>There is no room for [I] in [target]'s [parse_zone(target_zone)]!</span>")
			return -1
		...
		if(!organs.len)
			user.text2tab("<span class='notice'>There is no removeable organs in [target]'s [parse_zone(target_zone)]!</span>")
			return -1
		else
		...
		user.text2tab("<span class='warning'>[tool] was biten by someone! It's too damaged to use!</span>")
		return -1
