		...
		if(M.lying || isslime(M))	//if they're prone or a slime
			...
			if(!current_surgery)
				...
				if(P && user && user.Adjacent(M) && (I in user))
					...
					if(procedure)
						...
						if(procedure.ignore_clothes || get_location_accessible(M, selected_zone))
							...
							user.text2tab("<span class='warning'>You need to expose [M]'s [parse_zone(selected_zone)] first!</span>")
				...
				if(current_surgery.status == 1)
					...
					user.text2tab("<span class='warning'>You need to hold a cautery in inactive hand to stop [M]'s surgery!</span>")
