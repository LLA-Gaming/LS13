...
/obj/item/organ/cyberimp/arm/examine(mob/user)
	..()
	user.text2tab("<span class='info'>[src] is assembled in the [zone == "r_arm" ? "right" : "left"] arm configuration. You can use a screwdriver to reassemble it.</span>")
...
/obj/item/organ/cyberimp/arm/attackby(obj/item/weapon/W, mob/user, params)
	...
	if(istype(W, /obj/item/weapon/screwdriver))
		if(zone == "r_arm")
			...
			zone = "r_arm"
		slot = zone + "_device"
		user.text2tab("<span class='notice'>You modify [src] to be installed on the [zone == "r_arm" ? "right" : "left"] arm.</span>")
		update_icon()
...
/obj/item/organ/cyberimp/arm/gun/emp_act(severity)
	if(prob(15/severity) && owner)
		owner.text2tab("<span class='warning'>[src] is hit by EMP!</span>")
...
/obj/item/organ/cyberimp/arm/proc/Extend(var/obj/item/item)
	...
	if(arm_item)
		if(!owner.unEquip(arm_item))
			owner.text2tab("<span class='warning'>Your [arm_item] interferes with [src]!</span>")
			return
		else
			owner.text2tab("<span class='notice'>You drop [arm_item] to activate [src]!</span>")

	if(zone == "r_arm" ? !owner.put_in_r_hand(holder) : !owner.put_in_l_hand(holder))
		owner.text2tab("<span class='warning'>Your [src] fails to activate!</span>")
		return
...
/obj/item/organ/cyberimp/arm/ui_action_click()
	if(crit_fail || (!holder && !contents.len))
		owner.text2tab("<span class='warning'>The implant doesn't respond. It seems to be broken...</span>")
		return
...
/obj/item/organ/cyberimp/arm/gun/emp_act(severity)
	if(prob(30/severity) && owner && !crit_fail)
		...
		owner.text2tab("<span class='userdanger'>You feel an explosion erupt inside your [zone == "r_arm" ? "right" : "left"] arm as your implant breaks!</span>")
...
/obj/item/organ/cyberimp/arm/toolset/emag_act()
	if(!(locate(/obj/item/weapon/kitchen/knife/combat/cyborg) in items_list))
		usr.text2tab("<span class='notice'>You unlock [src]'s integrated knife!</span>")
