...
/obj/item/organ/cyberimp/chest/nutriment/on_life()
	...
	if(owner.nutrition <= hunger_threshold)
		synthesizing = TRUE
		owner.text2tab("<span class='notice'>You feel less hungry...</span>")
		owner.nutrition += 50
		sleep(50)
		synthesizing = FALSE
...
/obj/item/organ/cyberimp/chest/nutriment/emp_act(severity)
	...
	owner.text2tab("<span class='warning'>You feel like your insides are burning.</span>")
...
/obj/item/organ/cyberimp/chest/reviver/proc/undo_heart_attack()
	...
	if(H.stat == CONSCIOUS)
		H.text2tab("<span class='notice'>You feel your heart beating again!</span>")
...
/obj/item/organ/cyberimp/chest/thrusters/proc/toggle(silent=0)
	if(!on)
		if(crit_fail)
			if(!silent)
				owner.text2tab("<span class='warning'>Your thrusters set seems to be broken!</span>")
			return 0
		on = 1
		if(allow_thrust(0.01))
			ion_trail.start()
			if(!silent)
				owner.text2tab("<span class='notice'>You turn your thrusters set on.</span>")
	else
		ion_trail.stop()
		if(!silent)
			owner.text2tab("<span class='notice'>You turn your thrusters set off.</span>")
		on = 0
	update_icon()
