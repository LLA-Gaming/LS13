...
/obj/item/device/autoimplanter/attack_self(mob/user)//when the object it used...
	if(!uses)
		user.text2tab("<span class='warning'>[src] has already been used. The tools are dull and won't reactivate.</span>")
		return
	if(!storedorgan)
		user.text2tab("<span class='notice'>[src] currently has no implant stored.</span>")
		return
...
/obj/item/device/autoimplanter/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/organ/cyberimp))
		if(one_use && !storedorgan)
			user.text2tab("<span class='notice'>[src] doesn't accept implants. It must be a single-use device.</span>")
			return//prevents removing the intended implant to insert your own
		if(storedorgan)
			user.text2tab("<span class='notice'>[src] already has an implant stored.</span>")
			return
		if(!user.drop_item())
			return
		I.loc = src
		storedorgan = I
		user.text2tab("<span class='notice'>You insert the [I] into [src].</span>")
	else if(istype(I, /obj/item/weapon/screwdriver))
		if(!storedorgan)
			user.text2tab("<span class='notice'>There's no implant in [src] for you to remove.</span>")
		else
			var/turf/open/floorloc = get_turf(user)
			floorloc.contents += contents
			user.text2tab("<span class='notice'>You remove the [storedorgan] from [src].</span>")
