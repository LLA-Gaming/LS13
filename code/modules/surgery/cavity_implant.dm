...
/datum/surgery_step/handle_cavity/success(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(tool)
		if(IC || tool.w_class > 3 || (NODROP in tool.flags) || istype(tool, /obj/item/organ))
			user.text2tab("<span class='warning'>You can't seem to fit [tool] in [target]'s [target_zone]!</span>")
			return 0
		else
		...
		if(IC)
			...
			user.text2tab("<span class='warning'>You don't find anything in [target]'s [target_zone].</span>")
			return 0
