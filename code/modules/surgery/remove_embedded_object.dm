	...
	if(L)
		if(ishuman(target))
			...
			if(objects > 0)
				...
				user.text2tab("<span class='warning'>You find no objects embedded in [H]'s [L]!</span>")

	else
		user.text2tab("<span class='warning'>You can't find [target]'s [parse_zone(user.zone_selected)], let alone any objects embedded in it!</span>")
