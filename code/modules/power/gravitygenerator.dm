	...
	switch(broken_state)
		if(GRAV_NEEDS_SCREWDRIVER)
			if(istype(I, /obj/item/weapon/screwdriver))
				user.text2tab("<span class='notice'>You secure the screws of the framework.</span>")
		...
		if(GRAV_NEEDS_WELDING)
			if(istype(I, /obj/item/weapon/weldingtool))
				...
				if(WT.remove_fuel(1, user))
					user.text2tab("<span class='notice'>You mend the damaged framework.</span>")
					playsound(src.loc, 'sound/items/Welder2.ogg', 50, 1)
					broken_state++
					update_icon()
				else if(WT.isOn())
					user.text2tab("<span class='warning'>You don't have enough fuel to mend the damaged framework!</span>")
				return
		if(GRAV_NEEDS_PLASTEEL)
			if(istype(I, /obj/item/stack/sheet/plasteel))
				...
				if(PS.amount >= 10)
					PS.use(10)
					user.text2tab("<span class='notice'>You add the plating to the framework.</span>")
					playsound(src.loc, 'sound/machines/click.ogg', 75, 1)
					broken_state++
					update_icon()
				else
					user.text2tab("<span class='warning'>You need 10 sheets of plasteel!</span>")
				return
		if(GRAV_NEEDS_WRENCH)
			if(istype(I, /obj/item/weapon/wrench))
				user.text2tab("<span class='notice'>You secure the plating to the framework.</span>")
