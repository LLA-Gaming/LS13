...
/obj/machinery/light_construct/examine(mob/user)
	...
	switch(src.stage)
		if(1)
			user.text2tab("It's an empty frame.")
		if(2)
			user.text2tab("It's wired.")
		if(3)
			user.text2tab("The casing is closed.")
	...
	switch(stage)
		if(1)
			if(istype(W, /obj/item/weapon/wrench))
				...
				usr.text2tab("<span class='notice'>You begin deconstructing [src]...</span>")
			...
			if(istype(W, /obj/item/stack/cable_coil))
				...
				if(coil.use(1))
					...
					user.text2tab("<span class='warning'>You need one length of cable to wire [src]!</span>")
				return
		if(2)
			if(istype(W, /obj/item/weapon/wrench))
				usr.text2tab("<span class='warning'>You have to remove the wires first!</span>")
				return
...
/obj/machinery/light/examine(mob/user)
	...
	switch(status)
		if(LIGHT_OK)
			user.text2tab("It is turned [on? "on" : "off"].")
		if(LIGHT_EMPTY)
			user.text2tab("The [fitting] has been removed.")
		if(LIGHT_BURNED)
			user.text2tab("The [fitting] is burnt out.")
		if(LIGHT_BROKEN)
			user.text2tab("The [fitting] has been smashed.")
...
/obj/machinery/light/attackby(obj/item/W, mob/living/user, params)
	...
	// attempt to insert light
		...
		if(status != LIGHT_EMPTY)
			user.text2tab("<span class='warning'>There is a [fitting] already inserted!</span>")
		else
			...
			if(istype(L, light_type))
				if(!user.drop_item())
					return
				status = L.status
				user.text2tab("<span class='notice'>You insert the [L.name].</span>")
				...
				user.text2tab("<span class='warning'>This type of light requires a [fitting]!</span>")
	...
	// attempt to stick weapon into light socket
		...
		if(istype(W, /obj/item/weapon/screwdriver)) //If it's a screwdriver open it.
			...
			user.text2tab("<span class='userdanger'>You stick \the [W] into the light socket!</span>")
...
/obj/machinery/light/attack_hand(mob/living/carbon/human/user)
	...
	if(status == LIGHT_EMPTY)
		user.text2tab("There is no [fitting] in this light.")
		return
	...
	if(on)
		...
		if(prot > 0)
			user.text2tab("<span class='notice'>You remove the light [fitting].</span>")
		else if(istype(user) && user.dna.check_mutation(TK))
			user.text2tab("<span class='notice'>You telekinetically remove the light [fitting].</span>")
		else
			user.text2tab("<span class='warning'>You try to remove the light [fitting], but you burn your hand on it!</span>")
...
/obj/item/weapon/light/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/weapon/reagent_containers/syringe))
		...
		user.text2tab("<span class='notice'>You inject the solution into \the [src].</span>")
