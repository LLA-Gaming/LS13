...
/obj/machinery/power/port_gen/examine(mob/user)
	..()
	user.text2tab("It is[!active?"n't":""] running.")
...
/obj/machinery/power/port_gen/pacman/examine(mob/user)
	..()
	user.text2tab("<span class='notice'>The generator has [sheets] units of [sheet_name] fuel left, producing [power_gen] per cycle.</span>")
	if(crit_fail) user.text2tab("<span class='danger'>The generator seems to have broken down.</span>")
...
/obj/machinery/power/port_gen/pacman/attackby(obj/item/O, mob/user, params)
	if(istype(O, sheet_path))
		...
		if(amount < 1)
			user.text2tab("<span class='notice'>The [src.name] is full!</span>")
			return
		user.text2tab("<span class='notice'>You add [amount] sheets to the [src.name].</span>")
		sheets += amount
		addstack.use(amount)
		updateUsrDialog()
		return
		...
		if(istype(O, /obj/item/weapon/wrench))
			...
			if(!anchored && !isinspace())
				connect_to_network()
				user.text2tab("<span class='notice'>You secure the generator to the floor.</span>")
				anchored = 1
			else if(anchored)
				disconnect_from_network()
				user.text2tab("<span class='notice'>You unsecure the generator from the floor.</span>")
				anchored = 0
			...
			if(panel_open)
				user.text2tab("<span class='notice'>You open the access panel.</span>")
			else
				user.text2tab("<span class='notice'>You close the access panel.</span>")
			return
