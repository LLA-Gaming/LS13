...
/obj/structure/cable/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/weapon/wirecutters))
		...
		if (coil.get_amount() < 1)
			user.text2tab("<span class='warning'>Not enough cable!</span>")
			return
		coil.cable_join(src, user)

	else if(istype(W, /obj/item/device/multitool))
		if(powernet && (powernet.avail > 0))		// is it powered?
			user.text2tab("<span class='danger'>[powernet.avail]W in power network.</span>")
		else
			user.text2tab("<span class='danger'>The cable is not powered.</span>")
		shock(user, 5, 0.2)
...
/obj/item/stack/cable_coil/proc/place_turf(turf/T, mob/user)
	...
	if(!T.can_have_cabling())
		user.text2tab("<span class='warning'>You can only lay cables on catwalks and plating!</span>")
		return

	if(get_amount() < 1) // Out of cable
		user.text2tab("<span class='warning'>There is no cable left!</span>")
		return

	if(get_dist(T,user) > 1) // Too far
		user.text2tab("<span class='warning'>You can't lay cable at a place that far away!</span>")
		return
		...
		for(var/obj/structure/cable/LC in T)
			if(LC.d2 == dirn && LC.d1 == 0)
				user.text2tab("<span class='warning'>There's already a cable at that position!</span>")
				return
...
/obj/item/stack/cable_coil/proc/cable_join(obj/structure/cable/C, mob/user)
	...
	if(C.d1 == dirn || C.d2 == dirn)
		...
		if(U.intact)						//can't place a cable if it's a plating with a tile on it
			user.text2tab("<span class='warning'>You can't lay cable there unless the floor tiles are removed!</span>")
			return
		else
