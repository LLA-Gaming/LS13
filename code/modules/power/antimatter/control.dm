...
/obj/machinery/power/am_control_unit/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/weapon/wrench))
		if(!anchored)
			...
			user.text2tab("<span class='warning'>Once bolted and linked to a shielding unit it the [src.name] is unable to be moved!</span>")

	else if(istype(W, /obj/item/weapon/am_containment))
		if(fueljar)
			user.text2tab("<span class='warning'>There is already a [fueljar] inside!</span>")
			return
		fueljar = W
		W.loc = src
		if(user.client)
