...
/obj/structure/disposalconstruct/verb/rotate()
	...
	if(anchored)
		usr.text2tab("<span class='warning'>You must unfasten the pipe before rotating it!</span>")
		return
...
/obj/structure/disposalconstruct/verb/flip()
	...
	if(anchored)
		usr.text2tab("<span class='warning'>You must unfasten the pipe before flipping it!</span>")
		return
...
/obj/structure/disposalconstruct/attackby(obj/item/I, mob/user, params)
	...
	if(T.intact && istype(T, /turf/open/floor))
		user.text2tab("<span class='warning'>You can only attach the [nicetype] if the floor plating is removed!</span>")
		return

	if(!ispipe && istype(T, /turf/closed/wall))
		user.text2tab("<span class='warning'>You can't build [nicetype]s on walls, only disposal pipes!</span>")
		return
	...
	if(istype(I, /obj/item/weapon/wrench))
		if(anchored)
			...
			if(ispipe)
				level = 2
			density = 0
			user.text2tab("<span class='notice'>You detach the [nicetype] from the underfloor.</span>")
		else
			if(!is_pipe()) // Disposal or outlet
				if(CP) // There's something there
					if(!istype(CP,/obj/structure/disposalpipe/trunk))
						user.text2tab("<span class='warning'>The [nicetype] requires a trunk underneath it in order to work!</span>")
						return
				...
				if(CP)
					...
					if(istype(CP, /obj/structure/disposalpipe/broken))
						pdir = CP.dir
					if(pdir & dpdir)
						user.text2tab("<span class='warning'>There is already a [nicetype] at that location!</span>")
						return
			anchored = 1
			if(ispipe)
				level = 1 // We don't want disposal bins to disappear under the floors
			density = 0
			user.text2tab("<span class='notice'>You attach the [nicetype] to the underfloor.</span>")
		...
		if(anchored)
			...
			if(W.remove_fuel(0,user))
				...
				user.text2tab("<span class='notice'>You start welding the [nicetype] in place...</span>")
				if(do_after(user, 20/I.toolspeed, target = src))
					if(!loc || !W.isOn())
						return
					user.text2tab("<span class='notice'>The [nicetype] has been welded in place.</span>")
			...
			user.text2tab("<span class='warning'>You need to attach it to the plating first!</span>")
			return
