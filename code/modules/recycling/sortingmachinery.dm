...
/obj/structure/bigDelivery/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/device/destTagger))
		...
		if(sortTag != O.currTag)
			...
			user.text2tab("<span class='notice'>*[tag]*</span>")
			sortTag = O.currTag
...
/obj/structure/bigDelivery/relay_container_resist(mob/living/user, obj/O)
	if(istype(loc, /atom/movable))
		...
		return
	user.text2tab("<span class='notice'>You lean on the back of [O] and start pushing to rip the wrapping around it.</span>")
	if(do_after(user, 50, target = O))
		if(!user || user.stat != CONSCIOUS || user.loc != src || O.loc != src )
			return
		user.text2tab("<span class='notice'>You successfully removed [O]'s wrapping !</span>")
		O.loc = loc
		playsound(src.loc, 'sound/items/poster_ripped.ogg', 50, 1)
		qdel(src)
	else
		if(user.loc == src) //so we don't get the message if we resisted multiple times and succeeded.
			user.text2tab("<span class='warning'>You fail to remove [O]'s wrapping!</span>")
