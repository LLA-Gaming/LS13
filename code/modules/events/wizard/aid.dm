		...
		if(L.mind && L.mind.spell_list.len != 0)
			...
			for(var/obj/effect/proc_holder/spell/S in L.mind.spell_list)
				if(S.clothes_req)
					...
					spell_improved = 1
			if(spell_improved)
				L.text2tab("<span class='notice'>You suddenly feel like you never needed those garish robes in the first place...</span>")
...
/datum/round_event/wizard/improvedcasting/start()
	for(var/mob/living/L in mob_list)
		if(L.mind && L.mind.spell_list.len != 0)
			...
			L.text2tab("<span class='notice'>You suddenly feel more competent with your casting!</span>")
