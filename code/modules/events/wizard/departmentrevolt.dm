...
/datum/round_event/wizard/deprevolt/start()
	...
	switch(tidecolor)
		...
		if("white")
			jobs_to_revolt = list("Medical Director", "Medical Doctor", "Chemist", "Geneticist", "Virologist")
			nation = pick("Mede", "Healtha", "Recova", "Chemi", "Geneti", "Viro", "Psych")
		if("yellow")
			jobs_to_revolt = list("Head Atmospheric Engineer", "Mechanic", "Atmospheric Engineer")
			nation = pick("Atomo", "Engino", "Power", "Teleco")
		if("purple")
			jobs_to_revolt = list("Lead Scientist","Scientist")
			nation = pick("Sci", "Griffa", "Explosi", "Mecha", "Xeno")
		if("brown")
			jobs_to_revolt = list("Quartermaster", "Supply Technician", "Mining Technician")
		...
		if("whatevercolorrepresentstheservicepeople") //the few, the proud, the technically aligned
			jobs_to_revolt = list("Bartender", "Chef", "Agronomist", "Clown", "Mime", "Janitor", "Chaplain")
			...
			if(M.assigned_role && !(M in ticker.mode.traitors))
				for(var/job in jobs_to_revolt)
					if(M.assigned_role == job)
						...
						H.text2tab("<B>You are a separatist! [nation] forever! Protect the soverignty of your newfound land with your comrades in arms!</B>")
	if(citizens.len)
		var/message
		for(var/job in jobs_to_revolt)
			message += "[job],"
