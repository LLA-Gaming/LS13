...
/datum/round_event/wizard/greentext/start()
	...
	H.text2tab("<font color='green'>The mythical greentext appear at your feet! Pick it up if you dare...</font>")
...
/obj/item/weapon/greentext/equipped(mob/living/user as mob)
	user.text2tab("<font color='green'>So long as you leave this place with greentext in hand you know will be happy...</font>")
	if(user.mind && user.mind.objectives.len > 0)
		user.text2tab("<span class='warning'>... so long as you still perform your other objectives that is!</span>")
	new_holder = user
	if(!last_holder)
		last_holder = user
...
/obj/item/weapon/greentext/dropped(mob/living/user as mob)
	if(user in color_altered_mobs)
		user.text2tab("<span class='warning'>A sudden wave of failure washes over you...</span>")
...
/obj/item/weapon/greentext/process()
	if(new_holder && new_holder.z == ZLEVEL_CENTCOM)//you're winner!
		new_holder.text2tab("<font color='green'>At last it feels like victory is assured!</font>")
	...
	if(last_holder && last_holder != new_holder) //Somehow it was swiped without ever getting dropped
		last_holder.text2tab("<span class='warning'>A sudden wave of failure washes over you...</span>")
