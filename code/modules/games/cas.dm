...
/obj/item/toy/cards/singlecard/cas/examine(mob/user)
	if (flipped)
		user.text2tab("<span class='notice'>The card is face down.</span>")
	else if (blank)
		user.text2tab("<span class='notice'>The card is blank. Write on it with a pen.</span>")
	else
		user.text2tab("<span class='notice'>The card reads: [name]</span>")
...
/obj/item/toy/cards/singlecard/cas/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/weapon/pen))
		if(!blank)
			user.text2tab("You cannot write on that card.")
			return
