...
/obj/structure/filingcabinet/attackby(obj/item/P, mob/user, params)
	if(istype(P, /obj/item/weapon/paper) || istype(P, /obj/item/weapon/folder) || istype(P, /obj/item/weapon/photo) || istype(P, /obj/item/documents))
		...
		anchored = !anchored
		user.text2tab("<span class='notice'>You [anchored ? "wrench" : "unwrench"] [src].</span>")
	else if(user.a_intent != "harm")
		user.text2tab("<span class='warning'>You can't put [P] in [src]!</span>")
	else
		return ..()
...
/obj/structure/filingcabinet/attack_self_tk(mob/user)
	if(contents.len)
		if(prob(40 + contents.len * 5))
			...
			if(prob(25))
				step_rand(I)
			user.text2tab("<span class='notice'>You pull \a [I] out of [src] at random.</span>")
			return
	user.text2tab("<span class='notice'>You find nothing in [src].</span>")
...
/obj/structure/filingcabinet/employment/attack_hand(mob/user)
	if(!cooldown)
		...
		user.text2tab("<span class='warning'>The [src] is jammed, give it a few seconds.</span>")
...
/obj/structure/filingcabinet/employment/attackby(obj/item/P, mob/user, params)
	if(istype(P, /obj/item/weapon/wrench))
		user.text2tab("<span class='notice'>You begin to [anchored ? "wrench" : "unwrench"] [src].</span>")
		if (do_after(user,300,user))
			playsound(loc, 'sound/items/Ratchet.ogg', 50, 1)
			anchored = !anchored
			user.text2tab("<span class='notice'>You successfully [anchored ? "wrench" : "unwrench"] [src].</span>")
	else
		return ..()
