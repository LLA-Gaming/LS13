...
/obj/item/weapon/papercutter/attackby(obj/item/P, mob/user, params)
	if(istype(P, /obj/item/weapon/paper) && !storedpaper)
		...
		user.text2tab("<span class='notice'>You place \the [P] in [src].</span>")
		P.loc = src
		storedpaper = P
		update_icon()
		return
	if(istype(P, /obj/item/weapon/hatchet/cutterblade) && !storedcutter)
		if(!user.drop_item())
			return
		user.text2tab("<span class='notice'>You replace [src]'s [P].</span>")
		P.loc = src
		storedcutter = P
		update_icon()
		return
	if(istype(P, /obj/item/weapon/screwdriver) && storedcutter)
		...
		user.text2tab("<span class='notice'>\The [storedcutter] has been [cuttersecured ? "unsecured" : "secured"].</span>")
...
/obj/item/weapon/papercutter/attack_hand(mob/user)
	...
	if(!storedcutter)
		user.text2tab("<span class='notice'>The cutting blade is gone! You can't use \the [src] now.</span>")
		return

	if(!cuttersecured)
		user.text2tab("<span class='notice'>You remove [src]'s [storedcutter].</span>")
	...
	if(storedpaper)
		...
		user.text2tab("<span class='notice'>You neatly cut \the [storedpaper].</span>")
		storedpaper = null
		qdel(storedpaper)
