...
/obj/item/weapon/hand_labeler/afterattack(atom/A, mob/user,proximity)
	...
	if(!labels_left)
		user.text2tab("<span class='warning'>No labels left!</span>")
		return
	if(!label || !length(label))
		user.text2tab("<span class='warning'>No text set!</span>")
		return
	if(length(A.name) + length(label) > 64)
		user.text2tab("<span class='warning'>Label too big!</span>")
		return
	if(ishuman(A))
		user.text2tab("<span class='warning'>You can't label humans!</span>")
		return
	if(issilicon(A))
		user.text2tab("<span class='warning'>You can't label cyborgs!</span>")
		return
...
/obj/item/weapon/hand_labeler/attack_self(mob/user)
	if(!user.IsAdvancedToolUser())
		user.text2tab("<span class='warning'>You don't have the dexterity to use [src]!</span>")
		return
	mode = !mode
	icon_state = "labeler[mode]"
	if(mode)
		user.text2tab("<span class='notice'>You turn on [src].</span>")
		//Now let them chose the text.
		var/str = copytext(reject_bad_text(input(user,"Label text?","Set label","")),1,MAX_NAME_LEN)
		if(!str || !length(str))
			user.text2tab("<span class='warning'>Invalid text!</span>")
			return
		label = str
		user.text2tab("<span class='notice'>You set the text to '[str]'.</span>")
	else
		user.text2tab("<span class='notice'>You turn off [src].</span>")
