...
/obj/machinery/photocopier/Topic(href, href_list)
	...
	if(href_list["copy"])
		if(copy)
			...
			for(var/i = 0, i < copies, i++)
				...
				if(ishuman(ass) && (ass.get_item_by_slot(slot_w_uniform) || ass.get_item_by_slot(slot_wear_suit)))
					usr.text2tab("<span class='notice'>You feel kind of silly, copying [ass == usr ? "your" : ass][ass == usr ? "" : "\'s"] ass with [ass == usr ? "your" : "their"] clothes on.</span>")
					break
		...
		if(copy)
			...
			ass.text2tab("<span class='notice'>You feel a slight pressure on your ass.</span>")
		updateUsrDialog()
...
/obj/machinery/photocopier/proc/do_insertion(obj/item/O, mob/user)
	O.loc = src
	user.text2tab("<span class ='notice'>You insert [O] into [src].</span>")
...
/obj/machinery/photocopier/proc/remove_photocopy(obj/item/O, mob/user)
	if(!issilicon(user)) //surprised this check didn't exist before, putting stuff in AI's hand is bad
		...
		O.loc = src.loc
	user.text2tab("<span class='notice'>You take [O] out of [src].</span>")

/obj/machinery/photocopier/attackby(obj/item/O, mob/user, params)
	if(istype(O, /obj/item/weapon/paper))
		if(copier_empty())
			if(istype(O,/obj/item/weapon/paper/contract/infernal))
				user.text2tab("<span class='warning'>The [src] smokes, smelling of brimstone!</span>")
				burn_state = ON_FIRE
			else
				if(!user.drop_item())
					return
			...
			user.text2tab("<span class='warning'>There is already something in [src]!</span>")
		...
		if(toner <= 0)
			...
			user.text2tab("<span class='warning'>This cartridge is not yet ready for replacement! Use up the rest of the toner.</span>")

	else if(istype(O, /obj/item/weapon/wrench))
		if(isinspace())
			user.text2tab("<span class='warning'>There's nothing to fasten [src] to!</span>")
			return
		playsound(loc, 'sound/items/Ratchet.ogg', 50, 1)
		user.text2tab("<span class='warning'>You start [anchored ? "unwrenching" : "wrenching"] [src]...</span>")
		if(do_after(user, 20/O.toolspeed, target = src))
			if(qdeleted(src))
				return
			user.text2tab("<span class='notice'>You [anchored ? "unwrench" : "wrench"] [src].</span>")
			anchored = !anchored
	else
		return ..()
