...
/obj/item/weapon/pen/fourcolor/attack_self(mob/living/carbon/user)
	switch(colour)
		...
		if("green")
			...
			colour = "black"
	user.text2tab("<span class='notice'>\The [src] will now write in [colour].</span>")
...
/obj/item/weapon/pen/attack(mob/living/M, mob/user,stealth)
	...
	if(!force)
		if(M.can_inject(user, 1))
			user.text2tab("<span class='warning'>You stab [M] with the pen.</span>")
			if(!stealth)
				M.text2tab("<span class='danger'>You feel a tiny prick!</span>")
			. = 1
