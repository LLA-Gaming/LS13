...
/obj/item/weapon/paper/contract/employment/attack(mob/living/M, mob/living/carbon/human/user)
	...
	if(deconvert)
		...
		M.text2tab("<span class='boldnotice'>You feel that your soul has returned to it's rightful owner, Nanotrasen.</span>")
		M.return_soul()
	else
		if(ishuman(M))
			...
			if(!istype(N.head, /obj/item/clothing/head/helmet))
				...
				N.text2tab("<span class='danger'>You feel dumber.</span>")
...
/obj/item/weapon/paper/contract/infernal/attackby(obj/item/weapon/P, mob/living/carbon/human/user, params)
	...
	if(istype(P, /obj/item/weapon/pen) || istype(P, /obj/item/toy/crayon))
		...
		user.text2tab("<span class='notice'>You stamp the paper with your rubber stamp, however the ink ignites as you release the stamp.</span>")
	else if(P.is_hot())
		...
		if(user.mind == target)
			if(user.mind.soulOwner == user.mind)
				if (contractType == CONTRACT_REVIVE)
					user.text2tab("<span class='notice'>You are already alive, this contract would do nothing.</span>")
				else
					user.text2tab("<span class='notice'>You quickly scrawl your name on the contract</span>")
					if(FulfillContract()<=0)
						user.text2tab("<span class='notice'>But it seemed to have no effect, perhaps even Hell itself cannot grant this boon?</span>")
					return 1
			else
				user.text2tab("<span class='notice'>You are not in possession of your soul, you may not sell it.</span>")
		else
			user.text2tab("<span class='notice'>Your signature simply slides off of the sheet, it seems this contract is not meant for you to sign.</span>")
	else
