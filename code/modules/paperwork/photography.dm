...
/obj/item/weapon/photo/examine(mob/user)
	...
	if(in_range(user, src))
		...
		user.text2tab("<span class='warning'>You need to get closer to get a good look at this photo!</span>")
...
/obj/item/device/camera/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/device/camera_film))
		if(pictures_left)
			user.text2tab("<span class='notice'>[src] still has some film in it!</span>")
			return
		if(!user.unEquip(I))
			return
		user.text2tab("<span class='notice'>You insert [I] into [src].</span>")
		qdel(I)
...
/obj/item/device/camera/examine(mob/user)
	..()
	user.text2tab("It has [pictures_left] photos left.")
...
/obj/item/device/camera/proc/injectaialbum(icon, img, desc, pixel_x, pixel_y, blueprintsinject) //stores image information to a list similar to that of the datacore
	...
	aipictures += P
	usr.text2tab("<span class='unconscious'>Image recorded</span>") //feedback to the AI player that the picture was taken)
...
/obj/item/device/camera/proc/injectmasteralbum(icon, img, desc, pixel_x, pixel_y, blueprintsinject) //stores image information to a list similar to that of the datacore
	...
	if(C.connected_ai)
		...
		usr.text2tab("<span class='unconscious'>Image recorded and saved to remote database</span>")	//feedback to the Cyborg player that the picture was taken
	else
...
/obj/item/device/camera/afterattack(atom/target, mob/user, flag)
	...
	pictures_left--
	user.text2tab("<span class='notice'>[pictures_left] photos left.</span>")
...
/obj/item/device/camera/siliconcam/proc/camera_mode_off()
	...
	usr.text2tab("<B>Camera Mode deactivated</B>")

/obj/item/device/camera/siliconcam/proc/camera_mode_on()
	src.in_camera_mode = 1
	usr.text2tab("<B>Camera Mode activated</B>")
...
/obj/item/device/camera/siliconcam/robot_camera/proc/borgprint()
	...
	if(C.toner < 20)
		usr.text2tab("Insufficent toner to print image.")
		return
	if(C.connected_ai)
	...
	if(targetcam.aipictures.len == 0)
		usr.text2tab("<span class='userdanger'>No images saved</span>")
		return
	...
	usr.text2tab("<span class='notice'>You print a photograph.</span>")
