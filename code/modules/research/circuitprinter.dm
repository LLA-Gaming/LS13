...
/obj/machinery/r_n_d/circuit_imprinter/Insert_Item(obj/item/O, mob/user)
	...
	if(istype(O,/obj/item/stack/sheet))
		...
		if(!materials.has_space(sheet_material))
			user.text2tab("<span class='warning'>The [src.name]'s material bin is full! Please remove material before adding more.</span>")
			return 1
		...
		if(!amount_inserted)
			...
			user.text2tab("<span class='notice'>You add [amount_inserted] sheets to the [src.name].</span>")
		updateUsrDialog()

	else if(user.a_intent != "harm")
		user.text2tab("<span class='warning'>You cannot insert this item into the [name]!</span>")
		return 1
	else
		return 0
