...
/obj/machinery/r_n_d/destructive_analyzer/Insert_Item(obj/item/O, mob/user)
	if(user.a_intent != "harm")
		...
		if (temp_tech.len == 0)
			user.text2tab("<span class='warning'>You cannot deconstruct this item!</span>")
			return
		if(!user.drop_item())
			user.text2tab("<span class='warning'>\The [O] is stuck to your hand, you cannot put it in the [src.name]!</span>")
			return
		busy = 1
		loaded_item = O
		O.loc = src
		user.text2tab("<span class='notice'>You add the [O.name] to the [src.name]!</span>")
