...
/obj/item/weapon/reagent_containers/food/drinks/attack(mob/M, mob/user, def_zone)
	...
	if(M == user)
		M.text2tab("<span class='notice'>You swallow a gulp of [src].</span>")
...
/obj/item/weapon/reagent_containers/food/drinks/afterattack(obj/target, mob/user , proximity)
	...
	if(istype(target, /obj/structure/reagent_dispensers)) //A dispenser. Transfer FROM it TO us.
		...
		if(target.reagents.total_volume >= target.reagents.maximum_volume)
			user.text2tab("<span class='warning'>[target] is full.</span>")
			return
		var/refill = reagents.get_master_reagent_id()
		var/trans = src.reagents.trans_to(target, amount_per_transfer_from_this)
		user.text2tab("<span class='notice'>You transfer [trans] units of the solution to [target].</span>")
