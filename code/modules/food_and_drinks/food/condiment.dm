...
/obj/item/weapon/reagent_containers/food/condiment/attack(mob/M, mob/user, def_zone)
	...
	if(!reagents || !reagents.total_volume)
		user.text2tab("<span class='warning'>None of [src] left, oh no!</span>")
		return 0
	...
	if(M == user)
		M.text2tab("<span class='notice'>You swallow some of contents of \the [src].</span>")
	else
...
/obj/item/weapon/reagent_containers/food/condiment/afterattack(obj/target, mob/user , proximity)
	...
	if(istype(target, /obj/structure/reagent_dispensers)) //A dispenser. Transfer FROM it TO us.
		...
		user.text2tab("<span class='notice'>You fill [src] with [trans] units of the contents of [target].</span>")
	...
	//Something like a glass or a food item. Player probably wants to transfer TO it.
		...
		if(target.reagents.total_volume >= target.reagents.maximum_volume)
			user.text2tab("<span class='warning'>you can't add anymore to [target]!</span>")
			return
		var/trans = src.reagents.trans_to(target, amount_per_transfer_from_this)
		user.text2tab("<span class='notice'>You transfer [trans] units of the condiment to [target].</span>")
...
/obj/item/weapon/reagent_containers/food/condiment/pack/afterattack(obj/target, mob/user , proximity)
	...
	if(istype(target, /obj/item/weapon/reagent_containers/food/snacks))
		if(!reagents.total_volume)
			user.text2tab("<span class='warning'>You tear open [src], but there's nothing in it.</span>")
			qdel(src)
			return
		if(target.reagents.total_volume >= target.reagents.maximum_volume)
			user.text2tab("<span class='warning'>You tear open [src], but [target] is stacked so high that it just drips off!</span>") //Not sure if food can ever be full, but better safe than sorry.
			qdel(src)
			return
		else
			user.text2tab("<span class='notice'>You tear open [src] above [target] and the condiments drip onto it.</span>")
