...
/obj/item/weapon/reagent_containers/food/snacks/egg/attackby(obj/item/weapon/W, mob/user, params)
	if(istype( W, /obj/item/toy/crayon ))
		...
		if(!(clr in list("blue", "green", "mime", "orange", "purple", "rainbow", "red", "yellow")))
			usr.text2tab("<span class='notice'>[src] refuses to take on this colour!</span>")
			return

		usr.text2tab("<span class='notice'>You colour [src] [clr].</span>")
		icon_state = "egg-[clr]"
		item_color = clr
	else
		..()
...
/obj/item/weapon/reagent_containers/food/snacks/omelette/attackby(obj/item/weapon/W, mob/user, params)
	if(istype(W,/obj/item/weapon/kitchen/fork))
		...
		if(F.forkload)
			user.text2tab("<span class='warning'>You already have omelette on your fork!</span>")
		else
