...
/obj/machinery/food_cart/attackby(obj/item/O, mob/user, params)
	if(istype(O, /obj/item/weapon/reagent_containers/food/drinks/drinkingglass))
		...
		if(!DG.reagents.total_volume) //glass is empty
			...
			qdel(DG)
			glasses++
			user.text2tab("<span class='notice'>The [src] accepts the drinking glass, sterilizing it.</span>")
	else if(istype(O, /obj/item/weapon/reagent_containers/food/snacks))
		if(isFull())
			user.text2tab("<span class='warning'>The [src] is at full capacity.</span>")
		else
		...
		if(G.get_amount() >= 1)
			G.use(1)
			glasses += 4
			user.text2tab("<span class='notice'>The [src] accepts a sheet of glass.</span>")
...
/obj/machinery/food_cart/Topic(href, href_list)
	...
	if(href_list["pour"] || href_list["m_pour"])
		if(glasses-- <= 0)
			usr.text2tab("<span class='warning'>There are no glasses left!</span>")
			glasses = 0
		else
	...
	if(href_list["mix"])
		if(reagents.trans_id_to(mixer, href_list["mix"], portion) == 0)
			usr.text2tab("<span class='warning'>The [mixer] is full!</span>")

	if(href_list["transfer"])
		if(mixer.reagents.trans_id_to(src, href_list["transfer"], portion) == 0)
			usr.text2tab("<span class='warning'>The [src] is full!</span>")
