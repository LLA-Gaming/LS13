...
/obj/machinery/monkey_recycler/proc/stuff_monkey_in(mob/living/carbon/monkey/target, mob/living/user)
	if(!istype(target))
		return
	if(target.stat == 0)
		user.text2tab("<span class='warning'>The monkey is struggling far too much to put it in the recycler.</span>")
		return
	if(target.buckled || target.has_buckled_mobs())
		user.text2tab("<span class='warning'>The monkey is attached to something.</span>")
		return
	qdel(target)
	user.text2tab("<span class='notice'>You stuff the monkey into the machine.</span>")
	...
	user.text2tab("<span class='notice'>The machine now has [grinded] monkey\s worth of material stored.</span>")
...
/obj/machinery/monkey_recycler/attack_hand(mob/user)
	...
	if(grinded >= required_grind)
		user.text2tab("<span class='notice'>The machine hisses loudly as it condenses the grinded monkey meat. After a moment, it dispenses a brand new monkey cube.</span>")
		playsound(src.loc, 'sound/machines/hiss.ogg', 50, 1)
		grinded -= required_grind
		for(var/i = 0, i < cube_production, i++)
			new /obj/item/weapon/reagent_containers/food/snacks/monkeycube(src.loc)
		user.text2tab("<span class='notice'>The machine's display flashes that it has [grinded] monkeys worth of material left.</span>")
	else
		user.text2tab("<span class='danger'>The machine needs at least [required_grind] monkey(s) worth of material to produce a monkey cube. It only has [grinded].</span>")
	return
