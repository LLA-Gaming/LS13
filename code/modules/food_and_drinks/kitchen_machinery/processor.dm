...
/obj/machinery/processor/attackby(obj/item/O, mob/user, params)
	if(src.processing)
		user.text2tab("<span class='warning'>The processor is in the process of processing!</span>")
		return 1
	...
	if(P)
		...
		if(user.a_intent != "harm")
			user.text2tab("<span class='warning'>That probably won't blend!</span>")
			return 1
		else
			return ..()
...
/obj/machinery/processor/attack_hand(mob/user)
	...
	if(src.contents.len == 0)
		user.text2tab("<span class='warning'>The processor is empty!</span>")
		return 1
	src.processing = 1
