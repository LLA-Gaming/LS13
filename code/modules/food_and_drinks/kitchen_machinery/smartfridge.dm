...
/obj/item/weapon/circuitboard/machine/smartfridge/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/screwdriver))
		...
		user.text2tab("<span class='notice'>You set the board to [fridges[build_path]].</span>")
	else
		return ..()
...
/obj/machinery/smartfridge/attackby(obj/item/O, mob/user, params)
	...
	if(istype(O, /obj/item/weapon/storage/bag))
		...
		if(loaded)
			...
			if(O.contents.len > 0)
				user.text2tab("<span class='warning'>Some items are refused.</span>")
		else
			user.text2tab("<span class='warning'>There is nothing in [O] to put in [src]!</span>")
			return 0

	else if(user.a_intent != "harm")
		user.text2tab("<span class='warning'>\The [src] smartly refuses [O].</span>")
		updateUsrDialog()
		return 0
	else
		return ..()
...
/obj/machinery/smartfridge/proc/load(obj/item/O)
	if(istype(O.loc,/mob))
		var/mob/M = O.loc
		if(!M.unEquip(O))
			usr.text2tab("<span class='warning'>\the [O] is stuck to your hand, you cannot put it in \the [src]!</span>")
			return
