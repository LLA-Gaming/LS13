...
/obj/item/organ/body_egg/zombie_infection/on_find(mob/living/finder)
	finder.text2tab("<span class='warning'>Inside the head is a disgusting black \
	...
	switch(old_stat)
		if(DEAD, UNCONSCIOUS)
			owner.visible_message("<span class='danger'>[owner] staggers to \
				their feet!</span>")
			owner.text2tab("<span class='danger'>You stagger to your feet!</span>")
		...
		if(CONSCIOUS)
			...
			owner.text2tab("<span class='alertalien'>You are now a zombie!</span>")
