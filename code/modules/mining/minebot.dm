	...
	if(istype(I, /obj/item/weapon/weldingtool))
		...
		if(W.welding && !stat)
			if(AIStatus != AI_OFF && AIStatus != AI_IDLE)
				user.text2tab("<span class='info'>[src] is moving around too much to repair!</span>")
				return
			if(maxHealth == health)
				user.text2tab("<span class='info'>[src] is at full integrity.</span>")
			else
				adjustBruteLoss(-10)
				user.text2tab("<span class='info'>You repair some of the armor on [src].</span>")
			return
	if(istype(I, /obj/item/device/mining_scanner) || istype(I, /obj/item/device/t_scanner/adv_mining_scanner))
		user.text2tab("<span class='info'>You instruct [src] to drop any collected ore.</span>")
		DropOre()
		return
	..()
...
/mob/living/simple_animal/hostile/mining_drone/attack_hand(mob/living/carbon/human/M)
	if(M.a_intent == "help")
		switch(search_objects)
			if(0)
				SetCollectBehavior()
				M.text2tab("<span class='info'>[src] has been set to search and store loose ore.</span>")
			if(2)
				SetOffenseBehavior()
				M.text2tab("<span class='info'>[src] has been set to attack hostile wildlife.</span>")
		return
	..()
...
/obj/item/device/mine_bot_ugprade/proc/upgrade_bot(mob/living/simple_animal/hostile/mining_drone/M, mob/user)
	if(M.melee_damage_upper != initial(M.melee_damage_upper))
		user.text2tab("[src] already has a combat upgrade installed!")
		return
...
/obj/item/device/mine_bot_ugprade/health/upgrade_bot(mob/living/simple_animal/hostile/mining_drone/M, mob/user)
	if(M.maxHealth != initial(M.maxHealth))
		user.text2tab("[src] already has a reinforced chassis!")
		return
	M.maxHealth = 170
	qdel(src)
...
/obj/item/device/mine_bot_ugprade/cooldown/upgrade_bot(mob/living/simple_animal/hostile/mining_drone/M, mob/user)
	...
	if(M.ranged_cooldown_time != initial(M.ranged_cooldown_time))
		user.text2tab("[src] already has a decreased weapon cooldown!")
		return
