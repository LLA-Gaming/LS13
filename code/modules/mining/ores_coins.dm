...
/obj/item/weapon/ore/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/weldingtool))
		...
		if(W.remove_fuel(15) && refined_type)
			...
			user.text2tab("<span class='info'>Not enough fuel to smelt [src].</span>")
	..()
...
/obj/item/weapon/ore/glass/attack_self(mob/living/user)
	user.text2tab("<span class='notice'>You use the sand to make sandstone.</span>")
	var/sandAmt = 1
...
/obj/item/weapon/ore/glass/throw_impact(atom/hit_atom)
	...
	C.confused += 5
	C.text2tab("<span class='userdanger'>\The [src] gets into your eyes! The pain, it burns!</span>")
	qdel(src)
...
/obj/item/weapon/ore/plasma/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/weldingtool))
		var/obj/item/weapon/weldingtool/W = I
		if(W.welding)
			user.text2tab("<span class='warning'>You can't hit a high enough temperature to smelt [src] properly!</span>")
	else
		..()
...
/obj/item/weapon/coin/examine(mob/user)
	..()
	if(value)
		user.text2tab("<span class='info'>It's worth [value] credit\s.</span>")
...
/obj/item/weapon/coin/attackby(obj/item/weapon/W, mob/user, params)
	if(istype(W, /obj/item/stack/cable_coil))
		...
		if(string_attached)
			user.text2tab("<span class='warning'>There already is a string attached to this coin!</span>")
			return

		if (CC.use(1))
			add_overlay(image('icons/obj/economy.dmi',"coin_string_overlay"))
			string_attached = 1
			user.text2tab("<span class='notice'>You attach a string to the coin.</span>")
		else
			user.text2tab("<span class='warning'>You need one length of cable to attach a string to the coin!</span>")
			return
		...
		user.text2tab("<span class='notice'>You detach the string from the coin.</span>")
	else ..()

/obj/item/weapon/coin/attack_self(mob/user)
	if(cooldown < world.time - 15)
		if(string_attached) //does the coin have a wire attached
			user.text2tab("<span class='warning'>The coin won't flip very well with something attached!</span>") //Tell user it will not flip
