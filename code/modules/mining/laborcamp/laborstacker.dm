...
/obj/machinery/mineral/labor_claim_console/emag_act(mob/user)
	if(!emagged)
		emagged = 1
		user.text2tab("<span class='warning'>PZZTTPFFFT</span>")
	...
	if(href_list["choice"])
		if(istype(inserted_id)) //Sanity check against href spoofs
			...
			if(href_list["choice"] == "claim")
				...
				machine.points = 0
				usr.text2tab("Points transferred.")
			...
			if(istype(I))
				...
				I.loc = src
				inserted_id = I
			else usr.text2tab("<span class='warning'>Invalid ID.</span>")
		if(check_auth()) //Sanity check against hef spoofs
			if(href_list["choice"] == "station")
				if(!alone_in_area(get_area(src), usr))
					usr.text2tab("<span class='warning'>Prisoners are only allowed to be released while alone.</span>")
				else
					switch(SSshuttle.moveShuttle("laborcamp","laborcamp_home"))
						if(1)
							usr.text2tab("<span class='notice'>Shuttle not found</span>")
						if(2)
							usr.text2tab("<span class='notice'>Shuttle already at station</span>")
						if(3)
							usr.text2tab("<span class='notice'>No permission to dock could be granted.</span>")
						else
			...
			if(href_list["choice"] == "release")
				if(alone_in_area(get_area(loc), usr))
					...
					if(S && S.get_docked())
						...
						usr.text2tab("<span class='warning'>Prisoners can only be released while docked with the station.</span>")
				else
					usr.text2tab("<span class='warning'>Prisoners are only allowed to be released while alone.</span>")
...
/obj/machinery/mineral/labor_points_checker/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/card/id))
		if(istype(I, /obj/item/weapon/card/id/prisoner))
			...
			user.text2tab("<span class='notice'><B>ID: [prisoner_id.registered_name]</B></span>")
			user.text2tab("<span class='notice'>Points Collected:[prisoner_id.points]</span>")
			user.text2tab("<span class='notice'>Point Quota: [prisoner_id.goal]</span>")
			user.text2tab("<span class='notice'>Collect points by bringing smelted minerals to the Labor Shuttle stacking machine. Reach your quota to earn your release.</span>")
		else
			user.text2tab("<span class='warning'>Error: Invalid ID</span>")
	else
		return ..()
