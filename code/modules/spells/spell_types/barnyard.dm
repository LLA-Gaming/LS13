...
/obj/effect/proc_holder/spell/targeted/barnyardcurse/cast(list/targets, mob/user = usr)
	if(!targets.len)
		user.text2tab("<span class='notice'>No target found in range.</span>")
		return

	var/mob/living/carbon/target = targets[1]

	if(!(target.type in compatible_mobs))
		user.text2tab("<span class='notice'>You are unable to curse [target]'s head!</span>")
		return

	if(!(target in oview(range)))
		user.text2tab("<span class='notice'>They are too far away!</span>")
		return
