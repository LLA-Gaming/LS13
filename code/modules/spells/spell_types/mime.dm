...
/obj/effect/proc_holder/spell/aoe_turf/conjure/mime_wall/Click()
	if(usr && usr.mind)
		if(!usr.mind.miming)
			usr.text2tab("<span class='notice'>You must dedicate yourself to silence first.</span>")
			return
...
/obj/effect/proc_holder/spell/targeted/mime/speak/cast(list/targets,mob/user = usr)
	for(var/mob/living/carbon/human/H in targets)
		...
		if(H.mind.miming)
			H.text2tab("<span class='notice'>You make a vow of silence.</span>")
		else
			H.text2tab("<span class='notice'>You break your vow of silence.</span>")
