	...
	for(var/mob/M in targets)
		...
		if(stat_allowed) //Death is not my end!
			if(M.stat == CONSCIOUS && iscarbon(M))
				M.text2tab("<span class='notice'>You aren't dead enough to revive!</span>") //Usually a good problem to have
				charge_counter = charge_max
				return

			if(!marked_item || qdeleted(marked_item)) //Wait nevermind
				M.text2tab("<span class='warning'>Your phylactery is gone!</span>")
				return
			...
			if(user_turf.z != item_turf.z)
				M.text2tab("<span class='warning'>Your phylactery is out of range!</span>")
				return
			...
			lich.text2tab("<span class='warning'>Your bones clatter and shutter as they're pulled back into this world!</span>")
			charge_max += 600
		...
		if(!marked_item) //linking item to the spell
			...
			for(var/obj/item in hand_items)
				if(ABSTRACT in item.flags || NODROP in item.flags)
					continue
				marked_item = item
				M.text2tab("<span class='warning'>You begin to focus your very being into the [item.name]...</span>")
				break

			if(!marked_item)
				M.text2tab("<span class='caution'>You must hold an item you wish to make your phylactery...</span>")
				return
			if(!do_after(M, 50, needhand=FALSE, target=marked_item))
				M.text2tab("<span class='warning'>Your soul snaps back to your body as you stop ensouling [marked_item.name]!</span>")
				marked_item = null
				return
			...
			M.text2tab("<span class='userdanger'>With a hideous feeling of emptiness you watch in horrified fascination as skin sloughs off bone! Blood boils, nerves disintegrate, eyes boil in their sockets! As your organs crumble to dust in your fleshless chest you come to terms with your choice. You're a lich!</span>")
