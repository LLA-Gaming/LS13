			...
			if(M.mob_spell_list.len != 0 || (M.mind && M.mind.spell_list.len != 0))
				...
				if(M.mind)
					for(var/obj/effect/proc_holder/spell/S in M.mind.spell_list)
						S.charge_counter = S.charge_max
				M.text2tab("<span class='notice'>you feel raw magic flowing through you, it feels good!</span>")
			else
				M.text2tab("<span class='notice'>you feel very strange for a moment, but then it passes.</span>")
				burnt_out = 1
			charged_item = M
			break
		for(var/obj/item in hand_items)
			if(istype(item, /obj/item/weapon/spellbook))
				if(istype(item, /obj/item/weapon/spellbook/oneuse))
					...
					L.text2tab("<span class='caution'>Glowing red letters appear on the front cover...</span>")
					L.text2tab("<span class='warning'>[pick("NICE TRY BUT NO!","CLEVER BUT NOT CLEVER ENOUGH!", "SUCH FLAGRANT CHEESING IS WHY WE ACCEPTED YOUR APPLICATION!", "CUTE!", "YOU DIDN'T THINK IT'D BE THAT EASY, DID YOU?")]</span>")
					burnt_out = 1
				...
				for(I in item.contents)
					if(istype(I, /obj/item/weapon/stock_parts/cell/))
						...
						break
		if(!charged_item)
			L.text2tab("<span class='notice'>you feel magical power surging to your hands, but the feeling rapidly fades...</span>")
		else if(burnt_out)
			L.text2tab("<span class='caution'>[charged_item] doesn't seem to be reacting to the spell...</span>")
		else
			playsound(get_turf(L), "sound/magic/Charge.ogg", 50, 1)
			L.text2tab("<span class='notice'>[charged_item] suddenly feels very warm!</span>")
