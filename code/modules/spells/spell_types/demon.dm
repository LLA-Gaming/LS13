...
/obj/effect/proc_holder/spell/targeted/summon_contract/cast(list/targets, mob/user = usr)
	for(var/mob/living/carbon/C in targets)
		if(C.mind && user.mind)
			...
			user.text2tab("<span class='notice'>[C] seems to not be sentient.  You cannot summon a contract for them.</span>")
	...
	if(istype(user))
		if(istype(user.loc, /obj/effect/dummy/slaughter/))
			...
			if(continuing)
				...
				user.text2tab("<span class='warning'>You can only re-appear near a potential signer.")
				revert_cast()
				return ..()
		else
			user.notransform = 1
...
/mob/living/proc/infernalphaseout(var/turf/mobloc)
	if(get_turf(src) != mobloc)
		src.text2tab("<span class='warning'>You must remain still while exiting.")
		return
	dust_animation()
	spawn_dust()
...
/mob/living/proc/infernalphasein()
	if(src.notransform)
		src.text2tab("<span class='warning'>You're too busy to jaunt in.</span>")
		return 0
	fakefire()
