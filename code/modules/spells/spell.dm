...
/obj/effect/proc_holder/spell/proc/cast_check(skipcharge = 0,mob/user = usr) //checks if the spell can be cast based on its settings; skipcharge is used when an additional cast_check is called inside the spell
	...
	if(player_lock)
		if(!user.mind || !(src in user.mind.spell_list) && !(src in user.mob_spell_list))
			user.text2tab("<span class='warning'>You shouldn't have this spell! Something's wrong.</span>")
			return 0
	else
	...
	if(T.z == ZLEVEL_CENTCOM && (!centcom_cancast || ticker.mode.name == "ragin' mages")) //Certain spells are not allowed on the centcom zlevel
		user.text2tab("<span class='notice'>You can't cast this spell here.</span>")
		return 0
		...
		switch(charge_type)
			if("recharge")
				if(charge_counter < charge_max)
					user.text2tab(still_recharging_msg)
					return 0
			if("charges")
				if(!charge_counter)
					user.text2tab("<span class='notice'>[name] has no charges left.</span>")
					return 0

	if(user.stat && !stat_allowed)
		user.text2tab("<span class='notice'>Not when you're incapacitated.</span>")
		return 0

	if(!phase_allowed && istype(user.loc, /obj/effect/dummy))
		user.text2tab("<span class='notice'>[name] cannot be cast unless you are completely manifested in the material plane.</span>")
		return 0
	...
	if(ishuman(user))
		...
		if((invocation_type == "whisper" || invocation_type == "shout") && H.is_muzzled())
			user.text2tab("<span class='notice'>You can't get the words out!</span>")
			return 0
		...
		if(clothes_req) //clothes check
			if(!istype(H.wear_suit, /obj/item/clothing/suit/wizrobe) && !istype(H.wear_suit, /obj/item/clothing/suit/space/hardsuit/wizard))
				H.text2tab("<span class='notice'>I don't feel strong enough without my robe.</span>")
				return 0
			if(!istype(H.shoes, /obj/item/clothing/shoes/sandal))
				H.text2tab("<span class='notice'>I don't feel strong enough without my sandals.</span>")
				return 0
			if(!istype(H.head, /obj/item/clothing/head/wizard) && !istype(H.head, /obj/item/clothing/head/helmet/space/hardsuit/wizard))
				H.text2tab("<span class='notice'>I don't feel strong enough without my hat.</span>")
				return 0
		if(cult_req) //CULT_REQ CLOTHES CHECK
			if(!istype(H.wear_suit, /obj/item/clothing/suit/magusred) && !istype(H.wear_suit, /obj/item/clothing/suit/space/hardsuit/cult))
				H.text2tab("<span class='notice'>I don't feel strong enough without my armor.</span>")
				return 0
			if(!istype(H.head, /obj/item/clothing/head/magus) && !istype(H.head, /obj/item/clothing/head/helmet/space/hardsuit/cult))
				H.text2tab("<span class='notice'>I don't feel strong enough without my helmet.</span>")
				return 0
	else
		if(clothes_req || human_req)
			user.text2tab("<span class='notice'>This spell can only be cast by humans!</span>")
			return 0
		if(nonabstract_req && (isbrain(user) || ispAI(user)))
			user.text2tab("<span class='notice'>This spell can only be cast by physical beings!</span>")
			return 0
