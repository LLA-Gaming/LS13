	...
	switch (action)
		...
		if ("add")
			...
			if (get_stickyban_from_ckey(ckey))
				usr.text2tab("<span class='adminnotice'>Error: Can not add a stickyban: User already has a current sticky ban</span>")
		...
		if ("remove")
			...
			if (!ban)
				usr.text2tab("<span class='adminnotice'>Error: No sticky ban for [ckey] found!</span>")
				return
			if (alert("Are you sure you want to remove the sticky ban on [ckey]?","Are you sure","Yes","No") == "No")
				return
			if (!get_stickyban_from_ckey(ckey))
				usr.text2tab("<span class='adminnotice'>Error: The ban disappeared.</span>")
				return
		...
		if ("remove_alt")
			...
			if (!found)
				usr.text2tab("<span class='adminnotice'>Error: [alt] is not linked to [ckey]'s sticky ban!</span>")
				return
			...
			if (!found)
				usr.text2tab("<span class='adminnotice'>Error: [alt] link to [ckey]'s sticky ban disappeared.</span>")
				return
		...
		if ("edit")
			...
			var/ban = get_stickyban_from_ckey(ckey)
			if (!ban)
				usr.text2tab("<span class='adminnotice'>Error: No sticky ban for [ckey] found!")
				return
