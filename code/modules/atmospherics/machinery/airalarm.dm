...
/obj/machinery/airalarm/ui_status(mob/user)
	if(user.has_unlimited_silicon_privilege && aidisabled)
		user.text2tab("AI control has been disabled.")
	else if(!shorted)
		return ..()
	return UI_CLOSE
	...
	switch(buildstage)
		if(2)
			if(istype(W, /obj/item/weapon/wirecutters) && panel_open && wires.is_all_cut())
				...
				user.text2tab("<span class='notice'>You cut the final wires.</span>")
				...
				if(stat & (NOPOWER|BROKEN))
					user.text2tab("<span class='warning'>It does nothing!</span>")
				else
					if(src.allowed(usr) && !wires.is_cut(WIRE_IDSCAN))
						locked = !locked
						user.text2tab("<span class='notice'>You [ locked ? "lock" : "unlock"] the air alarm interface.</span>")
					else
		...
		if(1)
			if(istype(W, /obj/item/weapon/crowbar))
				...
				if (do_after(user, 20/W.toolspeed, target = src))
					if (buildstage == 1)
						user.text2tab("<span class='notice'>You remove the air alarm electronics.</span>")
			...
			if(istype(W, /obj/item/stack/cable_coil))
				...
				if(cable.get_amount() < 5)
					user.text2tab("<span class='warning'>You need five lengths of cable to wire the fire alarm!</span>")
					return
				...
				if (do_after(user, 20, target = src))
					if (cable.get_amount() >= 5 && buildstage == 1)
						cable.use(5)
						user.text2tab("<span class='notice'>You wire the air alarm.</span>")
						wires.repair()
						aidisabled = 0
						locked = 1
						mode = 1
		...
		if(0)
			...
			if(istype(W, /obj/item/weapon/wrench))
				user.text2tab("<span class='notice'>You detach \the [src] from the wall.</span>")
