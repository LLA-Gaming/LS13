...
/obj/machinery/computer/stockexchange/proc/sell_some_shares(var/datum/stock/S, var/mob/user)
	...
	var/li = logged_in
	if (!li)
		user.text2tab("<span class='danger'>No active account on the console!</span>")
		return
	var/b = SSshuttle.points
	var/avail = S.shareholders[logged_in]
	if (!avail)
		user.text2tab("<span class='danger'>This account does not own any shares of [S.name]!</span>")
		return
	...
	if (!S.sellShares(logged_in, amt))
		user.text2tab("<span class='danger'>Could not complete transaction.</span>")
		return
	user.text2tab("<span class='notice'>Sold [amt] shares of [S.name] at [S.current_value] a share for [total] credits.</span>")
...
/obj/machinery/computer/stockexchange/proc/buy_some_shares(var/datum/stock/S, var/mob/user)
	...
	var/total = amt * S.current_value
	user.text2tab("<span class='notice'>Bought [amt] shares of [S.name] at [S.current_value] a share for [total] credits.</span>")
...
/obj/machinery/computer/stockexchange/proc/do_borrowing_deal(var/datum/borrow/B, var/mob/user)
	if (B.stock.borrow(B, logged_in))
		user.text2tab("<span class='notice'>You successfully borrowed [B.share_amount] shares. Deposit: [B.deposit].</span>")
		stockExchange.add_log(/datum/stock_log/borrow, user.name, B.stock.name, B.share_amount, B.deposit)
	else
		user.text2tab("<span class='danger'>Could not complete transaction. Check your account balance.</span>")
