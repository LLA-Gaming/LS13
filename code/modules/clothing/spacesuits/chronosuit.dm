	...
	switch(severity)
		if(1)
			if(activated && user && ishuman(user) && (user.wear_suit == src))
				user.text2tab("<span class='userdanger'>E:FATAL:RAM_READ_FAIL\nE:FATAL:STACK_EMPTY\nE:FATAL:READ_NULL_POINT\nE:FATAL:PWR_BUS_OVERLOAD</span>")
			deactivate(1, 1)
	...
	if(activated && !teleporting && user && istype(user) && location && user.loc && location.loc && user.wear_suit == src && user.stat == CONSCIOUS)
		...
		for(var/slot in nonsafe_slots)
			...
			if(slot_item && !(slot_item.type in chronosafe_items) && user.unEquip(slot_item))
				user.text2tab("<span class='notice'>Your [slot_item.name] got left behind.</span>")
	...
	if(!activating && !activated && !teleporting)
		...
		if(user && ishuman(user))
			if(user.wear_suit == src)
				...
				user.text2tab("Password:\n")
				user.text2tab("root@ChronosuitMK4# chronowalk4 --start\n")
				if(user.head && istype(user.head, /obj/item/clothing/head/helmet/space/chronos))
					user.text2tab("\[ <span style='color: #00ff00;'>ok</span> \] Mounting /dev/helmet")
					helmet = user.head
					helmet.flags |= NODROP
					helmet.suit = src
					src.flags |= NODROP
					user.text2tab("\[ <span style='color: #00ff00;'>ok</span> \] Starting brainwave scanner")
					user.text2tab("\[ <span style='color: #00ff00;'>ok</span> \] Starting ui display driver")
					user.text2tab("\[ <span style='color: #00ff00;'>ok</span> \] Initializing chronowalk4-view")
					new_camera(user)
					SSobj.processing |= src
					activated = 1
				else
					user.text2tab("\[ <span style='color: #ff0000;'>fail</span> \] Mounting /dev/helmet")
					user.text2tab("<span style='color: #ff0000;'><b>FATAL: </b>Unable to locate /dev/helmet. <b>Aborting...</b>")
	...
	if(activated && (!teleporting || force))
		...
		if(user && ishuman(user))
			if(user.wear_suit == src)
				...
				if(camera)
					if(!silent)
						user.text2tab("\[ <span style='color: #ff5500;'>ok</span> \] Sending TERM signal to chronowalk4-view") //yes I know they aren't a different color when shutting down, but they were too similar at a glance
					qdel(camera)
				if(helmet)
					if(!silent)
						user.text2tab("\[ <span style='color: #ff5500;'>ok</span> \] Stopping ui display driver")
						user.text2tab("\[ <span style='color: #ff5500;'>ok</span> \] Stopping brainwave scanner")
						user.text2tab("\[ <span style='color: #ff5500;'>ok</span> \] Unmounting /dev/helmet")
					helmet.flags &= ~NODROP
					helmet.suit = null
					helmet = null
				if(!silent)
					user.text2tab("logout")
