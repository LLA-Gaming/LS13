...
/obj/item/clothing/mask/proc/adjustmask(mob/living/user)
	...
	if(!mask_adjusted)
		...
		user.text2tab("<span class='notice'>You push \the [src] back into place.</span>")
		slot_flags = initial(slot_flags)
	else
		icon_state += "_up"
		user.text2tab("<span class='notice'>You push \the [src] out of the way.</span>")
...
/obj/item/clothing/shoes/attackby(obj/item/I, mob/user, params)
	...
	if(!can_hold_items)
		return
	if(held_item)
		user.text2tab("<span class='notice'>There's already something in [src].</span>")
		return
	if(is_type_in_list(I, valid_held_items))//can hold both regular pens and energy daggers. made for your every-day tactical librarians/murderers.
		if(I.w_class > 2)//if the object is too big (like if it's a cleaver or an extended edagger) it wont fit
			user.text2tab("<span class='notice'>[I] is currently too big to fit into [src]. </span>")
			return
		if(!user.drop_item())
			return
		I.loc = src
		held_item = I
		user.text2tab("<span class='notice'>You discreetly slip [I] into [src]. Alt-click [src] to remove it.</span>")

/obj/item/clothing/shoes/AltClick(mob/user)
	if(user.incapacitated() || !held_item || !can_hold_items)
		return
	if(!user.put_in_hands(held_item))
		user.text2tab("<span class='notice'>You fumble for [held_item] and it falls on the floor.</span>")
		return 1
		held_item = null
...
/obj/item/clothing/under/proc/attachTie(obj/item/I, mob/user, notifyAttach = 1)
	if(istype(I, /obj/item/clothing/tie))
		if(hastie)
			if(user)
				user.text2tab("<span class='warning'>[src] already has an accessory.</span>")
			return 0
		else
			if(user)
				if(!user.drop_item())
			...
			if(user && notifyAttach)
				user.text2tab("<span class='notice'>You attach [I] to [src].</span>")
...
/obj/item/clothing/under/examine(mob/user)
	...
	switch(src.sensor_mode)
		if(0)
			user.text2tab("Its sensors appear to be disabled.")
		if(1)
			user.text2tab("Its binary life sensors appear to be enabled.")
		if(2)
			user.text2tab("Its vital tracker appears to be enabled.")
		if(3)
			user.text2tab("Its vital tracker and tracking beacon appear to be enabled.")
	if(hastie)
		user.text2tab("\A [hastie] is attached to it.")
...
/obj/item/clothing/under/verb/toggle()
	...
	if (!can_use(M))
		return
	if(src.has_sensor >= 2)
		usr.text2tab("The controls are locked.")
		return 0
	if(src.has_sensor <= 0)
		usr.text2tab("This suit does not have any sensors.")
		return 0

	var/list/modes = list("Off", "Binary vitals", "Exact vitals", "Tracking beacon")
	var/switchMode = input("Select a sensor mode:", "Suit Sensor Mode", modes[sensor_mode + 1]) in modes
	if(get_dist(usr, src) > 1)
		usr.text2tab("<span class='warning'>You have moved too far away!</span>")
		return
	...
	if (src.loc == usr)
		switch(sensor_mode)
			if(0)
				usr.text2tab("<span class='notice'>You disable your suit's remote sensing equipment.</span>")
			if(1)
				usr.text2tab("<span class='notice'>Your suit will now only report whether you are alive or dead.</span>")
			if(2)
				usr.text2tab("<span class='notice'>Your suit will now only report your exact vital lifesigns.</span>")
			if(3)
				usr.text2tab("<span class='notice'>Your suit will now report your exact vital lifesigns as well as your coordinate position.</span>")
...
/obj/item/clothing/under/proc/rolldown()
	if(!can_use(usr))
		return
	if(!can_adjust)
		usr.text2tab("<span class='warning'>You cannot wear this suit any differently!</span>")
		return
	if(toggle_jumpsuit_adjust())
		usr.text2tab("<span class='notice'>You adjust the suit to wear it more casually.</span>")
	else
		usr.text2tab("<span class='notice'>You adjust the suit back to normal.</span>")
...
/obj/item/clothing/under/examine(mob/user)
	..()
	if(src.adjusted)
		user.text2tab("Alt-click on [src] to wear it normally.")
	else
		user.text2tab("Alt-click on [src] to wear it casually.")
...
/obj/item/clothing/proc/weldingvisortoggle()			//Malk: proc to toggle welding visors on helmets, masks, goggles, etc.
	...
	usr.text2tab("<span class='notice'>You adjust \the [src] [up ? "up" : "down"].</span>")
