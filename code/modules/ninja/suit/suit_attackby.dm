		...
		if(istype(I, /obj/item/weapon/reagent_containers/glass))//If it's a glass beaker.
			...
			for(var/reagent_id in reagent_list)
				...
				if(R&&reagents.get_reagent_amount(reagent_id)<r_maxamount+(reagent_id == "radium"?(a_boost*a_transfer):0)&&R.volume>=a_transfer)//Radium is always special.
					...
					U.text2tab("Added [amount_to_transfer] units of [R.name].")//Reports on the specific reagent added.
					I.reagents.update_total()//Now we manually update the total to make sure everything is properly shoved under the rug.

			U.text2tab("Replenished a total of [total_reagent_transfer ? total_reagent_transfer : "zero"] chemical units.")//Let the player know how much total volume was added.
			return

		else if(istype(I, /obj/item/weapon/stock_parts/cell))
			var/obj/item/weapon/stock_parts/cell/CELL = I
			if(CELL.maxcharge > cell.maxcharge && n_gloves && n_gloves.candrain)
				U.text2tab("<span class='notice'>Higher maximum capacity detected.\nUpgrading...</span>")
				if (n_gloves && n_gloves.candrain && do_after(U,s_delay, target = src))
					...
					old_cell.updateicon()
					cell = CELL
					U.text2tab("<span class='notice'>Upgrade complete. Maximum capacity: <b>[round(cell.maxcharge/100)]</b>%</span>")
				else
					U.text2tab("<span class='danger'>Procedure interrupted. Protocol terminated.</span>")
			return

		else if(istype(I, /obj/item/weapon/disk/tech_disk))//If it's a data disk, we want to copy the research on to the suit.
			var/obj/item/weapon/disk/tech_disk/TD = I
			if(TD.stored)//If it has something on it.
				U.text2tab("Research information detected, processing...")
				if(do_after(U,s_delay, target = src))
					for(var/datum/tech/current_data in stored_research)
						if(current_data.id==TD.stored.id)
							...
							break
					TD.stored = null
					U.text2tab("<span class='notice'>Data analyzed and updated. Disk erased.</span>")
				else
					U.text2tab("<span class='userdanger'>ERROR</span>: Procedure interrupted. Process terminated.")
			else
				I.loc = src
				t_disk = I
				U.text2tab("<span class='notice'>You slot \the [I] into \the [src].</span>")
			return
	..()
