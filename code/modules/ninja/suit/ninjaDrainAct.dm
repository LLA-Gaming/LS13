...
/obj/machinery/computer/rdconsole/ninjadrain_act(obj/item/clothing/suit/space/space_ninja/S, mob/living/carbon/human/H, obj/item/clothing/gloves/space_ninja/G)
	...
	H.text2tab("<span class='notice'>Hacking \the [src]...</span>")
	spawn(0)
		var/turf/location = get_turf(H)
		for(var/mob/living/silicon/ai/AI in player_list)
			AI.text2tab("<span class='userdanger'>Network Alert: Hacking attempt detected[location?" in [location]":". Unable to pinpoint location"]</span>.")

	if(files && files.known_tech.len)
		for(var/datum/tech/current_data in S.stored_research)
			H.text2tab("<span class='notice'>Checking \the [current_data.name] database.</span>")
				...
				for(var/datum/tech/analyzing_data in files.known_tech)
					if(current_data.id == analyzing_data.id)
						if(analyzing_data.level > current_data.level)
							H.text2tab("<span class='notice'>Database:</span> <b>UPDATED</b>.")
	...
	H.text2tab("<span class='notice'>Data analyzed. Process finished.</span>")
...
/mob/living/silicon/robot/ninjadrain_act(obj/item/clothing/suit/space/space_ninja/S, mob/living/carbon/human/H, obj/item/clothing/gloves/space_ninja/G)
	...
	src.text2tab("<span class='danger'>Warning: Unauthorized access through sub-route 12, block C, detected.</span>")
