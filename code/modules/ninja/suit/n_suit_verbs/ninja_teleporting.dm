...
/obj/item/clothing/suit/space/space_ninja/proc/ninjajaunt()
	...
	if(!ninjacost(100,N_STEALTH_CANCEL))
		...
		if(destination&&istype(mobloc, /turf))//So we don't teleport out of containers
			...
			H.text2tab("<span class='danger'>The VOID-shift device is malfunctioning, <B>teleportation failed</B>.</span>")
	return
...
/obj/item/clothing/suit/space/space_ninja/proc/ninjashift(turf/T in oview())
	...
	if(!ninjacost(200,N_STEALTH_CANCEL))
		...
		if((!T.density)&&istype(mobloc, /turf))
			...
			H.text2tab("<span class='danger'>You cannot teleport into solid walls or from solid matter</span>")
	return
