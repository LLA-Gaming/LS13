...
/obj/effect/energy_net/Destroy()
	if(affecting)
		...
		if(master)//As long as they still exist.
			master.text2tab("<span class='userdanger'>ERROR</span>: unable to initiate transport protocol. Procedure terminated.")
	return ..()
...
/obj/effect/energy_net/process(mob/living/carbon/M)
	...
	if(isnull(M)||M.loc!=loc)//If mob is gone or not at the location.
		if(!isnull(master))//As long as they still exist.
			master.text2tab("<span class='userdanger'>ERROR</span>: unable to locate \the [mob_name]. Procedure terminated.")
	...
	if(!isnull(src))//As long as both net and person exist.
		...
		M.text2tab("<span class='danger'>You appear in a strange place!</span>")
		...
		if(!isnull(master))//As long as they still exist.
			master.text2tab("<span class='notice'><b>SUCCESS</b>: transport procedure of \the [affecting] complete.</span>")
		M.notransform = 0

	else//And they are free.
		M.text2tab("<span class='notice'>You are free of the net!</span>")
		M.notransform = 0
	return
