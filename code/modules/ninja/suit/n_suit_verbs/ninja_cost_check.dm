...
/obj/item/clothing/suit/space/space_ninja/proc/ninjacost(cost = 0, specificCheck = 0)
	...
	if((H.stat || H.incorporeal_move) && (specificCheck != N_ADRENALINE))//Will not return if user is using an adrenaline booster since you can use them when stat==1.
		H.text2tab("<span class='danger'>You must be conscious and solid to do this.</span>")//It's not a problem of stat==2 since the ninja will explode anyway if they die.
		return 1

	var/actualCost = cost*10
	if(cost && cell.charge < actualCost)
		H.text2tab("<span class='danger'>Not enough energy.</span>")
		return 1
	else
	...
	switch(specificCheck)
		...
		if(N_SMOKE_BOMB)
			if(!s_bombs)
				H.text2tab("<span class='danger'>There are no more smoke bombs remaining.</span>")
				return 1
		if(N_ADRENALINE)
			if(!a_boost)
				H.text2tab("<span class='danger'>You do not have any more adrenaline boosters.</span>")
				return 1
