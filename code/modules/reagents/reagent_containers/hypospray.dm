...
/obj/item/weapon/reagent_containers/hypospray/attack(mob/living/M, mob/user)
	...
	if(reagents.total_volume && (ignore_flags || M.can_inject(user, 1))) // Ignore flag should be checked first or there will be an error message.
		...
		user.text2tab("<span class='notice'>You inject [M] with [src].</span>")
		...
		if(M.reagents)
			...
			user.text2tab("<span class='notice'>[trans] unit\s injected.  [reagents.total_volume] unit\s remaining in [src].</span>")
...
/obj/item/weapon/reagent_containers/hypospray/medipen/examine()
	...
	if(reagents && reagents.reagent_list.len)
		usr.text2tab("<span class='notice'>It is currently loaded.</span>")
	else
		usr.text2tab("<span class='notice'>It is spent.</span>")
