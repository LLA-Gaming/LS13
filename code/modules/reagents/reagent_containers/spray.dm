...
/obj/item/weapon/reagent_containers/spray/afterattack(atom/A as mob|obj, mob/user)
	...
	if(istype(A, /obj/structure/reagent_dispensers) && get_dist(src,A) <= 1) //this block copypasted from reagent_containers/glass, for lack of a better solution
		if(!A.reagents.total_volume && A.reagents)
			user.text2tab("<span class='notice'>\The [A] is empty.</span>")
			return

		if(reagents.total_volume >= reagents.maximum_volume)
			user.text2tab("<span class='notice'>\The [src] is full.</span>")
			return

		var/trans = A.reagents.trans_to(src, 50) //transfer 50u , using the spray's transfer amount would take too long to refill
		user.text2tab("<span class='notice'>You fill \the [src] with [trans] units of the contents of \the [A].</span>")
		return

	if(reagents.total_volume < amount_per_transfer_from_this)
		user.text2tab("<span class='warning'>\The [src] is empty!</span>")
		return
...
/obj/item/weapon/reagent_containers/spray/attack_self(mob/user)
	...
	user.text2tab("<span class='notice'>You switch the nozzle setting to [stream_mode ? "\"stream\"":"\"spray\""]. You'll now use [amount_per_transfer_from_this] units per use.</span>")
...
/obj/item/weapon/reagent_containers/spray/verb/empty()
	...
	if(isturf(usr.loc) && src.loc == usr)
		usr.text2tab("<span class='notice'>You empty \the [src] onto the floor.</span>")
