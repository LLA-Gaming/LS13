	...
	if(istype(M))
		if(user.a_intent == "harm")
			...
			if(M != user)
				...
				user.text2tab("<span class='notice'>You swallow a gulp of [src].</span>")
...
/obj/item/weapon/reagent_containers/glass/afterattack(obj/target, mob/user, proximity)
	if((!proximity) || !check_allowed_items(target,target_self=1)) return
		...
		if(target.reagents && !target.reagents.total_volume)
			user.text2tab("<span class='warning'>[target] is empty and can't be refilled!</span>")
			return
		...
		user.text2tab("<span class='notice'>You fill [src] with [trans] unit\s of the contents of [target].</span>")
	...
	if(hotness)
		...
		if(reagents)
			if(reagents.chem_temp < hotness) //can't be heated to be hotter than the source
				...
				user.text2tab("<span class='notice'>You heat [src] with [I].</span>")
				reagents.handle_reactions()
			else
				user.text2tab("<span class='warning'>[src] is already hotter than [I]!</span>")
	...
	if(istype(I,/obj/item/weapon/reagent_containers/food/snacks/egg)) //breaking eggs
		...
		if(reagents)
			if(reagents.total_volume >= reagents.maximum_volume)
				...
				user.text2tab("<span class='notice'>You break [E] in [src].</span>")
...
/obj/item/weapon/reagent_containers/glass/bucket/attackby(obj/O, mob/user, params)
	if(istype(O, /obj/item/weapon/mop))
		if(reagents.total_volume < 1)
			user.text2tab("<span class='warning'>[src] is out of water!</span>")
		else
			reagents.trans_to(O, 5)
			user.text2tab("<span class='notice'>You wet [O] in [src].</span>")
			playsound(loc, 'sound/effects/slosh.ogg', 25, 1)
	else if(isprox(O))
		user.text2tab("<span class='notice'>You add [O] to [src].</span>")
		qdel(O)
		user.unEquip(src)
		qdel(src)
...
/obj/item/weapon/reagent_containers/glass/bucket/equipped(mob/user, slot)
	...
	if(slot == slot_head && reagents.total_volume)
		user.text2tab("<span class='userdanger'>[src]'s contents spill all over you!</span>")
