...
/datum/reagent/consumable/ethanol/reaction_obj(obj/O, reac_volume)
	if(istype(O,/obj/item/weapon/paper))
		...
		usr.text2tab("<span class='notice'>[paperaffected]'s ink washes away.</span>")
	if(istype(O,/obj/item/weapon/book))
		if(reac_volume >= 5)
			var/obj/item/weapon/book/affectedbook = O
			affectedbook.dat = null
			usr.text2tab("<span class='notice'>Through thorough application, you wash away [affectedbook]'s writing.</span>")
		else
			usr.text2tab("<span class='warning'>The ink smears, but doesn't wash away!</span>")
	return
...
/datum/reagent/consumable/ethanol/screwdrivercocktail/on_mob_life(mob/living/M)
	if(M.mind && M.mind.assigned_role in list("Mechanic", "Atmospheric Engineer", "Head Atmospheric Engineer")) //Engineers lose radiation poisoning at a massive rate.
