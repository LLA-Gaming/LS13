	...
	if(created_volume >= 150)
		...
		for(var/mob/living/simple_animal/revenant/R in get_hearers_in_view(7,get_turf(holder.my_atom)))
			...
			if(!ticker.Bible_deity_name)
				diety = "Christ"
			R.text2tab("<span class='userdanger'>The power of [diety] compels you!</span>")
			R.stun(20)
			R.reveal(100)
		sleep(20)
		for(var/mob/living/carbon/C in get_hearers_in_view(round(created_volume/48,1),get_turf(holder.my_atom)))
			if(iscultist(C) || is_handofgod_cultist(C) || C.dna.species.id == "shadowling" || C.dna.species.id == "l_shadowling")
				C.text2tab("<span class='userdanger'>The divine explosion sears you!</span>")
				C.Weaken(2)
...
/datum/chemical_reaction/sonic_powder/on_reaction(datum/reagents/holder, created_volume)
	...
	for(var/mob/living/carbon/C in get_hearers_in_view(created_volume/10, location))
		...
		if(C.ear_damage >= 15)
			C.text2tab("<span class='warning'>Your ears start to ring badly!</span>")
		else if(C.ear_damage >= 5)
			C.text2tab("<span class='warning'>Your ears start to ring!</span>")
