...
/obj/machinery/reagentgrinder/attackby(obj/item/I, mob/user, params)
		...
		if (istype(I, /obj/item/weapon/reagent_containers) && (I.flags & OPENCONTAINER) )
				if (!beaker)
						...
						user.text2tab("<span class='warning'>There's already a container inside.</span>")
		...
		if(is_type_in_list(I, dried_items))
				if(istype(I, /obj/item/weapon/reagent_containers/food/snacks/grown))
						...
						if(!G.dry)
								user.text2tab("<span class='warning'>You must dry that first!</span>")
								return 1

		if(holdingitems && holdingitems.len >= limit)
				usr.text2tab("The machine cannot hold anymore items.")
				return 1
		...
		if(istype(I, /obj/item/weapon/storage/bag))
				...
				for (var/obj/item/weapon/reagent_containers/food/snacks/grown/G in B.contents)
						...
						if(holdingitems && holdingitems.len >= limit) //Sanity checking so the blender doesn't overfill
								user.text2tab("<span class='notice'>You fill the All-In-One grinder to the brim.</span>")
								break

				if(!I.contents.len)
						user.text2tab("<span class='notice'>You empty the plant bag into the All-In-One grinder.</span>")
		...
		if (!is_type_in_list(I, blend_items) && !is_type_in_list(I, juice_items))
				if(user.a_intent == "harm")
						...
						user.text2tab("<span class='warning'>Cannot refine into a reagent!</span>")
						return 1
