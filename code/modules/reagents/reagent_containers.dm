	...
	if(possible_transfer_amounts.len)
		...
		for(var/A in possible_transfer_amounts)
			...
			if(A == amount_per_transfer_from_this)
				...
				user.text2tab("<span class='notice'>[src]'s transfer amount is now [amount_per_transfer_from_this] units.</span>")
				return
...
/obj/item/weapon/reagent_containers/proc/canconsume(mob/eater, mob/user)
	...
	if(covered)
		...
		user.text2tab("<span class='warning'>You have to remove [who] [covered] first!</span>")
		return 0
	return 1
