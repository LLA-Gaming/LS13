...
/datum/perseus_mission
	...
	targets/
		...
		detain/
			...
			checkCompletion()
				...
				if(A.type in list(/area/security/perseus/mycenaeiii, /area/security/prison, /area/mine/laborcamp))
					return 1

				if(T.type in list(/turf/open/floor/plasteel/shuttle/red))
					return 1

				if(T.onCentcom() || T.onSyndieBase())
					return 0
	...
	exterminate/
		...
		spiders/
			...
			checkCompletion()
				for(var/mob/living/simple_animal/hostile/poison/giant_spider/G in world)
					if(G.z == 1 && (G.stat))
						return 0
