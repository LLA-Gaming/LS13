...
/obj/item/weapon/gun/syringe/examine(mob/user)
	..()
	user.text2tab("Can hold [max_syringes] syringe\s. Has [syringes.len] syringe\s remaining.")
...
/obj/item/weapon/gun/syringe/attack_self(mob/living/user)
	...
	syringes.Remove(S)
	user.text2tab("<span class='notice'>You unload [S] from \the [src].</span>")
...
/obj/item/weapon/gun/syringe/attackby(obj/item/A, mob/user, params, show_msg = 1)
	if(istype(A, /obj/item/weapon/reagent_containers/syringe))
		if(syringes.len < max_syringes)
			if(!user.unEquip(A))
				return
			user.text2tab("<span class='notice'>You load [A] into \the [src].</span>")
			syringes.Add(A)
			A.loc = src
			return 1
		else
			usr.text2tab("<span class='warning'>[src] cannot hold more syringes!</span>")
	return 0
