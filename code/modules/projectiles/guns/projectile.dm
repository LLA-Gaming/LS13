...
/obj/item/weapon/gun/projectile/attackby(obj/item/A, mob/user, params)
	...
	if (istype(A, /obj/item/ammo_box/magazine))
		...
		if (!magazine && istype(AM, mag_type))
			...
			magazine = AM
			magazine.loc = src
			user.text2tab("<span class='notice'>You load a new magazine into \the [src].</span>")
			chamber_round()
			A.update_icon()
			update_icon()
			return 1
		else if (magazine)
			user.text2tab("<span class='notice'>There's already a magazine in \the [src].</span>")
	if(istype(A, /obj/item/weapon/suppressor))
		...
		if(can_suppress)
			if(!suppressed)
				if(!user.unEquip(A))
					return
				user.text2tab("<span class='notice'>You screw [S] onto [src].</span>")
				suppressed = A
				...
				user.text2tab("<span class='warning'>[src] already has a suppressor!</span>")
				return
		else
			user.text2tab("<span class='warning'>You can't seem to figure out how to fit [S] on [src]!</span>")
			return
	return 0
	...
	if(loc == user)
		if(suppressed && can_unsuppress)
			...
			if(user.l_hand != src && user.r_hand != src)
				..()
				return
			user.text2tab("<span class='notice'>You unscrew [suppressed] from [src].</span>")
...
/obj/item/weapon/gun/projectile/attack_self(mob/living/user)
	...
	if(magazine)
		...
		magazine = null
		user.text2tab("<span class='notice'>You pull the magazine out of \the [src].</span>")
	else if(chambered)
		AC.loc = get_turf(src)
		AC.SpinAnimation(10, 1)
		chambered = null
		user.text2tab("<span class='notice'>You unload the round from \the [src]'s chamber.</span>")
	else
		user.text2tab("<span class='notice'>There's no magazine in \the [src].</span>")
	update_icon()
	return
...
/obj/item/weapon/gun/projectile/examine(mob/user)
	..()
	user.text2tab("Has [get_ammo()] round\s remaining.")
...
/obj/item/weapon/gun/projectile/proc/sawoff(mob/user)
	if(sawn_state == SAWN_OFF)
		user.text2tab("<span class='warning'>\The [src] is already shortened!</span>")
		return
