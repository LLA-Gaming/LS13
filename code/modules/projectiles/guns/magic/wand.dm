...
/obj/item/weapon/gun/magic/wand/examine(mob/user)
	..()
	user.text2tab("Has [charges] charge\s remaining.")
	...
	if(target == user)
		if(no_den_usage)
			...
			if(istype(A, /area/wizard_station))
				user.text2tab("<span class='warning'>You know better than to violate the security of The Den, best wait until you leave to use [src].<span>")
				return
			else
				no_den_usage = 0
		zap_self(user)
...
/obj/item/weapon/gun/magic/wand/resurrection/zap_self(mob/living/user)
	...
	if(iscarbon(user))
		...
		C.regenerate_limbs()
	user.text2tab("<span class='notice'>You feel great!</span>")
	charges--
	..()
...
/obj/item/weapon/gun/magic/wand/door/zap_self(mob/living/user)
	user.text2tab("<span class='notice'>You feel vaguely more open with your feelings.</span>")
	charges--
	..()
