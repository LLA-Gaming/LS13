...
/obj/item/weapon/gun/projectile/automatic/attackby(obj/item/A, mob/user, params)
	...
	if(istype(A, /obj/item/ammo_box/magazine))
		...
		if(istype(AM, mag_type))
			if(magazine)
				user.text2tab("<span class='notice'>You perform a tactical reload on \the [src], replacing the magazine.</span>")
				magazine.loc = get_turf(src.loc)
				magazine.update_icon()
				magazine = null
			else
				user.text2tab("<span class='notice'>You insert the magazine into \the [src].</span>")
...
/obj/item/weapon/gun/projectile/automatic/proc/burst_select()
	...
	if(!select)
		burst_size = 1
		fire_delay = 0
		user.text2tab("<span class='notice'>You switch to semi-automatic.</span>")
	else
		burst_size = initial(burst_size)
		fire_delay = initial(fire_delay)
		user.text2tab("<span class='notice'>You switch to [burst_size]-rnd burst.</span>")
...
/obj/item/weapon/gun/projectile/automatic/m90/burst_select()
	...
	switch(select)
		...
		if(1)
			select = 2
			user.text2tab("<span class='notice'>You switch to grenades.</span>")
		if(2)
			select = 0
			burst_size = 1
			fire_delay = 0
			user.text2tab("<span class='notice'>You switch to semi-auto.</span>")
	playsound(user, 'sound/weapons/empty.ogg', 100, 1)
	update_icon()
	return
...
/obj/item/weapon/gun/projectile/automatic/l6_saw/attack_self(mob/user)
	...
	user.text2tab("<span class='notice'>You [cover_open ? "open" : "close"] [src]'s cover.</span>")
	update_icon()
...
/obj/item/weapon/gun/projectile/automatic/l6_saw/afterattack(atom/target as mob|obj|turf, mob/living/user as mob|obj, flag, params) //what I tried to do here is just add a check to see if the cover is open or not and add an icon_state change because I can't figure out how c-20rs do it with overlays
	if(cover_open)
		user.text2tab("<span class='warning'>[src]'s cover is open! Close it before firing!</span>")
	else
		..()
		update_icon()
...
/obj/item/weapon/gun/projectile/automatic/l6_saw/attack_hand(mob/user)
	...
	if(!cover_open || (cover_open && !magazine))
		...
		magazine = null
		update_icon()
		user.text2tab("<span class='notice'>You remove the magazine from [src].</span>")
...
/obj/item/weapon/gun/projectile/automatic/l6_saw/attackby(obj/item/A, mob/user, params)
	...
	if(.)
		return
	if(!cover_open)
		user.text2tab("<span class='warning'>[src]'s cover is closed! You can't insert a new mag.</span>")
		return
	..()
