		...
		if(!armed)
			if(user.get_item_by_slot(slot_back) == src)
				...
				if(!user.put_in_hands(gun))
					armed = 0
					user.text2tab("<span class='warning'>You need a free hand to hold the gun!</span>")
					return
				update_icon()
				gun.forceMove(user)
				user.update_inv_back()
		else
			user.text2tab("<span class='warning'>You are already holding the gun!</span>")
	else
		..()
...
/obj/item/weapon/minigunpack/proc/attach_gun(var/mob/user)
	...
	armed = 0
	if(user)
		user.text2tab("<span class='notice'>You attach the [gun.name] to the [name].</span>")
	else
...
/obj/item/weapon/gun/projectile/minigun/shoot_live_shot(mob/living/user as mob|obj, pointblank = 0, mob/pbtarget = null, message = 1)
	if(ammo_pack)
		if(ammo_pack.overheat < ammo_pack.overheat_max)
			...
			user.text2tab("The gun's heat sensor locked the trigger to prevent lens damage.")

/obj/item/weapon/gun/projectile/minigun/afterattack(atom/target, mob/living/user, flag, params)
	if(!ammo_pack || ammo_pack.loc != user)
		user.text2tab("You need the backpack power source to fire the gun!")
	..()
