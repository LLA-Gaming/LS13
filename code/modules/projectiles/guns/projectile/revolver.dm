...
/obj/item/weapon/gun/projectile/revolver/attack_self(mob/living/user)
	...
	while (get_ammo() > 0)
		...
		if(CB)
			...
			num_unloaded++
	if (num_unloaded)
		user.text2tab("<span class='notice'>You unload [num_unloaded] shell\s from [src].</span>")
	else
...
/obj/item/weapon/gun/projectile/revolver/examine(mob/user)
	..()
	user.text2tab("[get_ammo(0,0)] of those are live rounds.")
...
/obj/item/weapon/gun/projectile/revolver/detective/process_fire(atom/target as mob|obj|turf, mob/living/user as mob|obj, message = 1, params, zone_override = "")
	if(magazine.caliber != initial(magazine.caliber))
		if(prob(70 - (magazine.ammo_count() * 10)))	//minimum probability of 10, maximum of 60
			...
			user.text2tab("<span class='userdanger'>[src] blows up in your face!</span>")
...
/obj/item/weapon/gun/projectile/revolver/detective/attackby(obj/item/A, mob/user, params)
	...
	if(istype(A, /obj/item/weapon/screwdriver))
		if(magazine.caliber == "38")
			user.text2tab("<span class='notice'>You begin to reinforce the barrel of [src]...</span>")
			...
			if(do_after(user, 30/A.toolspeed, target = src))
				if(magazine.ammo_count())
					user.text2tab("<span class='warning'>You can't modify it!</span>")
					return
				magazine.caliber = "357"
				desc = "The barrel and chamber assembly seems to have been modified."
				user.text2tab("<span class='notice'>You reinforce the barrel of [src]. Now it will fire .357 rounds.</span>")
		else
			user.text2tab("<span class='notice'>You begin to revert the modifications to [src]...</span>")
			if(magazine.ammo_count())
			...
			if(do_after(user, 30/A.toolspeed, target = src))
				...
				magazine.caliber = "38"
				desc = initial(desc)
				user.text2tab("<span class='notice'>You remove the modifications on [src]. Now it will fire .38 rounds.</span>")
...
/obj/item/weapon/gun/projectile/revolver/russian/afterattack(atom/target as mob|obj|turf, mob/living/user as mob|obj, flag, params)
	...
	if(target != user)
		if(ismob(target))
			user.text2tab("<span class='warning'>A mechanism prevents you from shooting anyone but yourself!</span>")
		return

	if(ishuman(user))
		var/mob/living/carbon/human/H = user
		if(!spun)
			user.text2tab("<span class='warning'>You need to spin the revolver's chamber first!</span>")
			return
...
/obj/item/weapon/gun/projectile/revolver/doublebarrel/attack_self(mob/living/user)
	...
	while (get_ammo() > 0)
		...
		num_unloaded++
	if (num_unloaded)
		user.text2tab("<span class='notice'>You break open \the [src] and unload [num_unloaded] shell\s.</span>")
	else
		user.text2tab("<span class='warning'>[src] is empty!</span>")
...
/obj/item/weapon/gun/projectile/revolver/doublebarrel/improvised/attackby(obj/item/A, mob/user, params)
	...
	if(istype(A, /obj/item/stack/cable_coil) && !sawn_state)
		...
		if(C.use(10))
			slot_flags = SLOT_BACK
			user.text2tab("<span class='notice'>You tie the lengths of cable to the shotgun, making a sling.</span>")
			slung = 1
			update_icon()
		else
			user.text2tab("<span class='warning'>You need at least ten lengths of cable if you want to make a sling!</span>")
