...
/obj/machinery/computer/shuttle/syndicate/Topic(href, href_list)
	if(href_list["move"])
		...
		if(board.challenge && world.time < SYNDICATE_CHALLENGE_TIMER)
			usr.text2tab("<span class='warning'>You've issued a combat challenge to the station! You've got to give them at least [round(((SYNDICATE_CHALLENGE_TIMER - world.time) / 10) / 60)] more minutes to allow them to prepare.</span>")
			return 0
		board.moved = TRUE
	..()
...
/obj/machinery/computer/shuttle/syndicate/drop_pod/Topic(href, href_list)
	if(href_list["move"])
		if(z != ZLEVEL_CENTCOM)
			usr.text2tab("<span class='warning'>Pods are one way!</span>")
			return 0
	..()
