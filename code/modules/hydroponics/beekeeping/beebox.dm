...
/obj/structure/beebox/examine(mob/user)
	...
	if(!queen_bee)
		user.text2tab("<span class='warning'>There is no queen bee! There won't bee any honeycomb without a queen!</span>")

	var/half_bee = get_max_bees()*0.5
	if(half_bee && (bees.len >= half_bee))
		user.text2tab("<span class='notice'>This place is a BUZZ with activity... there are lots of bees!</span>")

	user.text2tab("<span class='notice'>[bee_resources]/100 resource supply.</span>")
	user.text2tab("<span class='notice'>[bee_resources]% towards a new honeycomb.</span>")
	user.text2tab("<span class='notice'>[bee_resources*2]% towards a new bee.</span>")

	if(honeycombs.len)
		var/plural = honeycombs.len > 1
		user.text2tab("<span class='notice'>There [plural? "are" : "is"] [honeycombs.len] uncollected honeycomb[plural ? "s":""] in the apiary.</span>")

	if(honeycombs.len >= get_max_honeycomb())
		user.text2tab("<span class='warning'>there's no room for more honeycomb!</span>")
...
/obj/structure/beebox/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/honey_frame))
		...
		if(honey_frames.len < BEEBOX_MAX_FRAMES)
			...
			user.text2tab("<span class='warning'>There's no room for anymore frames in the apiary!</span>")
	...
	if(istype(I, /obj/item/queen_bee))
		if(queen_bee)
			user.text2tab("<span class='warning'>This hive already has a queen!</span>")
			return
		...
		if(queen_bee)
			...
			for(var/b in bees)
				...
				if(B.reagent_incompatible(queen_bee))
					...
					relocated++
			if(relocated)
				user.text2tab("<span class='warning'>This queen has a different reagent to some of the bees who live here, those bees will not return to this apiary!</span>")

		else
			user.text2tab("<span class='warning'>The queen bee disappeared! bees disappearing has been in the news lately...</span>")
	...
	if(!user.bee_friendly())
		...
		switch(option)
			if("Remove a Honey Frame")
				if(!honey_frames.len)
					user.text2tab("<span class='warning'>There are no honey frames to remove!</span>")
					return
			...
			if("Remove the Queen Bee")
				if(!queen_bee || queen_bee.loc != src)
					user.text2tab("<span class='warning'>There is no queen bee to remove!</span>")
					return
