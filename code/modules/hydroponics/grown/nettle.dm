...
/obj/item/weapon/grown/nettle/pickup(mob/living/user)
	...
	C.text2tab("<span class='userdanger'>The nettle burns your bare hand!</span>")
	return 1
...
/obj/item/weapon/grown/nettle/afterattack(atom/A as mob|obj, mob/user,proximity)
	...
	if(force > 0)
		...
		usr.text2tab("All the leaves have fallen off the nettle from violent whacking.")
		usr.unEquip(src)
		qdel(src)
...
/obj/item/weapon/grown/nettle/death/pickup(mob/living/carbon/user)
	...
	if(..())
		if(prob(50))
			user.Paralyse(5)
			user.text2tab("<span class='userdanger'>You are stunned by the Deathnettle when you try picking it up!</span>")

/obj/item/weapon/grown/nettle/death/attack(mob/living/carbon/M, mob/user)
	if(!..()) return
	if(istype(M, /mob/living))
		M.text2tab("<span class='danger'>You are stunned by the powerful acid of the Deathnettle!</span>")
