...
/obj/machinery/biogenerator/attackby(obj/item/O, mob/user, params)
	if(processing)
		user.text2tab("<span class='warning'>The biogenerator is currently processing.</span>")
		return
	...
	if(istype(O, /obj/item/weapon/reagent_containers/glass))
		...
		if(!panel_open)
			if(beaker)
				user.text2tab("<span class='warning'>A container is already loaded into the machine.</span>")
			else
				if(!user.drop_item())
					return
				O.loc = src
				beaker = O
				user.text2tab("<span class='notice'>You add the container to the machine.</span>")
				update_icon()
				updateUsrDialog()
		else
			user.text2tab("<span class='warning'>Close the maintenance panel first.</span>")
		return
		...
		for(var/obj/item/weapon/reagent_containers/food/snacks/grown/G in contents)
			i++
		if(i >= max_items)
			user.text2tab("<span class='warning'>The biogenerator is already full! Activate it.</span>")
		else
			for(var/obj/item/weapon/reagent_containers/food/snacks/grown/G in PB.contents)
				...
				i++
			if(i<max_items)
				user.text2tab("<span class='info'>You empty the plant bag into the biogenerator.</span>")
			else if(PB.contents.len == 0)
				user.text2tab("<span class='info'>You empty the plant bag into the biogenerator, filling it to its capacity.</span>")
			else
				user.text2tab("<span class='info'>You fill the biogenerator to its capacity.</span>")
		...
		for(var/obj/item/weapon/reagent_containers/food/snacks/grown/G in contents)
			i++
		if(i >= max_items)
			user.text2tab("<span class='warning'>The biogenerator is full! Activate it.</span>")
		else
			user.unEquip(O)
			O.loc = src
			user.text2tab("<span class='info'>You put [O.name] in [src.name]</span>")
		return 1 //no afterattack

	else if(user.a_intent != "harm")
		user.text2tab("<span class='warning'>You cannot put this in [src.name]!</span>")
	else
		return ..()
...
/obj/machinery/biogenerator/proc/activate()
	...
	if (src.stat != 0) //NOPOWER etc
		return
	if(src.processing)
		usr.text2tab("<span class='warning'>The biogenerator is in the process of working.</span>")
		return
	var/S = 0
