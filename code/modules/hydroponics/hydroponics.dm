...
/obj/machinery/hydroponics/constructable/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/weapon/crowbar))
		if(using_irrigation)
			user.text2tab("<span class='warning'>Disconnect the hoses first!</span>")
...
/obj/machinery/hydroponics/examine(mob/user)
	...
	if(myseed)
		...
		if (dead)
			...
			user.text2tab("<span class='info'>It's ready to harvest.</span>")
	...
	if(!self_sustaining)
		...
		user.text2tab("<span class='info'>It doesn't require any water or nutrients.</span>")
...
/obj/machinery/hydroponics/proc/mutateweed() // If the weeds gets the mutagent instead. Mind you, this pretty much destroys the old plant
	if( weedlevel > 5 )
		...
		usr.text2tab("<span class='warning'>The few weeds in [src] seem to react, but only for a moment...</span>")
...
/obj/machinery/hydroponics/proc/mutatepest()
	if(pestlevel > 5)
		...
		usr.text2tab("<span class='warning'>The pests seem to behave oddly, but quickly settle down...</span>")
	...
	if(S.has_reagent("mutagen", 5) || S.has_reagent("radium", 10) || S.has_reagent("uranium", 10))
		switch(rand(100))
			...
			if(41 to 65)
				mutate()
			if(21 to 41)
				usr.text2tab("<span class='warning'>The plants don't seem to react...</span>")
			if(11 to 20)
				mutateweed()
			if(1 to 10)
				mutatepest()
			else
				usr.text2tab("<span class='warning'>Nothing happens...</span>")
