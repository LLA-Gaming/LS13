	...
	switch(action)
		if("cut")
			if(istype(I, /obj/item/weapon/wirecutters) || IsAdminGhost(usr))
				...
				L.text2tab("<span class='warning'>You need wirecutters!</span>")
		if("pulse")
			if(istype(I, /obj/item/device/multitool) || IsAdminGhost(usr))
				...
				L.text2tab("<span class='warning'>You need a multitool!</span>")
		if("attach")
			if(is_attached(target_wire))
				...
				if(istype(I, /obj/item/device/assembly))
					...
					if(A.attachable)
						...
						L.text2tab("<span class='warning'>You need an attachable assembly!</span>")
