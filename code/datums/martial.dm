...
/datum/martial_art/boxing/disarm_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	A.text2tab("<span class='warning'>Can't disarm while boxing!</span>")
	return 1

/datum/martial_art/boxing/grab_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	A.text2tab("<span class='warning'>Can't grab while boxing!</span>")
	return 1
...
/mob/living/carbon/human/proc/wrestling_help()
	...
	usr.text2tab("<b><i>You flex your muscles and have a revelation...</i></b>")
	usr.text2tab("<span class='notice'>Clinch</span>: Grab. Passively gives you a chance to immediately aggressively grab someone. Not always successful.")
	usr.text2tab("<span class='notice'>Suplex</span>: Disarm someone you are grabbing. Suplexes your target to the floor. Greatly injures them and leaves both you and your target on the floor.")
	usr.text2tab("<span class='notice'>Advanced grab</span>: Grab. Passively causes stamina damage when grabbing someone.")
...
/mob/living/carbon/human/proc/plasma_fist_help()
	...
	usr.text2tab("<b><i>You clench your fists and have a flashback of knowledge...</i></b>")
	usr.text2tab("<span class='notice'>Tornado Sweep</span>: Harm Harm Disarm. Repulses target and everyone back.")
	usr.text2tab("<span class='notice'>Throwback</span>: Disarm Harm Disarm. Throws the target and an item at them.")
	usr.text2tab("<span class='notice'>The Plasma Fist</span>: Harm Disarm Disarm Disarm Harm. Knocks the brain out of the opponent and gibs their body.")
...
/mob/living/carbon/human/proc/sleeping_carp_help()
	...
	usr.text2tab("<b><i>You retreat inward and recall the teachings of the Sleeping Carp...</i></b>")

	usr.text2tab("<span class='notice'>Wrist Wrench</span>: Disarm Disarm. Forces opponent to drop item in hand.")
	usr.text2tab("<span class='notice'>Back Kick</span>: Harm Grab. Opponent must be facing away. Knocks down.")
	usr.text2tab("<span class='notice'>Stomach Knee</span>: Grab Harm. Knocks the wind out of opponent and stuns.")
	usr.text2tab("<span class='notice'>Head Kick</span>: Disarm Harm Harm. Decent damage, forces opponent to drop item in hand.")
	usr.text2tab("<span class='notice'>Elbow Drop</span>: Harm Disarm Harm Disarm Harm. Opponent must be on the ground. Deals huge damage, instantly kills anyone in critical condition.")
...
/obj/item/weapon/plasma_fist_scroll/attack_self(mob/user)
	...
	if(!used)
		...
		F.teach(H)
		H.text2tab("<span class='boldannounce'>You have learned the ancient martial art of Plasma Fist.</span>")
		used = 1
...
/obj/item/weapon/sleeping_carp_scroll/attack_self(mob/living/carbon/human/user)
	if(!istype(user) || !user)
		return
	user.text2tab("<span class='sciradio'>You have learned the ancient martial art of the Sleeping Carp! Your hand-to-hand combat has become much more effective, and you are now able to deflect any projectiles \
...
/obj/item/weapon/twohanded/bostaff/attack(mob/target, mob/living/user)
	...
	if((CLUMSY in user.disabilities) && prob(50))
		user.text2tab("<span class ='warning'>You club yourself over the head with [src].</span>")
		user.Weaken(3)
		if(ishuman(user))
			var/mob/living/carbon/human/H = user
	...
	if(C.stat)
		user.text2tab("<span class='warning'>It would be dishonorable to attack a foe while they cannot retaliate.</span>")
		return
