...
/datum/action/slam/Trigger()
	if(owner.incapacitated())
		owner.text2tab("<span class='warning'>You can't WRESTLE while you're OUT FOR THE COUNT.</span>")
		return
...
/datum/martial_art/wrestling/teach(var/mob/living/carbon/human/H,var/make_temporary=0)
	..()
	H.text2tab("<span class = 'userdanger'>SNAP INTO A THIN TIM!</span>")
	H.text2tab("<span class = 'danger'>Place your cursor over a move at the top of the screen to see what it does.</span>")
	drop.Grant(H)
	kick.Grant(H)
	slam.Grant(H)
...
/datum/martial_art/wrestling/remove(var/mob/living/carbon/human/H)
	..()
	H.text2tab("<span class = 'userdanger'>You no longer feel that the tower of power is too sweet to be sour...</span>")
	drop.Remove(H)
	kick.Remove(H)
	slam.Remove(H)
...
/datum/martial_art/wrestling/proc/throw_wrassle(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	if(!A.pulling || A.pulling != D)
		A.text2tab("You need to have [D] in a cinch!")
		return
	D.forceMove(A.loc)
	...
	for (var/i = 0, i < 20, i++)
		...
		if (A && D)
			...
			if (get_dist(A, D) > 1)
				A.text2tab("[D] is too far away!")
				return 0

			if (!isturf(A.loc) || !isturf(D.loc))
				A.text2tab("You can't throw [D] from here!")
				return 0
...
/datum/martial_art/wrestling/proc/slam(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	for (var/i = 0, i < 3, i++)
		if (A && D)
			...
			if (!isturf(A.loc) || !isturf(D.loc))
				A.text2tab("You can't slam [D] here!")
				A.pixel_x = 0
				A.pixel_y = 0
				D.pixel_x = 0
				D.pixel_y = 0
...
/datum/martial_art/wrestling/proc/drop(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	if (A && D)
		...
		if (!isturf(A.loc) || !isturf(D.loc))
			A.pixel_y = 0
			A.text2tab("You can't drop onto [D] from here!")
			return 0
