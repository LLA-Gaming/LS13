...
/datum/disease/beesease/stage_act()
	...
	switch(stage)
		if(2) //also changes say, see say.dm
			if(prob(2))
				affected_mob.text2tab("<span class='notice'>You taste honey in your mouth.</span>")
		if(3)
			if(prob(10))
				affected_mob.text2tab("<span class='notice'>Your stomach rumbles.</span>")
			if(prob(2))
				affected_mob.text2tab("<span class='danger'>Your stomach stings painfully.</span>")
				if(prob(20))
		...
		if(4)
			...
			if(prob(5))
				affected_mob.text2tab("<span class='danger'>You feel something moving in your throat.</span>")
			if(prob(1))
