	...
	switch(stage)
		if(1)
			if (prob(stage_prob) && stage1)
				affected_mob.text2tab(pick(stage1))
		if(2)
			if (prob(stage_prob) && stage2)
				affected_mob.text2tab(pick(stage2))
		if(3)
			if (prob(stage_prob*2) && stage3)
				affected_mob.text2tab(pick(stage3))
		if(4)
			if (prob(stage_prob*2) && stage4)
				affected_mob.text2tab(pick(stage4))
		if(5)
...
/datum/disease/transformation/proc/do_disease_transformation(mob/living/affected_mob)
	if(istype(affected_mob, /mob/living/carbon) && affected_mob.stat != DEAD)
		if(stage5)
			affected_mob.text2tab(pick(stage5))
...
/datum/disease/transformation/jungle_fever/stage_act()
	...
	switch(stage)
		if(2)
			if(prob(2))
				affected_mob.text2tab("<span class='notice'>Your [pick("back", "arm", "leg", "elbow", "head")] itches.</span>")
		if(3)
			if(prob(4))
