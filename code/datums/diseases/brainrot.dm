...
/datum/disease/brainrot/stage_act() //Removed toxloss because damaging diseases are pretty horrible. Last round it killed the entire station because the cure didn't work -- Urist -ACTUALLY Removed rather than commented out, I don't see it returning - RR
	...
	switch(stage)
		if(2)
			...
			if(prob(2))
				affected_mob.text2tab("<span class='danger'>You don't feel like yourself.</span>")
			if(prob(5))
		...
		if(3)
			...
			if(prob(10) && affected_mob.getBrainLoss()<=98)//shouldn't retard you to death now
				...
				affected_mob.updatehealth()
				if(prob(2))
					affected_mob.text2tab("<span class='danger'>Your try to remember something important...but can't.</span>")
		...
		if(4)
			...
			if(prob(15) && affected_mob.getBrainLoss()<=98) //shouldn't retard you to death now
				...
				affected_mob.updatehealth()
				if(prob(2))
					affected_mob.text2tab("<span class='danger'>Strange buzzing fills your head, removing all thoughts.</span>")
			if(prob(3))
				affected_mob.text2tab("<span class='danger'>You lose consciousness...</span>")
