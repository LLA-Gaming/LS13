...
/datum/disease/dnaspread/stage_act()
	...
	switch(stage)
		if(2 || 3) //Pretend to be a cold and give time to spread.
			...
			if(prob(1))
				affected_mob.text2tab("<span class='danger'>Your muscles ache.</span>")
				if(prob(20))
					affected_mob.take_organ_damage(1)
			if(prob(1))
				affected_mob.text2tab("<span class='danger'>Your stomach hurts.</span>")
				if(prob(20))
...
/datum/disease/dnaspread/Destroy()
	if (original_dna && transformed && affected_mob)
		...
		affected_mob.text2tab("<span class='notice'>You feel more like yourself.</span>")
	return ..()
