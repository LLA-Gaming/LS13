...
/datum/symptom/weight_gain/Activate(datum/disease/advance/A)
	...
	if(prob(SYMPTOM_ACTIVATION_PROB))
		...
		switch(A.stage)
			if(1, 2, 3, 4)
				M.text2tab("<span class='warning'>[pick("You feel blubbery.", "Your stomach hurts.")]</span>")
			else
...
/datum/symptom/weight_loss/Activate(datum/disease/advance/A)
	...
	if(prob(SYMPTOM_ACTIVATION_PROB))
		...
		switch(A.stage)
			if(1, 2, 3, 4)
				M.text2tab("<span class='warning'>[pick("You feel hungry.", "You crave for food.")]</span>")
			else
				M.text2tab("<span class='warning'><i>[pick("So hungry...", "You'd kill someone for a bite of food...", "Hunger cramps seize you...")]</i></span>")
