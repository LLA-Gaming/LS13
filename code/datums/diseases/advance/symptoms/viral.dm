...
/datum/symptom/viraladaptation/Activate(datum/disease/advance/A)
	...
	if(prob(SYMPTOM_ACTIVATION_PROB))
		...
		switch(A.stage)
			if(1)
				M.text2tab("<span class='notice'>You feel off, but no different from before.</span>")
			if(5)
				M.text2tab("<span class='notice'>You feel better, but nothing interesting happens.</span>")
...
/datum/symptom/viraladaptation/Activate(datum/disease/advance/A)
	...
	if(prob(SYMPTOM_ACTIVATION_PROB))
		...
		switch(A.stage)
			if(1)
				M.text2tab("<span class='notice'>You feel better, but no different from before.</span>")
			if(5)
				M.text2tab("<span class='notice'>You feel off, but nothing interesting happens.</span>")
