...
/turf/closed/wall/attack_hulk(mob/user)
	...
	if(prob(hardness))
		playsound(src, 'sound/effects/meteorimpact.ogg', 100, 1)
		user.text2tab(text("<span class='notice'>You smash through the wall.</span>"))
		user.say(pick(";RAAAAAAAARGH!", ";HNNNNNNNNNGGGGGGH!", ";GWAAAAAAAARRRHHH!", "NNNNNNNNGGGGGGGGHH!", ";AAAAAAARRRGH!" ))
		dismantle_wall(1)

	else
		playsound(src, 'sound/effects/bang.ogg', 50, 1)
		user.text2tab(text("<span class='notice'>You punch the wall.</span>"))
	return 1

/turf/closed/wall/attack_hand(mob/user)
	user.changeNext_move(CLICK_CD_MELEE)
	user.text2tab("<span class='notice'>You push the wall but nothing happens!</span>")
...
/turf/closed/wall/proc/try_decon(obj/item/weapon/W, mob/user, turf/T)
	if( istype(W, /obj/item/weapon/weldingtool) )
		...
		if( WT.remove_fuel(0,user) )
			user.text2tab("<span class='notice'>You begin slicing through the outer plating...</span>")
			playsound(src, 'sound/items/Welder.ogg', 100, 1)
			if(do_after(user, slicing_duration/W.toolspeed, target = src))
				if( !istype(src, /turf/closed/wall) || !user || !WT || !WT.isOn() || !T )
					return 1
				if( user.loc == T && user.get_active_hand() == WT )
					user.text2tab("<span class='notice'>You remove the outer plating.</span>")
					dismantle_wall()
					return 1
