...
/obj/mecha/Topic(href, href_list)
	...
	if(in_range(src, usr))
		...
		if(href_list["maint_access"] && maint_access)
			...
			if(user)
				if(state==0)
					state = 1
					user.text2tab("The securing bolts are now exposed.")
				else if(state==1)
					state = 0
					user.text2tab("The securing bolts are now hidden.")
		...
		if(href_list["set_internal_tank_valve"] && state >=1)
			...
			if(user)
				...
				if(new_pressure)
					...
					user.text2tab("The internal pressure valve has been set to [internal_tank_valve]kPa.")
	...
	if(href_list["dna_lock"])
		if(occupant && !iscarbon(occupant))
			occupant.text2tab("<span class='danger'> You do not have any DNA!</span>")
			return
