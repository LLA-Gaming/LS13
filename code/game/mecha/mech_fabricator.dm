...
/obj/machinery/mecha_part_fabricator/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/stack/sheet))
		if(panel_open)
			user.text2tab("<span class='warning'>You can't load [src] while it's opened!</span>")
			return 1
		if(being_built)
			user.text2tab("<span class='warning'>\The [src] is currently processing! Please wait until completion.</span>")
			return 1

		var/material_amount = materials.get_item_material_amount(W)
		if(!material_amount)
			user.text2tab("<span class='warning'>This object does not contain sufficient amounts of materials to be accepted by [src].</span>")
			return 1
		if(!materials.has_space(material_amount))
			user.text2tab("<span class='warning'>\The [src] is full. Please remove some materials from [src] in order to insert more.</span>")
			return 1
		if(!user.unEquip(W))
			user.text2tab("<span class='warning'>\The [W] is stuck to you and cannot be placed into [src].</span>")
			return 1
