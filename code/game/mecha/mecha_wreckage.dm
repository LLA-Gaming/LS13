...
/obj/structure/mecha_wreckage/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/weldingtool))
		if(salvage_num <= 0)
			user.text2tab("<span class='warning'>You don't see anything that can be cut with [I]!</span>")
			return
		...
		if(welder_salvage && welder_salvage.len && WT.remove_fuel(0, user))
			...
			if(type)
				...
				user.text2tab("<span class='warning'>You fail to salvage anything valuable from [src]!</span>")
		else
			return
		...
		if(crowbar_salvage && crowbar_salvage.len)
			...
			user.text2tab("<span class='warning'>You don't see anything that can be pried with [I]!</span>")
