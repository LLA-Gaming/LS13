...
/mob/living/silicon/ai/proc/nuke_station()
	...
	src.text2tab("<span class='notice'>Nuclear device armed.</span>")
...
/obj/machinery/doomsday_device/proc/detonate(z_level = 1)
	...
	for(var/mob/living/L in mob_list)
		...
		if(issilicon(L))
			continue
		L.text2tab("<span class='danger'><B>The blast wave from the [src] tears you atom from atom!</B></span>")
		L.dust()
...
/mob/living/silicon/ai/proc/fireproof_core()
	...
	src.text2tab("<span class='notice'>Core fireproofed.</span>")
...
/mob/living/silicon/ai/proc/upgrade_turrets()
	...
	src.text2tab("<span class='notice'>Turrets upgraded.</span>")
...
/mob/living/silicon/ai/proc/lockdown()
	...
	src.text2tab("<span class = 'warning'>Lockdown Initiated. Network reset in 90 seconds.</span>")
...
/mob/living/silicon/ai/proc/disable_rcd()
	...
	src.text2tab("<span class='warning'>RCD detonation pulse emitted.</span>")
	malf_cooldown = 1
	spawn(100)
		malf_cooldown = 0
...
/mob/living/silicon/ai/proc/mech_takeover()
	...
	src.text2tab("Virus package compiled. Select a target mech at any time. <b>You must remain on the station at all times. Loss of signal will result in total system lockout.</b>")
...
/mob/living/silicon/ai/proc/break_fire_alarms()
	...
	for(var/obj/machinery/firealarm/F in machines)
		if(F.z != ZLEVEL_STATION)
			continue
		F.emagged = 1
	src.text2tab("<span class='notice'>All thermal sensors on the station have been disabled. Fire alerts will no longer be recognized.</span>")
...
/mob/living/silicon/ai/proc/break_air_alarms()
	...
	for(var/obj/machinery/airalarm/AA in machines)
		if(AA.z != ZLEVEL_STATION)
			continue
		AA.emagged = 1
	src.text2tab("<span class='notice'>All air alarm safeties on the station have been overriden. Air alarms may now use the Flood environmental mode.")
	...
	if (istype(M, /obj/machinery))
		for(var/datum/AI_Module/small/overload_machine/overload in current_modules)
			if(overload.uses > 0)
				...
				src.text2tab("<span class='warning'>Overloading machine circuitry...</span>")
				spawn(50)
					if(M)
						explosion(get_turf(M), 0,1,1,0)
						qdel(M)
			else src.text2tab("<span class='notice'>Out of uses.</span>")
	else src.text2tab("<span class='notice'>That's not a machine.</span>")
...
/mob/living/silicon/ai/proc/override_machine(obj/machinery/M in machines)
	...
	if (istype(M, /obj/machinery))
		if(!M.can_be_overridden())
			src.text2tab("Can't override this device.")
		for(var/datum/AI_Module/small/override_machine/override in current_modules)
			if(override.uses > 0)
				override.uses --
				audible_message("<span class='italics'>You hear a loud electrical buzzing sound!</span>")
				src.text2tab("<span class='warning'>Reprogramming machine behaviour...</span>")
				spawn(50)
					if(M && !qdeleted(M))
...
/mob/living/silicon/ai/proc/place_transformer()
	...
	if(sure == "Yes")
		...
		PCT.uses --
		can_shunt = 0
		src.text2tab("<span class='warning'>You cannot shunt anymore.</span>")
...
/mob/living/silicon/ai/proc/blackout()
	...
	for(var/datum/AI_Module/small/blackout/blackout in current_modules)
		if(blackout.uses > 0)
			...
			src.text2tab("<span class='notice'>Overcurrent applied to the powernet.</span>")
		else src.text2tab("<span class='notice'>Out of uses.</span>")
	...
	for(var/datum/AI_Module/small/reactivate_cameras/camera in current_modules)
		for(var/obj/machinery/camera/C in cameranet.cameras)
			...
			if(camera.uses > 0)
				...
				src.text2tab("<span class='warning'>Out of uses.</span>")
				verbs -= /mob/living/silicon/ai/proc/reactivate_cameras //It is useless now, clean it up.
				break
	src.text2tab("<span class='notice'>Diagnostic complete! Operations completed: [fixedcams].</span>")
...
/mob/living/silicon/ai/proc/upgrade_cameras()
	...
	src.text2tab("<span class='notice'>OTA firmware distribution complete! Cameras upgraded: [upgradedcams]. Light amplification system online.</span>")
...
/datum/module_picker/Topic(href, href_list)
	...
	if(A.stat == DEAD)
		A.text2tab("You are already dead!") //Omae Wa Mou Shindeiru
		return
...
/mob/living/silicon/ai/proc/surveillance()
	...
	src.text2tab("<span class='notice'>OTA firmware distribution complete! Cameras upgraded: Enhanced surveillance package online.</span>")
