...
/obj/effect/proc_holder/spell/proc/shadowling_check(var/mob/living/carbon/human/H)
	...
	if(!is_shadow_or_thrall(usr)) usr.text2tab("<span class='warning'>You can't wrap your head around how to do this.</span>")
	else if(is_thrall(usr)) usr.text2tab("<span class='warning'>You aren't powerful enough to do this.</span>")
	else if(is_shadow(usr)) usr.text2tab("<span class='warning'>Your telepathic ability is suppressed. Hatch or use Rapid Re-Hatch first.</span>")
	return 0
...
/obj/effect/proc_holder/spell/targeted/glare/cast(list/targets,mob/user = usr)
	for(var/mob/living/target in targets)
		if(!ishuman(target))
			user.text2tab("<span class='warning'>You may only glare at humans!</span>")
			revert_cast()
			return
		if(!shadowling_check(user))
			...
			return
		if(target.stat)
			user.text2tab("<span class='warning'>[target] must be conscious!</span>")
			revert_cast()
			return
		if(is_shadow_or_thrall(target))
			user.text2tab("<span class='warning'>You cannot glare at allies!</span>")
			revert_cast()
			return
		var/mob/living/L = user
		if(L.incorporeal_move) //Other abilities can still be used, but glare needed balancing
			user.text2tab("<span class='warning'>You cannot glare while shadow walking!</span>")
			revert_cast()
			return
		...
		if(in_range(target, user))
			target.text2tab("<span class='userdanger'>Your gaze is forcibly drawn into [user]'s eyes, and you are mesmerized by the heavenly lights...</span>")
		else //Only alludes to the shadowling if the target is close by
			target.text2tab("<span class='userdanger'>Red lights suddenly dance in your vision, and you are mesmerized by their heavenly beauty...</span>")
		target.Stun(10)
		M.silent += 10
...
/obj/effect/proc_holder/spell/aoe_turf/veil/cast(list/targets,mob/user = usr)
	if(!shadowling_check(user) && !admin_override)
		revert_cast()
		return
	user.text2tab("<span class='shadowling'>You silently disable all nearby lights.</span>")
...
/obj/effect/proc_holder/spell/aoe_turf/flashfreeze/cast(list/targets,mob/user = usr)
	if(!shadowling_check(user))
		revert_cast()
		return
	user.text2tab("<span class='shadowling'>You freeze the nearby air.</span>")
	for(var/turf/T in targets)
		for(var/mob/living/carbon/M in T.contents)
			if(is_shadow_or_thrall(M))
				if(M == user) //No message for the user, of course
					...
					M.text2tab("<span class='danger'>You feel a blast of paralyzingly cold air wrap around you and flow past, but you are unaffected!</span>")
					continue
			M.text2tab("<span class='userdanger'>A wave of shockingly cold air engulfs you!</span>")
			M.Stun(2)
...
/obj/effect/proc_holder/spell/targeted/enthrall/cast(list/targets,mob/living/carbon/human/user = usr)
	...
	for(var/mob/living/carbon/human/target in targets)
		if(!in_range(user, target))
			user.text2tab("<span class='warning'>You need to be closer to enthrall [target]!</span>")
			revert_cast()
			return
		if(!target.key || !target.mind)
			user.text2tab("<span class='warning'>The target has no mind!</span>")
			revert_cast()
			return
		if(target.stat)
			user.text2tab("<span class='warning'>The target must be conscious!</span>")
			revert_cast()
			return
		if(is_shadow_or_thrall(target))
			user.text2tab("<span class='warning'>You can not enthrall allies!</span>")
			revert_cast()
			return
		if(!ishuman(target))
			user.text2tab("<span class='warning'>You can only enthrall humans!</span>")
			revert_cast()
			return
		if(enthralling)
			user.text2tab("<span class='warning'>You are already enthralling!</span>")
			revert_cast()
			return
		if(!target.client)
			user.text2tab("<span class='warning'>[target]'s mind is vacant of activity.</span>")
		enthralling = 1
		user.text2tab("<span class='danger'>This target is valid. You begin the enthralling...</span>")
		target.text2tab("<span class='userdanger'>[user] stares at you. You feel your head begin to pulse.</span>")

		for(var/progress = 0, progress <= 3, progress++)
			switch(progress)
				if(1)
					user.text2tab("<span class='notice'>You place your hands to [target]'s head...</span>")
					user.visible_message("<span class='warning'>[user] places their hands onto the sides of [target]'s head!</span>")
				if(2)
					user.text2tab("<span class='notice'>You begin preparing [target]'s mind as a blank slate...</span>")
					user.visible_message("<span class='warning'>[user]'s palms flare a bright red against [target]'s temples!</span>")
					target.text2tab("<span class='danger'>A terrible red light floods your mind. You collapse as conscious thought is wiped away.</span>")
					target.Weaken(12)
					sleep(20)
					if(isloyal(target))
						user.text2tab("<span class='notice'>They are protected by a mindshield implant. You begin to shut down the nanobot implant - this will take some time.</span>")
						user.visible_message("<span class='warning'>[user] pauses, then dips their head in concentration!</span>")
						target.text2tab("<span class='boldannounce'>You feel your resolve begin to fade!</span>")
						sleep(100) //10 seconds - not spawn() so the enthralling takes longer
						user.text2tab("<span class='notice'>The nanobots composing the mindshield implant have been rendered inert. Now to continue.</span>")
						user.visible_message("<span class='warning'>[user] relaxes again.</span>")
						for(var/obj/item/weapon/implant/mindshield/L in target)
							if(L && L.implanted)
								qdel(L)
						target.text2tab("<span class='boldannounce'>You feel the protection from your mindshield implant strain and fail.</span>")
				if(3)
					user.text2tab("<span class='notice'>You begin planting the tumor that will control the new thrall...</span>")
					user.visible_message("<span class='warning'>A strange energy passes from [user]'s hands into [target]'s head!</span>")
					target.text2tab("<span class='boldannounce'>You feel your memories twisting, morphing. A sense of horror dominates your mind.</span>")
			if(!do_mob(user, target, 70)) //around 21 seconds total for enthralling, 31 for someone with a mindshield implant
				user.text2tab("<span class='warning'>The enthralling has been interrupted - your target's mind returns to its previous state.</span>")
				target.text2tab("<span class='userdanger'>You wrest yourself away from [user]'s hands and compose yourself</span>")
				enthralling = 0
				return

		enthralling = 0
		user.text2tab("<span class='shadowling'>You have enthralled <b>[target.real_name]</b>!</span>")
...
/obj/effect/proc_holder/spell/self/shadowling_hivemind/cast(mob/living/user,mob/user = usr)
	if(!is_shadow(user))
		user.text2tab("<span class='warning'>You must be a shadowling to do that!</span>")
		return
...
/obj/effect/proc_holder/spell/self/shadowling_regenarmor/cast(mob/living/carbon/human/user)
	if(!is_shadow(user))
		user.text2tab("<span class='warning'>You must be a shadowling to do this!</span>")
		revert_cast()
		return
...
/obj/effect/proc_holder/spell/self/collective_mind/cast(mob/living/carbon/human/user)
	...
	user.text2tab("<span class='shadowling'><b>You focus your telepathic energies abound, harnessing and drawing together the strength of your thralls.</b></span>")

	for(M in living_mob_list)
		if(is_thrall(M))
			thralls++
			M.text2tab("<span class='shadowling'>You feel hooks sink into your mind and pull.</span>")

	if(!do_after(user, 30, target = user))
		user.text2tab("<span class='warning'>Your concentration has been broken. The mental hooks you have sent out now retract into your mind.</span>")
		return

	if(thralls >= 3 && !screech_acquired)
		screech_acquired = 1
		user.text2tab("<span class='shadowling'><i>The power of your thralls has granted you the <b>Sonic Screech</b> ability. This ability will shatter nearby windows and deafen enemies, plus stunning silicon lifeforms.</span>")
	...
	if(thralls >= 5 && !blind_smoke_acquired)
		...
		user.text2tab("<span class='shadowling'><i>The power of your thralls has granted you the <b>Blinding Smoke</b> ability. It will create a choking cloud that will blind any non-thralls who enter. \
			</i></span>")
		user.mind.AddSpell(new /obj/effect/proc_holder/spell/self/blindness_smoke(null))

	if(thralls >= 7 && !drainLifeAcquired)
		drainLifeAcquired = 1
		user.text2tab("<span class='shadowling'><i>The power of your thralls has granted you the <b>Drain Life</b> ability. You can now drain the health of nearby humans to heal yourself.</i></span>")
	...
	if(thralls >= 9 && !reviveThrallAcquired)
		...
		user.text2tab("<span class='shadowling'><i>The power of your thralls has granted you the <b>Black Recuperation</b> ability. This will, after a short time, bring a dead thrall completely back to life \
		with no bodily defects.</i></span>")
		user.mind.AddSpell(new /obj/effect/proc_holder/spell/targeted/revive_thrall(null))

	if(thralls < victory_threshold)
		user.text2tab("<span class='shadowling'>You do not have the power to ascend. You require [victory_threshold] thralls, but only [thralls] living thralls are present.</span>")

	else if(thralls >= victory_threshold)
		user.text2tab("<span class='shadowling'><b>You are now powerful enough to ascend. Use the Ascendance ability when you are ready. <i>This will kill all of your thralls.</i></span>")
		user.text2tab("<span class='shadowling'><b>You may find Ascendance in the Shadowling Evolution tab.</b></span>")
		for(M in living_mob_list)
			if(is_shadow(M))
				...
				if(M == user)
					M.text2tab("<span class='shadowling'><i>You project this power to the rest of the shadowlings.</i></span>")
				else
					M.text2tab("<span class='shadowling'><b>[user.real_name] has coalesced the strength of the thralls. You can draw upon it at any time to ascend. (Shadowling Evolution Tab)</b></span>") //Tells all the other shadowlings
...
/obj/effect/proc_holder/spell/self/blindness_smoke/cast(mob/living/carbon/human/user) //Extremely hacky
	...
	user.text2tab("<span class='shadowling'>You regurgitate a vast cloud of blinding smoke.</span>")
	...
	for(var/turf/T in targets)
		for(var/mob/target in T.contents)
			...
			if(iscarbon(target))
				...
				M.text2tab("<span class='danger'><b>A spike of pain drives into your head and scrambles your thoughts!</b></span>")
				M.confused += 10
				M.setEarDamage(M.ear_damage + 3)
			else if(issilicon(target))
				var/mob/living/silicon/S = target
				S.text2tab("<span class='warning'><b>ERROR $!(@ ERROR )#^! SENSORY OVERLOAD \[$(!@#</b></span>")
...
/obj/effect/proc_holder/spell/aoe_turf/drain_life/cast(list/targets, mob/living/carbon/human/user = usr)
	...
	if(!targetsDrained)
		revert_cast()
		user.text2tab("<span class='warning'>There were no nearby humans for you to drain.</span>")
		return
	for(var/mob/living/carbon/M in nearbyTargets)
		...
		M.text2tab("<span class='boldannounce'>You feel a wave of exhaustion and a curious draining sensation directed towards [usr]!</span>")
	user.text2tab("<span class='shadowling'>You draw life from those around you to heal your wounds.</span>")
		...
		switch(choice)
			if("Empower")
				if(!is_thrall(thrallToRevive))
					user.text2tab("<span class='warning'>[thrallToRevive] is not a thrall.</span>")
					revert_cast()
					return
				if(thrallToRevive.stat != CONSCIOUS)
					user.text2tab("<span class='warning'>[thrallToRevive] must be conscious to become empowered.</span>")
					revert_cast()
					return
				if(thrallToRevive.dna.species.id == "l_shadowling")
					user.text2tab("<span class='warning'>[thrallToRevive] is already empowered.</span>")
					revert_cast()
					return
				...
				if(empowered_thralls >= EMPOWERED_THRALL_LIMIT)
					user.text2tab("<span class='warning'>You cannot spare this much energy. There are too many empowered thralls.</span>")
					revert_cast()
					return
				...
				thrallToRevive.text2tab("<span class='userdanger'>[user] places their hands over your face. You feel energy gathering. Stand still...</span>")
				if(!do_mob(user, thrallToRevive, 80))
					user.text2tab("<span class='warning'>Your concentration snaps. The flow of energy ebbs.</span>")
					revert_cast()
					return
				user.text2tab("<span class='shadowling'><b><i>You release a massive surge of power into [thrallToRevive]!</b></i></span>")
			...
			if("Revive")
				...
				if(thrallToRevive.stat != DEAD)
					user.text2tab("<span class='warning'>[thrallToRevive] is not dead.</span>")
					revert_cast()
					return
...
/obj/effect/proc_holder/spell/targeted/shadowling_extend_shuttle/cast(list/targets, mob/living/carbon/human/user = usr)
	...
	for(var/mob/living/carbon/human/target in targets)
		if(target.stat)
			...
			return
		if(!is_thrall(target))
			user.text2tab("<span class='warning'>[target] must be a thrall.</span>")
			revert_cast()
			return
		if(SSshuttle.emergency.mode != SHUTTLE_CALL)
			user.text2tab("<span class='warning'>The shuttle must be inbound only to the station.</span>")
			revert_cast()
			return
		var/mob/living/carbon/human/M = target
		...
		if(!do_after(user, 50, target = M))
			M.text2tab("<span class='warning'>You are snapped back to reality, your haze dissipating!</span>")
			user.text2tab("<span class='warning'>You have been interrupted. The draw has failed.</span>")
			return
		user.text2tab("<span class='notice'>You project [M]'s life force toward the approaching shuttle, extending its arrival duration!</span>")
...
/obj/effect/proc_holder/spell/targeted/lesser_glare/cast(list/targets,mob/user = usr)
	for(var/mob/living/target in targets)
		if(!ishuman(target) || !target)
			user.text2tab("<span class='warning'>You nay only glare at humans!</span>")
			revert_cast()
			return
		if(target.stat)
			user.text2tab("<span class='warning'>[target] must be conscious!</span>")
		...
		if(in_range(target, user))
			target.text2tab("<span class='userdanger'>Your gaze is forcibly drawn into [user]'s eyes, and you are starstruck by the heavenly lights...</span>")
		else //Only alludes to the shadowling if the target is close by
			target.text2tab("<span class='userdanger'>Red lights suddenly dance in your vision, and you are starstruck by their heavenly beauty...</span>")
...
/obj/effect/proc_holder/spell/self/thrall_vision/cast(mob/living/carbon/human/user)
	active = !active
	if(active)
		user.text2tab("<span class='notice'>You shift the nerves in your eyes, allowing you to see in the dark.</span>")
		user.dna.species.darksight = 8
		user.dna.species.invis_sight = SEE_INVISIBLE_MINIMUM
	else
		user.text2tab("<span class='notice'>You return your vision to normal.</span>")
...
/obj/effect/proc_holder/spell/self/lesser_shadowling_hivemind/cast(mob/living/carbon/human/user)
	if(!is_shadow_or_thrall(user))
		user.text2tab("<span class='warning'><b>As you attempt to commune with the others, an agonizing spike of pain drives itself into your head!</b></span>")
...
/obj/effect/proc_holder/spell/targeted/annihilate/cast(list/targets,mob/living/simple_animal/ascendant_shadowling/user = usr)
	if(user.incorporeal_move)
		user.text2tab("<span class='warning'>You are not in the same plane of existence. Unphase first.</span>")
		revert_cast()
		return
	for(var/mob/living/boom in targets)
		if(is_shadow(boom)) //Used to not work on thralls. Now it does so you can PUNISH THEM LIKE THE WRATHFUL GOD YOU ARE.
			user.text2tab("<span class='warning'>Making an ally explode seems unwise.<span>")
			revert_cast()
			return
...
/obj/effect/proc_holder/spell/targeted/hypnosis/cast(list/targets,mob/living/simple_animal/ascendant_shadowling/user = usr)
	...
	for(var/mob/living/carbon/human/target in targets)
		if(is_shadow_or_thrall(target))
			user.text2tab("<span class='warning'>You cannot enthrall an ally.<span>")
			revert_cast()
			return
		...
		user.text2tab("<span class='shadowling'>You instantly rearrange <b>[target]</b>'s memories, hyptonitizing them into a thrall.</span>")
		target.text2tab("<span class='userdanger'><font size=3>An agonizing spike of pain drives into your mind, and--</font></span>")
		target.mind.special_role = "thrall"
		ticker.mode.add_thrall(target.mind)
...
/obj/effect/proc_holder/spell/aoe_turf/ascendant_storm/cast(list/targets,mob/living/simple_animal/ascendant_shadowling/user = usr)
	...
	for(var/turf/T in targets)
		for(var/mob/living/carbon/human/target in T.contents)
			if(is_shadow_or_thrall(target))
				continue
			target.text2tab("<span class='userdanger'>You are struck by a bolt of lightning!</span>")
