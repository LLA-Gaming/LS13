...
/obj/item/device/nuclear_challenge/attack_self(mob/living/user)
	...
	declaring_war = 0
	if(are_you_sure == "No")
		user.text2tab("On second thought, the element of surprise isn't so bad after all.")
		return
	if(!check_allowed(user))
		return
	...
	user.text2tab("You've attracted the attention of powerful forces within the syndicate. A bonus bundle of telecrystals has been granted to your team. Great things await you if you complete the mission.")
...
/obj/item/device/nuclear_challenge/proc/check_allowed(mob/living/user)
	...
	if(player_list.len < CHALLENGE_MIN_PLAYERS)
		user.text2tab("The enemy crew is too small to be worth declaring war on.")
		return 0
	if(user.z != ZLEVEL_CENTCOM)
		user.text2tab("You have to be at your base to use this.")
		return 0
	if(world.time > CHALLENGE_TIME_LIMIT)
		user.text2tab("It's too late to declare hostilities. Your benefactors are already busy with other schemes. You'll have to make  do with what you have on hand.")
		return 0
	for(var/V in syndicate_shuttle_boards)
		var/obj/item/weapon/circuitboard/computer/syndicate_shuttle/board = V
		if(board.moved)
			user.text2tab("The shuttle has already been moved! You have forfeit the right to declare war.")
			return 0
	return 1
