...
/obj/item/weapon/pinpointer/attack_self()
	if(!active)
		active = 1
		workdisk()
		usr.text2tab("<span class='notice'>You activate the pinpointer.</span>")
	else
		active = 0
		icon_state = "pinoff"
		usr.text2tab("<span class='notice'>You deactivate the pinpointer.</span>")
...
/obj/item/weapon/pinpointer/examine(mob/user)
	...
	for(var/obj/machinery/nuclearbomb/bomb in machines)
		if(bomb.timing)
			user.text2tab("Extreme danger.  Arming signal detected.   Time remaining: [bomb.timeleft]")
...
/obj/item/weapon/pinpointer/advpinpointer/verb/toggle_mode()
	...
	switch(alert("Please select the mode you want to put the pinpointer in.", "Pinpointer Mode Select", "Location", "Disk Recovery", "Other Signature"))
		if("Location")
			...
			usr.text2tab("<span class='notice'>You set the pinpointer to locate [locationx],[locationy]</span>")
		...
		if("Other Signature")
			...
			switch(alert("Search for item signature or DNA fragment?" , "Signature Mode Select" , "" , "Item" , "DNA"))
				if("Item")
					...
					if(!target)
						usr.text2tab("<span class='warning'>Failed to locate [targetitem]!</span>")
						return
					usr.text2tab("<span class='notice'>You set the pinpointer to locate [targetitem].</span>")
				if("DNA")
...
/obj/item/weapon/pinpointer/nukeop/attack_self(mob/user)
	if(!active)
		...
		if(!mode)
			...
			worklocation()
		user.text2tab("<span class='notice'>You activate the pinpointer([mode_text]).</span>")
	else
		active = 0
		icon_state = "pinoff"
		user.text2tab("<span class='notice'>You deactivate the pinpointer.</span>")
...
/obj/item/weapon/pinpointer/operative/attack_self()
	if(!usr.mind || !(usr.mind in ticker.mode.syndicates))
		usr.text2tab("<span class='danger'>AUTHENTICATION FAILURE. ACCESS DENIED.</span>")
		return 0
	if(!active)
		active = 1
		workop()
...
/obj/item/weapon/pinpointer/operative/examine(mob/user)
	...
	if(active)
		if(nearest_op)
			user.text2tab("Nearest operative detected is <i>[nearest_op.real_name].</i>")
		else
			user.text2tab("No operatives detected within scanning range.")
