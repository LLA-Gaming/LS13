...
/obj/item/weapon/veilrender/attack_self(mob/user)
	if(charges > 0)
		...
		user.text2tab("<span class='danger'>The unearthly energies that powered the blade are now dormant.</span>")
...
/obj/item/weapon/scrying/attack_self(mob/user)
	user.text2tab("<span class='notice'>You can see...everything!</span>")
...
/obj/item/device/necromantic_stone/attack(mob/living/carbon/human/M, mob/living/carbon/human/user)
	...
	if(M.stat != DEAD)
		user.text2tab("<span class='warning'>This artifact can only affect the dead!</span>")
		return

	if(!M.mind || !M.client)
		user.text2tab("<span class='warning'>There is no soul connected to this body...</span>")
		return

	check_spooky()//clean out/refresh the list
	if(spooky_scaries.len >= 3 && !unlimited)
		user.text2tab("<span class='warning'>This artifact can only affect three undead at a time!</span>")
		return

	M.set_species(/datum/species/skeleton, icon_update=0)
	M.revive(full_heal = 1, admin_revive = 1)
	spooky_scaries |= M
	M.text2tab("<span class='userdanger'>You have been revived by </span><B>[user.real_name]!</B>")
	M.text2tab("<span class='userdanger'>They are your master now, assist them even if it costs you your new life!</span>")
...
/obj/item/weapon/multisword/attack_self(mob/user)
	if(user.mind.special_role == "apprentice")
		user.text2tab("<span class='warning'>You know better than to touch your teacher's stuff.</span>")
		return
	if(cooldown < world.time)
		...
		if(faction_check == 0)
			...
			user.text2tab("You bind the sword to yourself. You can now use it to summon help.")
			if(!usr.mind.special_role)
				if(prob(30))
					user.text2tab("<span class='warning'><B>With your new found power you could easily conquer the station!</B></span>")
					...
					usr.text2tab("<B>Objective #[1]</B>: [hijack_objective.explanation_text]")
					ticker.mode.traitors += usr.mind
					usr.mind.special_role = "[usr.real_name] Prime"
					evil = TRUE
				else
					user.text2tab("<span class='warning'><B>With your new found power you could easily defend the station!</B></span>")
					...
					usr.text2tab("<B>Objective #[1]</B>: [new_objective.explanation_text]")
			...
			if(candidates.len)
				...
				user.text2tab("<span class='warning'><B>The sword flashes, and you find yourself face to face with...you!</B></span>")
				...
				user.text2tab("You fail to summon any copies of yourself. Perhaps you should try again in a bit.")
	else
		user.text2tab("<span class='warning'><B>[src] is recharging! Keep in mind it shares a cooldown with the swords wielded by your copies.</span>")
...
/obj/item/weapon/multisword/proc/spawn_copy(var/client/C, var/turf/T)
	...
	M.text2tab("<B>You are an alternate version of [usr.real_name] from another universe! Help them accomplish their goals at all costs.</B>")
	...
	if(evil)
		...
		hijack_objective.explanation_text = "Ensure only [usr.real_name] and their copies are on the shuttle!"
		M.text2tab("<B>Objective #[1]</B>: [hijack_objective.explanation_text]")
		...
		M.text2tab("<B>Objective #[1]</B>: [new_objective.explanation_text]")
		M.mind.special_role = "multiverse traveller"
...
/obj/item/voodoo/attackby(obj/item/I, mob/user, params)
	if(target && cooldown < world.time)
		if(I.is_hot())
			target.text2tab("<span class='userdanger'>You suddenly feel very hot</span>")
			target.bodytemperature += 50
			GiveHint(target)
		else if(is_pointed(I))
			target.text2tab("<span class='userdanger'>You feel a stabbing pain in [parse_zone(user.zone_selected)]!</span>")
			target.Weaken(2)
			GiveHint(target)
		else if(istype(I,/obj/item/weapon/bikehorn))
			target.text2tab("<span class='userdanger'>HONK</span>")
	...
	if(!link)
		if(I.loc == user && istype(I) && I.w_class <= 2)
			...
			I.loc = src
			link = I
			user.text2tab("You attach [I] to the doll.")
			update_targets()
...
/obj/item/voodoo/attack_self(mob/user)
	...
	if(user.zone_selected == "chest")
		if(link)
			...
			user.text2tab("<span class='notice'>You remove the [link] from the doll.</span>")
			link = null
			update_targets()
			return
		...
		switch(user.zone_selected)
			...
			if("eyes")
				...
				spawn(100)
					...
					user.unset_machine()
			if("r_leg","l_leg")
				user.text2tab("<span class='notice'>You move the doll's legs around.</span>")
			...
			if("r_arm","l_arm")
				...
				if(nearby_mobs.len)
					...
					GiveHint(target)
			if("head")
				user.text2tab("<span class='notice'>You smack the doll's head with your hand.</span>")
				target.Dizzy(10)
				target.text2tab("<span class='warning'>You suddenly feel as if your head was hit with a hammer!</span>")
...
/obj/item/voodoo/proc/GiveHint(mob/victim,force=0)
	if(prob(50) || force)
		...
		victim.text2tab("<span class='notice'>You feel a dark presence from [way]</span>")
	if(prob(20) || force)
		var/area/A = get_area(src)
		victim.text2tab("<span class='notice'>You feel a dark presence from [A.name]</span>")
