...
/obj/item/weapon/banner/attack_self(mob/living/carbon/human/user)
	...
	if(!side)
		return
	user.text2tab("<span class='notice'>You increase the morale of your fellows!</span>")
	moralecooldown = world.time

	for(var/mob/living/carbon/human/H in range(4,get_turf(src)))
		if((side == "red") && is_handofgod_redcultist(H) || (side == "blue") && is_handofgod_bluecultist(H))
			H.text2tab("<span class='notice'>Your morale is increased by [user]'s banner!</span>")
...
/obj/item/weapon/banner/red/examine(mob/user)
	..()
	if(is_handofgod_redcultist(user))
		user.text2tab("A banner representing our might against the heretics. We may use it to increase the morale of our fellow members!")
	else if(is_handofgod_bluecultist(user))
		user.text2tab("A heretical banner that should be destroyed posthaste.")
...
/obj/item/clothing/suit/armor/plate/crusader/examine(mob/user)
	...
	if(!is_handofgod_cultist(user))
		user.text2tab("Armour that's comprised of metal and cloth.")
	else
		user.text2tab("Armour that was used to protect from backstabs, gunshots, explosives, and lasers.  The original wearers of this type of armour were trying to avoid being murdered.  Since they're not around anymore, you're not sure if they were successful or not.")
...
/obj/item/clothing/head/helmet/plate/crusader/examine(mob/user)
	..()
	if(!is_handofgod_cultist(user))
		user.text2tab("A brownish hood.")
	else
		user.text2tab("A hood that's very protective, despite being made of cloth.  Due to the tendency of the wearer to be targeted for assassinations, being protected from being shot in the face was very important..")
	...
	if(slot == slot_head)
		switch(side)
			...
			if("red")
				...
				faithful = 1
		if(!faithful)
			user.text2tab("<span class='danger'>Your mind is assaulted by a vast power, furious at your desecration!</span>")
			user.emote("scream")
...
/obj/item/clothing/head/helmet/plate/crusader/prophet/examine(mob/user)
	..()
	if(!is_handofgod_cultist(user))
		user.text2tab("A brownish, religious-looking hat.")
	else
		user.text2tab("A hat bestowed upon a prophet of gods and demigods.")
		user.text2tab("This hat belongs to the [side] god.")
...
/obj/item/weapon/godstaff/examine(mob/user)
	..()
	if(!is_handofgod_cultist(user))
		user.text2tab("It's a stick..?")
	else
		user.text2tab("A powerful staff capable of changing the allegiance of god/demigod structures.")



/obj/item/weapon/godstaff/attack_self(mob/living/carbon/user)
	if((god && !god.is_handofgod_myprophet(user)) || !god)
		user.text2tab("<span class='danger'>YOU ARE NOT THE CHOSEN ONE!</span>")
		return
	if(!(istype(user.head, /obj/item/clothing/head/helmet/plate/crusader/prophet)))
		user.text2tab("<span class='warning'>Your connection to your diety isn't strong enough! You must wear your big hat!</span>")
		return
...
/obj/item/clothing/gloves/plate/examine(mob/user)
	..()
	if(!is_handofgod_cultist(user))
		usr.text2tab("They're like gloves, but made of metal.")
	else
		usr.text2tab("Protective gloves that are also blessed to protect from heat and shock.")
...
/obj/item/clothing/shoes/plate/examine(mob/user)
	..()
	if(!is_handofgod_cultist(user))
		usr.text2tab("Metal boots, they look heavy.")
	else
		usr.text2tab("Heavy boots that are blessed for sure footing.  You'll be safe from being taken down by the heresy that is the banana peel.")
