...
/mob/camera/god/Destroy()
	...
	for(var/datum/mind/F in followers)
		if(F.current)
			F.current.text2tab("<span class='danger'>Your god is DEAD!</span>")
...
/mob/camera/god/proc/forceplacenexus()
	...
	src.text2tab("<span class='danger'>You failed to place your nexus, and it has been placed for you!</span>")
...
/mob/camera/god/Login()
	..()
	sync_mind()
	src.text2tab("<span class='notice'>You are a deity!</span>")
	src.text2tab("You are a deity and are worshipped by a cult!  You are rather weak right now, but that will change as you gain more followers.")
	src.text2tab("You will need to place an anchor to this world, a <b>Nexus</b>, in two minutes.  If you don't, one will be placed immediately below you.")
	src.text2tab("Your <b>Follower</b> count determines how many people believe in you and are a part of your cult.")
	src.text2tab("Your <b>Nexus Integrity</b> tells you the condition of your nexus.  If your nexus is destroyed, you will die. Place your Nexus on a safe, isolated place, that is still accessible to your followers.")
	src.text2tab("Your <b>Faith</b> is used to interact with the world.  This will regenerate on its own, and it goes faster when you have more followers and power pylons.")
	src.text2tab("The first thing you should do after placing your nexus is to <b>appoint a prophet</b>.  Only prophets can hear you talk, unless you use an expensive power.")
	update_health_hud()
...
/mob/camera/god/proc/place_nexus()
	...
	if(A)
		...
		for(var/datum/mind/F in followers)
			if(F.current)
				F.current.text2tab("<span class='boldnotice'>Your god's nexus is in \the [areaname]</span>")
...
/mob/camera/god/proc/check_death()
	if(!alive_followers)
		src.text2tab("<span class='userdanger'>You no longer have any followers. You shudder as you feel your existence cease...</span>")
...
/mob/camera/god/say(msg)
	...
	if(client)
		if(client.prefs.muted & MUTE_IC)
			src.text2tab("You cannot send IC messages (muted).")
			return
...
/mob/camera/god/Topic(href, href_list)
	if(href_list["create_structure"])
		...
		src.text2tab("<span class='boldnotice'>You may click a construction site to cancel it, but only faith is refunded.</span>")
	...
	if(href_list["place_trap"])
		...
		src.text2tab("You lay \a [initial(trap_type.name)]")
		add_faith(-20)
