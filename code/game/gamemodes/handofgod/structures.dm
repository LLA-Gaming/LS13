...
/obj/structure/divine/attackby(obj/item/I, mob/user)
	...
	if(istype(I, /obj/item/weapon/godstaff))
		if(!is_handofgod_cultist(user))
			user.text2tab("<span class='notice'>You're not quite sure what the hell you're even doing.</span>")
			return
...
/obj/structure/divine/proc/assign_deity(mob/camera/god/new_deity, alert_old_deity = TRUE)
	...
	if(deity)
		if(alert_old_deity)
			deity.text2tab("<span class='danger'><B>Your [name] was captured by [new_deity]'s cult!</B></span>")
		deity.structures -= src
	deity = new_deity
	deity.structures |= src
	side = deity.side
...
/obj/structure/divine/construction_holder/attackby(obj/item/I, mob/user)
	...
	if(istype(I, /obj/item/stack/sheet/metal))
		...
		if(metal_cost)
			...
			user.text2tab("<span class='notice'>You add [spend] metal to \the [src].")
			metal_cost = max(0, metal_cost - spend)
			M.use(spend)
			check_completion()
		else
			user.text2tab("<span class='notice'>\The [src] does not require any more metal!")
		return
	...
	if(istype(I, /obj/item/stack/sheet/glass))
		...
		if(glass_cost)
			...
			user.text2tab("<span class='notice'>You add [spend] glass to \the [src].")
			glass_cost = max(0, glass_cost - spend)
			G.use(spend)
			check_completion()
		else
			user.text2tab("<span class='notice'>\The [src] does not require any more glass!")
		return
	...
	if(istype(I, /obj/item/stack/sheet/lessergem))
		...
		if(lesser_gem_cost)
			...
			user.text2tab("<span class='notice'>You add [spend] lesser gems to \the [src].")
			lesser_gem_cost = max(0, lesser_gem_cost - spend)
			LG.use(spend)
			check_completion()
		else
			user.text2tab("<span class='notice'>\The [src] does not require any more lesser gems!")
		return
	...
	if(istype(I, /obj/item/stack/sheet/greatergem))
		...
		if(greater_gem_cost)
			...
			user.text2tab("<span class='notice'>You add [spend] greater gems to \the [src].")
			greater_gem_cost = max(0, greater_gem_cost - spend)
			GG.use(spend)
			check_completion()
		else
			user.text2tab("<span class='notice'>\The [src] does not require any more greater gems!")
		return
...
/obj/structure/divine/construction_holder/examine(mob/user)
	...
	if(metal_cost || glass_cost || lesser_gem_cost || greater_gem_cost)
		user.text2tab("To finish construction it requires the following materials:")
		if(metal_cost)
			user.text2tab("[metal_cost] metal <IMG CLASS=icon SRC=icons/obj/items.dmi ICONSTATE='sheet-metal'>")
		if(glass_cost)
			user.text2tab("[glass_cost] glass <IMG CLASS=icon SRC=icons/obj/items.dmi ICONSTATE='sheet-glass'>")
		if(lesser_gem_cost)
			user.text2tab("[lesser_gem_cost] lesser gems <IMG CLASS=icon SRC=icons/obj/items.dmi ICONSTATE='sheet-lessergem'>")
		if(greater_gem_cost)
			user.text2tab("[greater_gem_cost] greater gems <IMG CLASS=icon SRC=icons/obj/items.dmi ICONSTATE='sheet-greatergem'>")
...
/obj/structure/divine/nexus/take_damage(damage, damage_type = BRUTE, sound_effect = 1)
	...
	if(!health)
		if(!qdeleted(deity) && deity.nexus_required)
			deity.text2tab("<span class='danger'>Your nexus was destroyed. You feel yourself fading...</span>")
			qdel(deity)
		visible_message("<span class='danger'>\The [src] was destroyed!</span>")
		qdel(src)
...
/obj/structure/divine/convertaltar/attack_hand(mob/living/user)
	...
	if(!is_handofgod_cultist(user))
		user.text2tab("<span class='notice'>You try to use it, but unfortunately you don't know any rituals.</span>")
		return
	if(!H)
		return
	if(!H.mind)
		user.text2tab("<span class='danger'>Only sentients may serve your deity.</span>")
		return
	if((side == "red" && is_handofgod_redcultist(user) && !is_handofgod_redcultist(H)) || (side == "blue" && is_handofgod_bluecultist(user) && !is_handofgod_bluecultist(H)))
		user.text2tab("<span class='notice'>You invoke the conversion ritual.</span>")
		ticker.mode.add_hog_follower(H.mind, side)
	else
		user.text2tab("<span class='notice'>You invoke the conversion ritual.</span>")
		user.text2tab("<span class='danger'>But the altar ignores your words...</span>")
...
/obj/structure/divine/sacrificealtar/attack_hand(mob/living/user)
	...
	if((side == "red" && is_handofgod_redcultist(user))	|| (side == "blue" && is_handofgod_bluecultist(user)))
		if((side == "red" && is_handofgod_redcultist(L)) || (side == "blue" && is_handofgod_bluecultist(L)))
			user.text2tab("<span class='danger'>You cannot sacrifice a fellow cultist.</span>")
			return
		user.text2tab("<span class='notice'>You attempt to sacrifice [L] by invoking the sacrificial ritual.</span>")
		sacrifice(L)
	else
		user.text2tab("<span class='notice'>You attempt to sacrifice [L] by invoking the sacrificial ritual.</span>")
		user.text2tab("<span class='danger'>But the altar ignores your words...</span>")
...
/obj/structure/divine/healingfountain/attack_hand(mob/living/user)
	...
	if(last_process + time_between_uses > world.time)
		user.text2tab("<span class='notice'>The fountain appears to be empty.</span>")
		return
	last_process = world.time
	if(!is_handofgod_cultist(user) && cult_only)
		user.text2tab("<span class='danger'><B>The water burns!</b></spam>")
		user.reagents.add_reagent("hell_water",20)
	else
		user.text2tab("<span class='notice'>The water feels warm and soothing as you touch it. The fountain immediately dries up shortly afterwards.</span>")
...
/obj/structure/divine/defensepylon/examine(mob/user)
        ..()
        user.text2tab("<span class='notice'>\The [src] looks [pylon_gun.on ? "on" : "off"].</span>")
...
/obj/structure/divine/defensepylon/attack_god(mob/camera/god/user)
	if(user.side == side)
		if(deactivated)
			user.text2tab("You need to reveal it first!")
			return
...
/obj/structure/divine/lazarusaltar/attack_hand(mob/living/user)
	...
	if((side == "red" && is_handofgod_redcultist(user))) || (side == "blue" && is_handofgod_bluecultist(user)))
		user.text2tab("<span class='notice'>You attempt to revive [L] by invoking the rebirth ritual.</span>")
		L.revive()
