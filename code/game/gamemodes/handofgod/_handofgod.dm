...
/datum/game_mode/hand_of_god
	...
	restricted_jobs = list("Chaplain","AI", "Cyborg", "Security Officer", "Warden", "Detective", "Head of Security", "Station Manager", "Head of Personnel")
...
/datum/game_mode/proc/greet_hog_follower(datum/mind/follower_mind,colour)
	if(follower_mind in blue_deity_prophets || follower_mind in red_deity_prophets)
		follower_mind.current.text2tab("<span class='danger'><B>You have been appointed as the prophet of the [colour] deity! You are the only one who can communicate with your deity at will. Guide your followers, but be wary, for many will want you dead.</span>")
	else if(colour)
		follower_mind.current.text2tab("<span class='danger'><B>You are a follower of the [colour] cult's deity!</span>")
	else
		follower_mind.current.text2tab("<span class='danger'><B>You are a follower of a cult's deity!</span>")
...
/datum/game_mode/proc/add_hog_follower(datum/mind/follower_mind, colour = "No Colour")
	...
	if(isloyal(H))
		H.text2tab("<span class='danger'>Your mindshield implant blocked the influence of the [colour] deity. </span>")
		return 0
	if((follower_mind in red_deity_followers) || (follower_mind in red_deity_prophets) || (follower_mind in blue_deity_followers) || (follower_mind in blue_deity_prophets))
		H.text2tab("<span class='danger'>You already belong to a deity. Your strong faith has blocked out the conversion attempt by the followers of the [colour] deity.</span>")
		return 0
	var/obj/item/weapon/nullrod/N = H.null_rod_check()
	if(N)
		H.text2tab("<span class='danger'>Your holy weapon prevented the [colour] deity from brainwashing you.</span>")
		return 0
	...
	follower_mind.current.text2tab("<span class='danger'><FONT size = 3>You are now a follower of the [colour] deity! Follow your deity's prophet in order to complete your deity's objectives. Convert crewmembers to your cause by using your deity's nexus. And remember - there is no you, there is only the cult.</FONT></span>")
...
/datum/game_mode/proc/remove_hog_follower(datum/mind/follower_mind, announce = 1)//deconverts both
	...
	if(announce)
		...
		follower_mind.current.text2tab("<span class='danger'><b>Your mind has been cleared from the brainwashing the followers have done to you.  Now you serve yourself and the crew.</b></span>")
		for(var/mob/living/M in view(follower_mind.current))
			M.text2tab("[follower_mind.current] looks like their faith is shattered. They're no longer a cultist!")
