...
/obj/effect/proc_holder/changeling/weapon/sting_action(mob/living/user)
	if(!user.drop_item())
		user.text2tab("<span class='warning'>The [user.get_active_hand()] is stuck to your hand, you cannot grow a [weapon_name_simple] over it!</span>")
		return
	var/limb_regen = 0
...
/obj/effect/proc_holder/changeling/suit/sting_action(mob/living/carbon/human/user)
	if(!user.canUnEquip(user.wear_suit))
		user.text2tab("\the [user.wear_suit] is stuck to your body, you cannot grow a [suit_name_simple] over it!")
		return
	if(!user.canUnEquip(user.head))
		user.text2tab("\the [user.head] is stuck on your head, you cannot grow a [helmet_name_simple] over it!")
		return
	...
	if(istype(target, /obj/structure/table))
		...
		if(A.hasPower())
			if(A.locked)
				...
				return
			user.text2tab("<span class='warning'>The airlock's motors are resisting, this may take time...</span>")
