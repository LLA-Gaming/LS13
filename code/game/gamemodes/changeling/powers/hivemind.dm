...
/obj/effect/proc_holder/changeling/hivemind_comms/on_purchase(var/mob/user)
	...
	user.text2tab("<i><font color=#800080>Use say \":g message\" to communicate with the other changelings.</font></i>")
...
/obj/effect/proc_holder/changeling/hivemind_upload/sting_action(var/mob/user)
	...
	if(names.len <= 0)
		user.text2tab("<span class='notice'>The airwaves already have all of our DNA.</span>")
		return
	...
	user.text2tab("<span class='notice'>We channel the DNA of [chosen_name] to the air.</span>")
...
/obj/effect/proc_holder/changeling/hivemind_download/sting_action(mob/user)
	...
	if(names.len <= 0)
		user.text2tab("<span class='notice'>There's no new DNA to absorb from the air.</span>")
		return
	...
	user.text2tab("<span class='notice'>We absorb the DNA of [S] from the air.</span>")
