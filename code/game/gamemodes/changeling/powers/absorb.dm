...
/obj/effect/proc_holder/changeling/absorbDNA/can_sting(mob/living/carbon/user)
	...
	if(changeling.isabsorbing)
		user.text2tab("<span class='warning'>We are already absorbing!</span>")
		return

	if(!user.pulling || !iscarbon(user.pulling))
		user.text2tab("<span class='warning'>We must be grabbing a creature to absorb them!</span>")
		return
	if(user.grab_state <= GRAB_NECK)
		user.text2tab("<span class='warning'>We must have a tighter grip to absorb this creature!</span>")
		return
...
/obj/effect/proc_holder/changeling/absorbDNA/sting_action(mob/user)
	...
	for(var/stage = 1, stage<=3, stage++)
		switch(stage)
			...
			if(3)
				...
				target.text2tab("<span class='userdanger'>You feel a sharp stabbing pain!</span>")
				target.take_overall_damage(40)

		feedback_add_details("changeling_powers","A[stage]")
		if(!do_mob(user, target, 150))
			user.text2tab("<span class='warning'>Our absorption of [target] has been interrupted!</span>")
			changeling.isabsorbing = 0
			return

	user.visible_message("<span class='danger'>[user] sucks the fluids from [target]!</span>", "<span class='notice'>We have absorbed [target].</span>")
	target.text2tab("<span class='userdanger'>You are absorbed by the changeling!</span>")
	...
	if(target.mind)//if the victim has got a mind
		...
		if(recent_speech.len)
			...
			user.text2tab("<span class='boldnotice'>Some of [target]'s speech patterns, we should study these to better impersonate them!</span>")
			for(var/spoken_memory in recent_speech)
				user.mind.store_memory("\"[spoken_memory]\"")
				user.text2tab("<span class='notice'>\"[spoken_memory]\"</span>")
			user.mind.store_memory("<B>We have no more knowledge of [target]'s speech patterns.</B>")
			user.text2tab("<span class='boldnotice'>We have no more knowledge of [target]'s speech patterns.</span>")
...
/obj/effect/proc_holder/changeling/swap_form/can_sting(mob/living/carbon/user)
	...
	if(!user.pulling || !iscarbon(user.pulling) || user.grab_state < GRAB_AGGRESSIVE)
		user.text2tab("<span class='warning'>We must have an aggressive grab on creature to do this!</span>")
		return
	var/mob/living/carbon/target = user.pulling
	if((target.disabilities & NOCLONE) || (target.disabilities & HUSK))
		...
		return
	if(!ishuman(target))
		user.text2tab("<span class='warning'>[target] is not compatible with this ability.</span>")
		return
	return 1
...
/obj/effect/proc_holder/changeling/swap_form/sting_action(mob/living/carbon/user)
	...
	user.text2tab("<span class='notice'>We tighen our grip. We must hold still....</span>")
	target.do_jitter_animation(500)
	user.do_jitter_animation(500)

	if(!do_mob(user,target,20))
		user.text2tab("<span class='warning'>The body swap has been interrupted!</span>")
		return

	target.text2tab("<span class='userdanger'>[user] tightens their grip as a painful sensation invades your body.</span>")
	...
	user.Paralyse(2)
	target.text2tab("<span class='warning'>Our genes cry out as we swap our [user] form for [target].</span>")
