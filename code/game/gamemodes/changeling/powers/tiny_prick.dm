...
/obj/effect/proc_holder/changeling/sting/proc/set_sting(mob/user)
	user.text2tab("<span class='notice'>We prepare our sting, use alt+click or middle mouse button on target to sting them.</span>")
...
/obj/effect/proc_holder/changeling/sting/proc/unset_sting(mob/user)
	user.text2tab("<span class='warning'>We retract our sting, we can't sting anyone for now.</span>")
...
/obj/effect/proc_holder/changeling/sting/can_sting(mob/user, mob/target)
	if(!..())
		return
	if(!user.mind.changeling.chosen_sting)
		user.text2tab("We haven't prepared our sting yet!")
	if(!iscarbon(target))
		return
	if(!isturf(user.loc))
		return
...
/obj/effect/proc_holder/changeling/sting/sting_feedback(mob/user, mob/target)
	if(!target)
		return
	user.text2tab("<span class='notice'>We stealthily sting [target.name].</span>")
	if(target.mind && target.mind.changeling)
		target.text2tab("<span class='warning'>You feel a tiny prick.</span>")
	return 1
...
/obj/effect/proc_holder/changeling/sting/transformation/Click()
	...
	if(NOTRANSSTING in selected_dna.dna.species.specflags)
		user.text2tab("<span class = 'notice'>That DNA is not compatible with changeling retrovirus!")
		return
	..()
...
/obj/effect/proc_holder/changeling/sting/transformation/can_sting(mob/user, mob/target)
	...
	if((target.disabilities & HUSK) || !target.has_dna())
		user.text2tab("<span class='warning'>Our sting appears ineffective against its DNA.</span>")
		return 0
	return 1
...
/obj/effect/proc_holder/changeling/sting/transformation/sting_action(mob/user, mob/target)
	...
	if(ismonkey(target))
		user.text2tab("<span class='notice'>Our genes cry out as we sting [target.name]!</span>")
...
/obj/effect/proc_holder/changeling/sting/false_armblade/sting_action(mob/user, mob/target)
	...
	if(!target.drop_item())
		user.text2tab("<span class='warning'>The [target.get_active_hand()] is stuck to their hand, you cannot grow a false armblade over it!</span>")
		return
...
/obj/effect/proc_holder/changeling/sting/blind/sting_action(mob/user, mob/living/carbon/target)
	...
	target.text2tab("<span class='danger'>Your eyes burn horrifically!</span>")
