...
/obj/effect/proc_holder/changeling/fakedeath/sting_action(mob/living/user)
	user.text2tab("<span class='notice'>We begin our stasis, preparing energy to arise once more.</span>")
	...
	spawn(LING_FAKEDEATH_TIME)
		if(user && user.mind && user.mind.changeling && user.mind.changeling.purchasedpowers)
			user.text2tab("<span class='notice'>We are ready to regenerate.</span>")
...
/obj/effect/proc_holder/changeling/fakedeath/can_sting(mob/user)
	if(user.status_flags & FAKEDEATH)
		user.text2tab("<span class='warning'>We are already regenerating.</span>")
		return
