...
/obj/effect/proc_holder/changeling/strained_muscles/sting_action(mob/living/carbon/user)
	active = !active
	if(active)
		user.text2tab("<span class='notice'>Our muscles tense and strengthen.</span>")
	else
		user.status_flags -= GOTTAGOFAST
		user.text2tab("<span class='notice'>Our muscles relax.</span>")
		if(stacks >= 10)
			user.text2tab("<span class='danger'>We collapse in exhaustion.</span>")
			user.Weaken(3)
			user.emote("gasp")
	...
	while(active)
		...
		if(user.stat != CONSCIOUS || user.staminaloss >= 90)
			active = !active
			user.text2tab("<span class='notice'>Our muscles relax without the energy to strengthen them.</span>")
			user.Weaken(2)
			user.status_flags -= GOTTAGOFAST
			break
		...
		if(stacks == 11) //Warning message that the stacks are getting too high
			user.text2tab("<span class='warning'>Our legs are really starting to hurt...</span>")
