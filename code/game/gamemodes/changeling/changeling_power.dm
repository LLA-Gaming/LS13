...
/obj/effect/proc_holder/changeling/proc/can_sting(mob/user, mob/target)
	...
	if(req_human && !ishuman(user))
		user.text2tab("<span class='warning'>We cannot do that in this form!</span>")
		return 0
	var/datum/changeling/c = user.mind.changeling
	if(c.chem_charges<chemical_cost)
		user.text2tab("<span class='warning'>We require at least [chemical_cost] unit\s of chemicals to do that!</span>")
		return 0
	if(c.absorbedcount<req_dna)
		user.text2tab("<span class='warning'>We require at least [req_dna] sample\s of compatible DNA.</span>")
		return 0
	if(req_stat < user.stat)
		user.text2tab("<span class='warning'>We are incapacitated.</span>")
		return 0
	if((user.status_flags & FAKEDEATH) && name != "Regenerate")
		user.text2tab("<span class='warning'>We are incapacitated.</span>")
		return 0
	if(c.geneticdamage > max_genetic_damage)
		user.text2tab("<span class='warning'>Our genomes are still reassembling. We need time to recover first.</span>")
		return 0
	return 1
