/datum/changeling/proc/purchasePower(mob/living/carbon/user, sting_name)
	...
	if(thepower == null)
		user.text2tab("This is awkward. Changeling power purchase failed, please report this bug to a coder!")
		return

	if(absorbedcount < thepower.req_dna)
		user.text2tab("We lack the energy to evolve this ability!")
		return

	if(has_sting(thepower))
		user.text2tab("We have already evolved this ability!")
		return

	if(thepower.dna_cost < 0)
		user.text2tab("We cannot evolve this ability.")
		return

	if(geneticpoints < thepower.dna_cost)
		user.text2tab("We have reached our capacity for abilities.")
		return
	...
	if(user.status_flags & FAKEDEATH)//To avoid potential exploits by buying new powers while in stasis, which clears your verblist.
		user.text2tab("We lack the energy to evolve new abilities right now.")
		return
...
/datum/changeling/proc/lingRespec(mob/user)
	if(!ishuman(user))
		user.text2tab("<span class='danger'>We can't remove our evolutions in this form!</span>")
		return
	if(canrespec)
		user.text2tab("<span class='notice'>We have removed our evolutions from this form, and are now ready to readapt.</span>")
		user.remove_changeling_powers(1)
		canrespec = 0
		user.make_changeling()
		return 1
	else
		user.text2tab("<span class='danger'>You lack the power to readapt your evolutions!</span>")
		return 0
