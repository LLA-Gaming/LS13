...
/obj/item/device/gangtool/Topic(href, href_list)
	...
	if(href_list["purchase"])
		...
		switch(href_list["purchase"])
			...
			if("scroll")
				if(gang.points >= 30)
					...
					usr.text2tab("<span class='notice'>Anyone who reads the <b>sleeping carp scroll</b> will learn secrets of the sleeping carp martial arts style.</span>")
					pointcost = 30
			if("wrestlingbelt")
				if(gang.points >= 20)
					item_type = /obj/item/weapon/storage/belt/champion/wrestling
					usr.text2tab("<span class='notice'>Anyone wearing the <b>wresting belt</b> will know how to be effective with wrestling.</span>")
					pointcost = 20
			if("bostaff")
				if(gang.points >= 10)
			...
			if("pen")
				if((gang.points >= 50) || free_pen)
					...
					usr.text2tab("<span class='notice'>More <b>recruitment pens</b> will allow you to recruit gangsters faster. Only gang leaders can recruit with pens.</span>")
					if(free_pen)
						free_pen = 0
					else
						pointcost = 50
			if("implant")
				if(gang.points >= 15)
					...
					usr.text2tab("<span class='notice'>The <b>implant breaker</b> is a single-use device that destroys all implants within the target before trying to recruit them to your gang. Also works on enemy gangsters.</span>")
					pointcost = 15
			if("gangtool")
				if(gang.points >= 10)
					if(usr.mind == gang.bosses[1])
						...
						if(gang.bosses.len < 3)
							usr.text2tab("<span class='notice'><b>Gangtools</b> allow you to promote a gangster to be your Lieutenant, enabling them to recruit and purchase items like you. Simply have them register the gangtool. You may promote up to [3-gang.bosses.len] more Lieutenants</span>")
					else
			...
			if("dominator")
				...
				if(initial(usrarea.name) == "Space" || istype(usrturf,/turf/open/space) || usr.z != 1)
					usr.text2tab("<span class='warning'>You can only use this on the station!</span>")
					return

				for(var/obj/obj in usrturf)
					if(obj.density)
						usr.text2tab("<span class='warning'>There's not enough room here!</span>")
						return

				if(usrarea.type in gang.territory|gang.territory_new)
					if(gang.points >= 30)
						item_type = /obj/machinery/dominator
						usr.text2tab("<span class='notice'>The <b>dominator</b> will secure your gang's dominance over the station. Turn it on when you are ready to defend it.</span>")
						pointcost = 30
				else
					usr.text2tab("<span class='warning'>The <b>dominator</b> can be spawned only on territory controlled by your gang!</span>")
					return
		...
		if(item_type)
			...
			usr.text2tab("<span class='warning'>Not enough influence.</span>")
			...
			if("outfit")
				if(outfits > 0)
					if(gang.gang_outfit(usr,src))
						usr.text2tab("<span class='notice'><b>Gang Outfits</b> can act as armor with moderate protection against ballistic and melee attacks. Every gangster wearing one will also help grow your gang's influence.</span>")
						outfits -= 1
			if("ping")
				ping_gang(usr)
	attack_self(usr)
...
/obj/item/device/gangtool/proc/ping_gang(mob/user)
	...
	if(!message || !can_use(user))
		return
	if(user.z > 2)
		user.text2tab("<span class='info'>\icon[src]Error: Station out of range.</span>")
		return
	...
	if(members.len)
		...
		for(var/datum/mind/ganger in members)
			if(ganger.current && (ganger.current.z <= 2) && (ganger.current.stat == CONSCIOUS))
				ganger.current.text2tab(ping)
		for(var/mob/M in dead_mob_list)
			var/link = FOLLOW_LINK(M, user)
			M.text2tab("[link] [ping]")
...
/obj/item/device/gangtool/proc/register_device(mob/user)
	...
	if((promotable && (user.mind in ticker.mode.get_gangsters())) || (user.mind in ticker.mode.get_gang_bosses()))
		...
		if(!(user.mind in gang.bosses))
			...
			user.text2tab("<FONT size=3 color=red><B>You have been promoted to Lieutenant!</B></FONT>")
			ticker.mode.forge_gang_objectives(user.mind)
			ticker.mode.greet_gang(user.mind,0)
			user.text2tab("The <b>Gangtool</b> you registered will allow you to purchase weapons and equipment, and send messages to your gang.")
			user.text2tab("Unlike regular gangsters, you may use <b>recruitment pens</b> to add recruits to your gang. Use them on unsuspecting crew members to recruit them. Don't forget to get your one free pen from the gangtool.")
	else
		usr.text2tab("<span class='warning'>ACCESS DENIED: Unauthorized user.</span>")
...
/obj/item/device/gangtool/proc/recall(mob/user)
	...
	if(recalling)
		usr.text2tab("<span class='warning'>Error: Recall already in progress.</span>")
		return 0

	gang.message_gangtools("[usr] is attempting to recall the emergency shuttle.")
	recalling = 1
	if(ismob(loc))
		loc:text2tab("<span class='info'>\icon[src]Generating shuttle recall order with codes retrieved from last call signal...</span>")

	sleep(rand(100,300))

	if(SSshuttle.emergency.mode != SHUTTLE_CALL) //Shuttle can only be recalled when it's moving to the station
		user.text2tab("<span class='warning'>\icon[src]Emergency shuttle cannot be recalled at this time.</span>")
		recalling = 0
		return 0
	if(ismob(loc))
		loc:text2tab("<span class='info'>\icon[src]Shuttle recall order generated. Accessing station long-range communication arrays...</span>")

	sleep(rand(100,300))

	if(!gang.dom_attempts)
		user.text2tab("<span class='warning'>\icon[src]Error: Unable to access communication arrays. Firewall has logged our signature and is blocking all further attempts.</span>")
		recalling = 0
		return 0

	var/turf/userturf = get_turf(user)
	if(userturf.z != 1) //Shuttle can only be recalled while on station
		user.text2tab("<span class='warning'>\icon[src]Error: Device out of range of station communication arrays.</span>")
		recalling = 0
		return 0
	var/datum/station_state/end_state = new /datum/station_state()
	end_state.count()
	if((100 *  start_state.score(end_state)) < 80) //Shuttle cannot be recalled if the station is too damaged
		user.text2tab("<span class='warning'>\icon[src]Error: Station communication systems compromised. Unable to establish connection.</span>")
		recalling = 0
		return 0
	if(ismob(loc))
		loc:text2tab("<span class='info'>\icon[src]Comm arrays accessed. Broadcasting recall signal...</span>")
	...
	if(userturf.z == 1) //Check one more time that they are on station.
		if(SSshuttle.cancelEvac(user))
			return 1
	if(ismob(loc))
		loc:text2tab("<span class='info'>\icon[src]No response recieved. Emergency shuttle cannot be recalled at this time.</span>")
	return 0
