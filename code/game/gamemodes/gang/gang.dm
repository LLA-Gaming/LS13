...
/datum/game_mode/gang
	...
	restricted_jobs = list("Security Officer", "Warden", "Detective", "AI", "Cyborg","Station Chief", "Human Resources", "Head of Security", "Head Atmospheric Engineer", "Research Director", "Chief Medical Officer")
	required_players = 18
	required_enemies = 2
...
/datum/game_mode/proc/greet_gang(datum/mind/boss_mind, you_are=1)
	var/obj_count = 1
	if (you_are)
		boss_mind.current.text2tab("<FONT size=3 color=red><B>You are the Boss of the [boss_mind.gang_datum.name] Gang!</B></FONT>")
	for(var/datum/objective/objective in boss_mind.objectives)
		boss_mind.current.text2tab("<B>Objective #[obj_count]</B>: [objective.explanation_text]")
		obj_count++
...
/datum/game_mode/proc/equip_gang(mob/living/carbon/human/mob, gang)
	...
	if (!where)
		mob.text2tab("Your Syndicate benefactors were unfortunately unable to get you a Gangtool.")
		. += 1
	else
		gangtool.register_device(mob)
		mob.text2tab("The <b>Gangtool</b> in your [where] will allow you to purchase weapons and equipment, send messages to your gang, and recall the emergency shuttle from anywhere on the station.")
		mob.text2tab("As the gang boss, you can also promote your gang members to <b>lieutenant</b>. Unlike regular gangsters, Lieutenants cannot be deconverted and are able to use recruitment pens and gangtools.")

	var/where2 = mob.equip_in_one_of_slots(T, slots)
	if (!where2)
		mob.text2tab("Your Syndicate benefactors were unfortunately unable to get you a recruitment pen to start.")
		. += 1
	else
		mob.text2tab("The <b>recruitment pen</b> in your [where2] will help you get your gang started. Stab unsuspecting crew members with it to recruit them.")

	var/where3 = mob.equip_in_one_of_slots(SC, slots)
	if (!where3)
		mob.text2tab("Your Syndicate benefactors were unfortunately unable to get you a territory spraycan to start.")
		. += 1
	else
		mob.text2tab("The <b>territory spraycan</b> in your [where3] can be used to claim areas of the station for your gang. The more territory your gang controls, the more influence you get. All gangsters can use these, so distribute them to grow your influence faster.")

	var/where4 = mob.equip_in_one_of_slots(C, slots)
	if (!where4)
		mob.text2tab("Your Syndicate benefactors were unfortunately unable to get you a chameleon security HUD.")
		. += 1
	else
		mob.text2tab("The <b>chameleon security HUD</b> in your [where4] will help you keep track of who is mindshield-implanted, and unable to be recruited.")
	mob.update_icons()
	return .
...
/datum/game_mode/proc/add_gangster(datum/mind/gangster_mind, datum/gang/G, check = 1)
	...
	gangster_mind.current.text2tab("<FONT size=3 color=red><B>You are now a member of the [G.name] Gang!</B></FONT>")
	gangster_mind.current.text2tab("<font color='red'>Help your bosses take over the station by claiming territory with <b>special spraycans</b> only they can provide. Simply spray on any unclaimed area of the station.</font>")
	gangster_mind.current.text2tab("<font color='red'>Their ultimate objective is to take over the station with a Dominator machine.</font>")
	gangster_mind.current.text2tab("<font color='red'>You can identify your bosses by their <b>red \[G\] icon</b>.</font>")
...
/datum/game_mode/proc/remove_gangster(datum/mind/gangster_mind, beingborged, silent, remove_bosses=0)
	...
	if(silent < 2)
		...
		if(beingborged)
			...
			gangster_mind.current.text2tab("<FONT size=3 color=red><B>The frame's firmware detects and deletes your criminal behavior! You are no longer a gangster!</B></FONT>")
			...
			gangster_mind.current.text2tab("<FONT size=3 color=red><B>You have been reformed! You are no longer a gangster!</B><BR>You try as hard as you can, but you can't seem to recall any of the identities of your former gangsters...</FONT>")
