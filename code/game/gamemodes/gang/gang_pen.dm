...
/obj/item/weapon/pen/gang/attack(mob/living/M, mob/user)
	...
	if(ishuman(M) && ishuman(user) && M.stat != DEAD)
		if(user.mind && (user.mind in ticker.mode.get_gang_bosses()))
			if(..(M,user,1))
				if(cooldown)
					user.text2tab("<span class='warning'>[src] needs more time to recharge before it can be used.</span>")
					return
				if(M.client)
					...
					switch(recruitable)
						if(2)
							...
							cooldown(G)
						if(1)
							user.text2tab("<span class='warning'>This mind is resistant to recruitment!</span>")
						else
							user.text2tab("<span class='warning'>This mind has already been recruited into a gang!</span>")
			return
	..()
...
/obj/item/weapon/pen/gang/proc/cooldown(datum/gang/gang)
	...
	spawn(cooldown_time)
		...
		M.text2tab("<span class='notice'>\icon[src] [src][(src.loc == M)?(""):(" in your [src.loc]")] vibrates softly. It is ready to be used again.</span>")
