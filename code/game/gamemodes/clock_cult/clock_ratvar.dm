...
/obj/structure/clockwork/massive/celestial_gateway/New()
	...
	for(var/mob/M in mob_list)
		if(is_servant_of_ratvar(M) || isobserver(M))
			M.text2tab("<span class='large_brass'><b>A gateway to the Celestial Derelict has been created in [gate_area.map_name]!</b></span>")
...
/obj/structure/clockwork/massive/celestial_gateway/Destroy()
	...
	if(!purpose_fulfilled)
		...
		for(var/mob/M in mob_list)
			if(is_servant_of_ratvar(M) || isobserver(M))
				M.text2tab("<span class='large_brass'><b>A gateway to the Celestial Derelict has fallen at [gate_area.map_name]!</b></span>")
...
/obj/structure/clockwork/massive/ratvar/proc/clash_of_the_titans(obj/singularity/narsie/narsie)
	...
	switch(winner)
		if("Ratvar")
			text2world("<span class='heavy_brass'><font size=5>\"[pick("DIE! DIE! DIE!", "REEEEEEEEE!", "FILTH!!!", "SUFFER!!!", "EBG SBE PRAGHEVRF NF V UNIR!!")]\"</font></span>") //nar-sie get out
