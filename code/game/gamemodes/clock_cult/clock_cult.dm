...
/proc/is_eligible_servant(mob/living/M)
	...
	if(!M.mind)
		return 0
	if(ishuman(M) && (M.mind.assigned_role in list("Station Manager", "Chaplain")))
		return 0
	if(iscultist(M))
		return 0
	if(isconstruct(M))
...
/datum/game_mode/clockwork_cult
	...
	protected_jobs = list("AI", "Cyborg", "Security Officer", "Warden", "Detective", "Head of Security", "Station Manager") //Silicons can eventually be converted
	restricted_jobs = list("Chaplain", "Station Manager")
