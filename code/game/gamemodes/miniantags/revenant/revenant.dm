...
/mob/living/simple_animal/revenant/New()
	...
	spawn(5)
		if(src.mind)
			...
			src.text2tab("<br>")
			src.text2tab("<span class='deadsay'><font size=3><b>You are a revenant.</b></font></span>")
			src.text2tab("<b>Your formerly mundane spirit has been infused with alien energies and empowered into a revenant.</b>")
			src.text2tab("<b>You are not dead, not alive, but somewhere in between. You are capable of limited interaction with both worlds.</b>")
			src.text2tab("<b>You are invincible and invisible to everyone but other ghosts. Most abilities will reveal you, rendering you vulnerable.</b>")
			src.text2tab("<b>To function, you are to drain the life essence from humans. This essence is a resource, as well as your health, and will power all of your abilities.</b>")
			src.text2tab("<b><i>You do not remember anything of your past lives, nor will you remember anything about this one after your death.</i></b>")
			src.text2tab("<b>Be sure to read the wiki page at https://tgstation13.org/wiki/Revenant to learn more.</b>")
			var/datum/objective/revenant/objective = new
			objective.owner = src.mind
			src.mind.objectives += objective
			src.text2tab("<b>Objective #1</b>: [objective.explanation_text]")
			var/datum/objective/revenantFluff/objective2 = new
			objective2.owner = src.mind
			src.mind.objectives += objective2
			src.text2tab("<b>Objective #2</b>: [objective2.explanation_text]")
...
/mob/living/simple_animal/revenant/Life()
	...
	if(unreveal_time && world.time >= unreveal_time)
		...
		incorporeal_move = 3
		invisibility = INVISIBILITY_REVENANT
		src.text2tab("<span class='revenboldnotice'>You are once more concealed.</span>")
	if(unstun_time && world.time >= unstun_time)
		unstun_time = 0
		notransform = 0
		src.text2tab("<span class='revenboldnotice'>You can move again!</span>")
...
/mob/living/simple_animal/revenant/adjustHealth(amount)
	...
	update_health_hud()
	if(essence == 0)
		src.text2tab("<span class='revendanger'>You feel your essence fraying!</span>")
...
/mob/living/simple_animal/revenant/death()
	...
	ghost_darkness_images -= ghostimage
	updateallghostimages()
	src.text2tab("<span class='revendanger'>NO! No... it's too late, you can feel your essence [pick("breaking apart", "drifting away")]...</span>")
	notransform = 1
	revealed = 1
	invisibility = 0
...
/mob/living/simple_animal/revenant/proc/reveal(time)
	...
	incorporeal_move = 0
	if(!unreveal_time)
		src.text2tab("<span class='revendanger'>You have been revealed!</span>")
		unreveal_time = world.time + time
	else
		src.text2tab("<span class='revenwarning'>You have been revealed!</span>")
...
/mob/living/simple_animal/revenant/proc/stun(time)
	...
	notransform = 1
	if(!unstun_time)
		src.text2tab("<span class='revendanger'>You cannot move!</span>")
		unstun_time = world.time + time
	else
		src.text2tab("<span class='revenwarning'>You cannot move!</span>")
...
/mob/living/simple_animal/revenant/proc/castcheck(essence_cost)
	...
	if(istype(T, /turf/closed))
		src.text2tab("<span class='revenwarning'>You cannot use abilities from inside of a wall.</span>")
		return 0
	for(var/obj/O in T)
		if(O.density && !O.CanPass(src, T, 5))
			src.text2tab("<span class='revenwarning'>You cannot use abilities inside of a dense object.</span>")
			return 0
	if(src.inhibited)
		src.text2tab("<span class='revenwarning'>Your powers have been suppressed by nulling energy!</span>")
		return 0
	if(!src.change_essence_amount(essence_cost, 1))
		src.text2tab("<span class='revenwarning'>You lack the essence to use that ability.</span>")
		return 0
	return 1
...
/mob/living/simple_animal/revenant/proc/change_essence_amount(essence_amt, silent = 0, source = null)
	...
	if(!silent)
		if(essence_amt > 0)
			src.text2tab("<span class='revennotice'>Gained [essence_amt]E from [source].</span>")
		else
			src.text2tab("<span class='revenminor'>Lost [essence_amt]E from [source].</span>")
	return 1
...
/obj/item/weapon/ectoplasm/revenant/examine(mob/user)
	..()
	if(inert)
		user.text2tab("<span class='revennotice'>It seems inert.</span>")
	else if(reforming)
		user.text2tab("<span class='revenwarning'>It is shifting and distorted. It would be wise to destroy this.</span>")
