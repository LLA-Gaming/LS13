...
/obj/machinery/abductor/console/attack_hand(mob/user)
	if(..())
		return
	if(!isabductor(user))
		user.text2tab("<span class='warning'>You start mashing alien buttons at random!</span>")
...
/obj/machinery/abductor/console/proc/SetDroppoint(turf/open/location,mob/user)
	if(!istype(location))
		user.text2tab("<span class='warning'>That place is not safe for the specimen.</span>")
		return

	if(pad)
		pad.teleport_target = location
		user.text2tab("<span class='notice'>Location marked as test subject release point.</span>")
...
/obj/machinery/abductor/console/attackby(obj/O, mob/user, params)
	if(istype(O, /obj/item/device/abductor/gizmo))
		...
		user.text2tab("<span class='notice'>You link the tool to the console.</span>")
		gizmo = G
		G.console = src
	else if(istype(O, /obj/item/clothing/suit/armor/abductor/vest))
		var/obj/item/clothing/suit/armor/abductor/vest/V = O
		user.text2tab("<span class='notice'>You link the vest to the console.</span>")
		vest = V
	else
		return ..()
