...
/obj/machinery/abductor/experiment/proc/Experiment(mob/occupant,type)
	...
	if(H.mind != null && H.ckey != null)
		...
		switch(text2num(type))
			if(1)
				H.text2tab("<span class='warning'>You feel violated.</span>")
			if(2)
				H.text2tab("<span class='warning'>You feel yourself being sliced apart and put back together.</span>")
			if(3)
				H.text2tab("<span class='warning'>You feel intensely watched.</span>")
		sleep(5)
		H.text2tab("<span class='warning'><b>Your mind snaps!</b></span>")
		var/objtype = pick(subtypesof(/datum/objective/abductee/))
		var/datum/objective/abductee/O = new objtype()
		ticker.mode.abductees += H.mind
		H.mind.objectives += O
		var/obj_count = 1
		H.text2tab("<span class='notice'>Your current objectives:</span>")
		for(var/datum/objective/objective in H.mind.objectives)
			H.text2tab("<B>Objective #[obj_count]</B>: [objective.explanation_text]")
			obj_count++
