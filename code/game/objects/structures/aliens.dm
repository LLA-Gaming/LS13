...
/obj/structure/alien/egg/attack_hand(mob/living/user)
	if(user.getorgan(/obj/item/organ/alien/plasmavessel))
		switch(status)
			if(BURST)
				user.text2tab("<span class='notice'>You clear the hatched egg.</span>")
				playsound(loc, 'sound/effects/attackblob.ogg', 100, 1)
				qdel(src)
				return
			if(GROWING)
				user.text2tab("<span class='notice'>The child is not developed yet.</span>")
				return
			if(GROWN)
				user.text2tab("<span class='notice'>You retrieve the child.</span>")
				Burst(0)
				return
	else
		user.text2tab("<span class='notice'>It feels slimy.</span>")
		user.changeNext_move(CLICK_CD_MELEE)
