...
/obj/structure/janitorialcart/proc/wet_mop(obj/item/weapon/mop, mob/user)
	if(reagents.total_volume < 1)
		...
		user.text2tab("<span class='notice'>You wet [mop] in [src].</span>")
...
/obj/structure/janitorialcart/proc/put_in_cart(obj/item/I, mob/user)
	...
	I.loc = src
	updateUsrDialog()
	user.text2tab("<span class='notice'>You put [I] into [src].</span>")
	return
...
/obj/structure/janitorialcart/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/weapon/mop))
		...
		if(signs < max_signs)
			...
			user.text2tab("<span class='warning'>[src] can't hold any more signs!</span>")
	else if(mybag)
		...
		if(do_after(user, 30/I.toolspeed, target = src))
			usr.text2tab("<span class='notice'>You empty the contents of [src]'s bucket onto the floor.</span>")
...
/obj/structure/janitorialcart/Topic(href, href_list)
	...
	if(href_list["garbage"])
		if(mybag)
			...
			user.text2tab("<span class='notice'>You take [mybag] from [src].</span>")
			mybag = null
	if(href_list["mop"])
		if(mymop)
			user.put_in_hands(mymop)
			user.text2tab("<span class='notice'>You take [mymop] from [src].</span>")
			mymop = null
	if(href_list["spray"])
		if(myspray)
			user.put_in_hands(myspray)
			user.text2tab("<span class='notice'>You take [myspray] from [src].</span>")
			myspray = null
	if(href_list["replacer"])
		if(myreplacer)
			user.put_in_hands(myreplacer)
			user.text2tab("<span class='notice'>You take [myreplacer] from [src].</span>")
			myreplacer = null
	if(href_list["sign"])
		if(signs)
			...
			if(Sign)
				...
				user.text2tab("<span class='notice'>You take \a [Sign] from [src].</span>")
				signs--
			else
