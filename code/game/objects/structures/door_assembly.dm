...
/obj/structure/door_assembly/attackby(obj/item/W, mob/user, params)
	...
	//	or you will get broken icons.
		...
		if(WT.can_use(user))
			...
			user.text2tab("<span class='notice'>You change the paintjob on the airlock assembly.</span>")
		...
		if(!anchored )
			...
			if(door_check)
				...
				if(do_after(user, 40/W.toolspeed, target = src))
					if( src.anchored )
						return
					user.text2tab("<span class='notice'>You secure the airlock assembly.</span>")
					src.name = "secured airlock assembly"
					src.anchored = 1
			else
				user.text2tab("There is another door here!")
			...
			if(do_after(user, 40/W.toolspeed, target = src))
				if(!anchored )
					return
				user.text2tab("<span class='notice'>You unsecure the airlock assembly.</span>")
				name = "airlock assembly"
				anchored = 0
		...
		if (C.get_amount() < 1)
			user.text2tab("<span class='warning'>You need one length of cable to wire the airlock assembly!</span>")
			return
		...
		if(do_after(user, 40, target = src))
			...
			C.use(1)
			src.state = 1
			user.text2tab("<span class='notice'>You wire the airlock assembly.</span>")
		...
		if(do_after(user, 40/W.toolspeed, target = src))
			if( src.state != 1 )
				return
			user.text2tab("<span class='notice'>You cut the wires from the airlock assembly.</span>")
				...
				if(istype(G, /obj/item/stack/sheet/rglass) || istype(G, /obj/item/stack/sheet/glass))
					...
					if(do_after(user, 40, target = src))
						...
						if (G.type == /obj/item/stack/sheet/rglass)
							user.text2tab("<span class='notice'>You install reinforced glass windows into the airlock assembly.</span>")
							...
							user.text2tab("<span class='notice'>You install regular glass windows into the airlock assembly.</span>")
					...
					if(G.get_amount() >= 2)
						...
						if(do_after(user, 40, target = src))
							...
							user.text2tab("<span class='notice'>You install [M] plating into the airlock assembly.</span>")
							G.use(2)
							mineral = "[M]"
		...
		if(do_after(user, 40/W.toolspeed, target = src))
			if(src.loc && state == 2)
				user.text2tab("<span class='notice'>You finish the airlock.</span>")
