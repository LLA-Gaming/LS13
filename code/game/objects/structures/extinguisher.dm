...
/obj/structure/extinguisher_cabinet/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/wrench) && !has_extinguisher)
		user.text2tab("<span class='notice'>You start unsecuring [name]...</span>")
		playsound(loc, 'sound/items/Ratchet.ogg', 50, 1)
		if(do_after(user, 60/I.toolspeed, target = src))
			playsound(loc, 'sound/items/Deconstruct.ogg', 50, 1)
			user.text2tab("<span class='notice'>You unsecure [name].</span>")
	...
	if(istype(I, /obj/item/weapon/extinguisher))
		if(!has_extinguisher && opened)
			...
			contents += I
			has_extinguisher = I
			user.text2tab("<span class='notice'>You place [I] in [src].</span>")
		else
			opened = !opened
	else
		opened = !opened
...
/obj/structure/extinguisher_cabinet/attack_hand(mob/user)
	...
	if(has_extinguisher)
		...
		user.text2tab("<span class='notice'>You take [has_extinguisher] from [src].</span>")
...
/obj/structure/extinguisher_cabinet/attack_tk(mob/user)
	if(has_extinguisher)
		...
		user.text2tab("<span class='notice'>You telekinetically remove [has_extinguisher] from [src].</span>")
		has_extinguisher = null
		opened = 1
	else
		opened = !opened
