...
/obj/item/weapon/bedsheet/attack_self(mob/user)
	...
	if(layer == initial(layer))
		...
		user.text2tab("<span class='notice'>You cover yourself with [src].</span>")
	else
		layer = initial(layer)
		user.text2tab("<span class='notice'>You smooth [src] out beneath you.</span>")
...
/obj/item/weapon/bedsheet/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/wirecutters) || I.is_sharp())
		...
		qdel(src)
		user.text2tab("<span class='notice'>You tear [src] up.</span>")
	else
		return ..()
...
/obj/structure/bedsheetbin/examine(mob/user)
	..()
	if(amount < 1)
		user.text2tab("There are no bed sheets in the bin.")
	else if(amount == 1)
		user.text2tab("There is one bed sheet in the bin.")
	else
		user.text2tab("There are [amount] bed sheets in the bin.")
...
/obj/structure/bedsheetbin/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/bedsheet))
		...
		if(!user.drop_item())
			user.text2tab("<span class='warning'>\The [I] is stuck to your hand, you cannot hide it among the sheets!</span>")
			return
		I.loc = src
		hidden = I
		user.text2tab("<span class='notice'>You hide [I] among the sheets.</span>")
...
/obj/structure/bedsheetbin/attack_hand(mob/user)
	...
	if(amount >= 1)
		...
		B.loc = user.loc
		user.put_in_hands(B)
		user.text2tab("<span class='notice'>You take [B] out of [src].</span>")
		update_icon()

		if(hidden)
			hidden.loc = user.loc
			user.text2tab("<span class='notice'>[hidden] falls out of [B]!</span>")
			hidden = null
...
/obj/structure/bedsheetbin/attack_tk(mob/user)
	if(amount >= 1)
		...
		B.loc = loc
		user.text2tab("<span class='notice'>You telekinetically remove [B] from [src].</span>")
		update_icon()
