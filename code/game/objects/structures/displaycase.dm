...
/obj/structure/displaycase/examine(mob/user)
	..()
	if(showpiece)
		user.text2tab("<span class='notice'>There's [showpiece] inside.</span>")
	if(alert)
		user.text2tab("<span class='notice'>Hooked up with an anti-theft system.</span>")
...
/obj/structure/displaycase/attackby(obj/item/weapon/W, mob/user, params)
	if(W.GetID() && electronics && !destroyed)
		if(allowed(user))
			user.text2tab( "<span class='notice'>You [open ? "close":"open"] the [src]</span>")
			open = !open
			update_icon()
		else
		...
		if(destroyed)
			if(showpiece)
				user.text2tab("<span class='notice'>Remove the displayed object first.</span>")
			else
				user.text2tab("<span class='notice'>You remove the destroyed case</span>")
				qdel(src)
		else
			user.text2tab("<span class='notice'>You start to [open ? "close":"open"] the [src]</span>")
		...
		if(user.drop_item())
			W.loc = src
			showpiece = W
			user.text2tab("<span class='notice'>You put [W] on display</span>")
			update_icon()
	else if(istype(W, /obj/item/stack/sheet/glass) && destroyed)
		var/obj/item/stack/sheet/glass/G = W
		if(G.get_amount() < 2)
			user.text2tab("<span class='warning'>You need two glass sheets to fix the case!</span>")
			return
		user.text2tab("<span class='notice'>You start fixing the [src]...</span>")
...
/obj/structure/displaycase/attack_hand(mob/user)
	...
	if (showpiece && (destroyed || open))
		dump()
		user.text2tab("<span class='notice'>You deactivate the hover field built into the case.</span>")
...
/obj/structure/displaycase_chassis/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/wrench))
		...
		user.text2tab("<span class='notice'>You start installing the electronics into [src]...</span>")
		playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
		...
		if(G.get_amount() < 10)
			user.text2tab("<span class='warning'>You need ten glass sheets to do this!</span>")
			return
		user.text2tab("<span class='notice'>You start adding [G] to [src]...</span>")
		if(do_after(user, 20, target = src))
			G.use(10)
