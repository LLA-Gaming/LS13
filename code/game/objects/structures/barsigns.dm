...
/obj/structure/sign/barsign/attack_hand(mob/user)
	if (!src.allowed(user))
		user.text2tab("<span class='info'>Access denied.</span>")
		return
	if (broken)
		user.text2tab("<span class ='danger'>The controls seem unresponsive.</span>")
		return
	pick_sign()
...
/obj/structure/sign/barsign/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/weapon/screwdriver))
		if(!allowed(user))
			...
			return
		if(!panel_open)
			user.text2tab("<span class='notice'>You open the maintenance panel.</span>")
			set_sign(new /datum/barsign/hiddensigns/signoff)
			panel_open = 1
		else
			user.text2tab("<span class='notice'>You close the maintenance panel.</span>")
			if(!broken && !emagged)
				set_sign(pick(barsigns))
			else if(emagged)
		...
		if(emagged) //Emagged, not broken by EMP
			user.text2tab("<span class='warning'>Sign has been damaged beyond repair!</span>")
			return
		else if(!broken)
			user.text2tab("<span class='warning'>This sign is functioning properly!</span>")
			return

		if(C.use(2))
			user.text2tab("<span class='notice'>You replace the burnt wiring.</span>")
			broken = 0
		else
			user.text2tab("<span class='warning'>You need at least two lengths of cable!</span>")
	else
		return ..()
...
/obj/structure/sign/barsign/emag_act(mob/user)
	if(broken || emagged)
		user.text2tab("<span class='warning'>Nothing interesting happens!</span>")
		return
	user.text2tab("<span class='notice'>You emag the barsign. Takeover in progress...</span>")
