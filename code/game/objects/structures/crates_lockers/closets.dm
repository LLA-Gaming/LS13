...
/obj/structure/closet/examine(mob/user)
	..()
	if(broken)
		user.text2tab("<span class='notice'>It appears to be broken.</span>")
	else if(secure && !opened)
		user.text2tab("<span class='notice'>Alt-click to [locked ? "unlock" : "lock"].</span>")
...
/obj/structure/closet/proc/can_open(mob/living/user)
	...
	for(var/mob/living/L in T)
		if(L.anchored || horizontal && L.mob_size > MOB_SIZE_TINY && L.density)
			if(user)
				user.text2tab("<span class='danger'>There's something large on top of [src], preventing it from opening.</span>") //you... think? there's something standing on it ffs
			return 0
	return 1
...
/obj/structure/closet/proc/can_close(mob/living/user)
	...
	for(var/mob/living/L in T)
		if(L.anchored || horizontal && L.mob_size > MOB_SIZE_TINY && L.density)
			if(user)
				user.text2tab("<span class='danger'>There's something too large in [src], preventing it from closing.</span>")
			return 0
	return 1
...
/obj/structure/closet/attackby(obj/item/weapon/W, mob/user, params)
	...
	if(opened)
		if(istype(W, cutting_tool))
			if(istype(W, /obj/item/weapon/weldingtool))
				...
				if(!WT.remove_fuel(0, user))
					return
				user.text2tab("<span class='notice'>You begin cutting \the [src] apart...</span>")
		...
		if(!WT.remove_fuel(0, user))
			return
		user.text2tab("<span class='notice'>You begin [welded ? "unwelding":"welding"] \the [src]...</span>")
...
/obj/structure/closet/verb/verb_toggleopen()
	...
	if(iscarbon(usr) || issilicon(usr))
		...
		usr.text2tab("<span class='warning'>This mob type can't use this verb.</span>")
...
/obj/structure/closet/container_resist(mob/living/user)
	...
	user.changeNext_move(CLICK_CD_BREAKOUT)
	user.last_special = world.time + CLICK_CD_BREAKOUT
	user.text2tab("<span class='notice'>You lean on the back of [src] and start pushing the door open.</span>")
	...
	if(do_after(user,(breakout_time * 60 * 10), target = src)) //minutes * 60seconds * 10deciseconds
		...
		if(user.loc == src) //so we don't get the message if we resisted multiple times and succeeded.
			user.text2tab("<span class='warning'>You fail to break out of [src]!</span>")
...
/obj/structure/closet/proc/togglelock(mob/living/user)
	if(secure && !broken)
		...
		user.text2tab("<span class='warning'>\The [src] is broken!</span>")
