			...
			if(MUTCOLORS in H.dna.species.specflags)
				...
				if(new_mutantcolor)
					...
					if(ReadHSV(temp_hsv)[3] >= ReadHSV("#7F7F7F")[3]) // mutantcolors must be bright
						...
						H.text2tab("<span class='notice'>Invalid color. Your color is not bright enough.</span>")
		...
		if("gender")
			...
			if(H.gender == "male")
				if(alert(H, "Become a Witch?", "Confirmation", "Yes", "No") == "Yes")
					H.gender = "female"
					H.text2tab("<span class='notice'>Man, you feel like a woman!</span>")
				else
					return
				...
				if(alert(H, "Become a Warlock?", "Confirmation", "Yes", "No") == "Yes")
					H.gender = "male"
					H.text2tab("<span class='notice'>Whoa man, you feel like a man!</span>")
				else
					return
