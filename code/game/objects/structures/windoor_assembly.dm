		...
		if("01")
			if(istype(W, /obj/item/weapon/weldingtool) && !anchored )
				...
				if (WT.remove_fuel(0,user))
					...
					if(do_after(user, 40/W.toolspeed, target = src))
						...
						user.text2tab("<span class='notice'>You disassemble the windoor assembly.</span>")
			...
			if(istype(W, /obj/item/weapon/wrench) && !anchored)
				for(var/obj/machinery/door/window/WD in loc)
					if(WD.dir == dir)
						user.text2tab("<span class='warning'>There is already a windoor in that location!</span>")
						return
				...
				if(do_after(user, 40/W.toolspeed, target = src))
					...
					for(var/obj/machinery/door/window/WD in loc)
						if(WD.dir == dir)
							user.text2tab("<span class='warning'>There is already a windoor in that location!</span>")
							return
					user.text2tab("<span class='notice'>You secure the windoor assembly.</span>")
					anchored = 1
					if(secure)
			...
			//Unwrenching an unsecure assembly un-anchors it. Step 4 undone
				...
				if(do_after(user, 40/W.toolspeed, target = src))
					if(!src || !anchored)
						return
					user.text2tab("<span class='notice'>You unsecure the windoor assembly.</span>")
					anchored = 0
					if(secure)
			...
			//Adding plasteel makes the assembly a secure windoor assembly. Step 2 (optional) complete.
				...
				if(P.amount < 2)
					user.text2tab("<span class='warning'>You need more plasteel to do this!</span>")
					return
				user.text2tab("<span class='notice'>You start to reinforce the windoor with plasteel...</span>")
				...
				if(do_after(user,40, target = src))
					...
					P.use(2)
					user.text2tab("<span class='notice'>You reinforce the windoor.</span>")
					secure = 1
					if(anchored)
						name = "secure anchored windoor assembly"
					else
			...
			//Adding cable to the assembly. Step 5 complete.
				...
				if(do_after(user, 40, target = src))
					...
					if(!CC.use(1))
						user.text2tab("<span class='warning'>You need more cable to do this!</span>")
						return
					user.text2tab("<span class='notice'>You wire the windoor.</span>")
					state = "02"
					if(secure)
		...
		if("02")
			...
			if(istype(W, /obj/item/weapon/wirecutters))
				...
				if(do_after(user, 40/W.toolspeed, target = src))
					...
					user.text2tab("<span class='notice'>You cut the windoor wires.</span>")
			...
			//Adding airlock electronics for access. Step 6 complete.
				...
				if(do_after(user, 40, target = src))
					if(!src || electronics)
						W.loc = src.loc
						return
					user.text2tab("<span class='notice'>You install the airlock electronics.</span>")
			...
			//Crowbar to complete the assembly, Step 7 complete.
				...
				if(!electronics)
					usr.text2tab("<span class='warning'>The assembly is missing electronics!</span>")
					return
				...
				if(do_after(user, 40/W.toolspeed, target = src))
					...
					if(loc && electronics)
						...
						user.text2tab("<span class='notice'>You finish the windoor.</span>")
...
/obj/structure/windoor_assembly/verb/flip()
	...
	if(facing == "l")
		usr.text2tab("<span class='notice'>The windoor will now slide to the right.</span>")
		facing = "r"
	else
		facing = "l"
		usr.text2tab("<span class='notice'>The windoor will now slide to the left.</span>")
