	...
	if(swirlie)
		...
		if(user.grab_state >= GRAB_AGGRESSIVE)
			if(GM.loc != get_turf(src))
				user.text2tab("<span class='warning'>[GM] needs to be on [src]!</span>")
				return
			if(!swirlie)
				if(open)
			...
			user.text2tab("<span class='warning'>You need a tighter grip!</span>")

	else if(cistern && !open)
		if(!contents.len)
			user.text2tab("<span class='notice'>The cistern is empty.</span>")
		else
			...
			if(ishuman(user))
				...
				I.loc = get_turf(src)
			user.text2tab("<span class='notice'>You find [I] in the cistern.</span>")
			w_items -= I.w_class
	else
		open = !open
		update_icon()
...
/obj/structure/toilet/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/weapon/crowbar))
		user.text2tab("<span class='notice'>You start to [cistern ? "replace the lid on the cistern" : "lift the lid off the cistern"]...</span>")
		...
		if(user.a_intent != "harm")
			if(I.w_class > 3)
				user.text2tab("<span class='warning'>[I] does not fit!</span>")
				return
			if(w_items + I.w_class > 5)
				user.text2tab("<span class='warning'>The cistern is full!</span>")
				return
			if(!user.drop_item())
				user.text2tab("<span class='warning'>\The [I] is stuck to your hand, you cannot put it in the cistern!</span>")
				return
			I.loc = src
			w_items += I.w_class
			user.text2tab("<span class='notice'>You carefully place [I] into the cistern.</span>")
		...
		user.text2tab("<span class='notice'>You fill [RG] from [src]. Gross.</span>")
	else
		return ..()
...
/obj/structure/urinal/attack_hand(mob/user)
	if(user.pulling && user.a_intent == "grab" && isliving(user.pulling))
		...
		if(user.grab_state >= GRAB_AGGRESSIVE)
			if(GM.loc != get_turf(src))
				user.text2tab("<span class='notice'>[GM.name] needs to be on [src].</span>")
				return
			user.changeNext_move(CLICK_CD_MELEE)
...
/obj/machinery/shower/attackby(obj/item/I, mob/user, params)
	if(I.type == /obj/item/device/analyzer)
		user.text2tab("<span class='notice'>The water temperature seems to be [watertemp].</span>")
	if(istype(I, /obj/item/weapon/wrench))
		user.text2tab("<span class='notice'>You begin to adjust the temperature valve with \the [I]...</span>")
...
/obj/machinery/shower/proc/check_heat(mob/living/carbon/C)
	if(watertemp == "freezing")
		...
		C.text2tab("<span class='warning'>The water is freezing!</span>")
	else if(watertemp == "boiling")
		C.bodytemperature = min(500, C.bodytemperature + 35)
		C.adjustFireLoss(5)
		C.text2tab("<span class='danger'>The water is searing!</span>")
...
/obj/structure/sink/attack_hand(mob/living/user)
	...
	if(busy)
		user.text2tab("<span class='notice'>Someone's already washing here.</span>")
		return
...
/obj/structure/sink/attackby(obj/item/O, mob/user, params)
	if(busy)
		user.text2tab("<span class='warning'>Someone's already washing here!</span>")
		return
	...
	if(istype(O, /obj/item/weapon/reagent_containers))
		...
		if(RG.flags & OPENCONTAINER)
			RG.reagents.add_reagent("water", min(RG.volume - RG.reagents.total_volume, RG.amount_per_transfer_from_this))
			user.text2tab("<span class='notice'>You fill [RG] from [src].</span>")
			return 1
	...
	if(istype(O, /obj/item/weapon/mop))
		...
		user.text2tab("<span class='notice'>You wet [O] in [src].</span>")
		playsound(loc, 'sound/effects/slosh.ogg', 25, 1)

	if(istype(O, /obj/item/weapon/reagent_containers/food/snacks/monkeycube))
		var/obj/item/weapon/reagent_containers/food/snacks/monkeycube/M = O
		user.text2tab("<span class='notice'>You place [src] under a stream of water...</span>")
		user.drop_item()
		M.loc = get_turf(src)
		M.Expand()
		return
	...
	if(user.a_intent != "harm")
		user.text2tab("<span class='notice'>You start washing [O]...</span>")
		busy = 1
		if(!do_after(user, 40, target = src))
			busy = 0
			return 1
