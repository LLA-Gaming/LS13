...
/obj/structure/window/examine(mob/user)
	..()
	user.text2tab("<span class='notice'>Alt-click to rotate it clockwise.</span>")
	...
	if(!(flags&NODECONSTRUCT))
		if(istype(I, /obj/item/weapon/screwdriver))
			...
			if(reinf && (state == 2 || state == 1))
				user.text2tab((state == 2 ? "<span class='notice'>You begin to unscrew the window from the frame...</span>" : "<span class='notice'>You begin to screw the window to the frame...</span>"))
			else if(reinf && state == 0)
				user.text2tab((anchored ? "<span class='notice'>You begin to unscrew the frame from the floor...</span>" : "<span class='notice'>You begin to screw the frame to the floor...</span>"))
			else if(!reinf)
				user.text2tab((anchored ? "<span class='notice'>You begin to unscrew the window from the floor...</span>" : "<span class='notice'>You begin to screw the window to the floor...</span>"))

			if(do_after(user, 30/I.toolspeed, target = src))
				if(reinf && (state == 1 || state == 2))
					//If state was unfastened, fasten it, else do the reverse
					state = (state == 1 ? 2 : 1)
					user.text2tab((state == 1 ? "<span class='notice'>You unfasten the window from the frame.</span>" : "<span class='notice'>You fasten the window to the frame.</span>"))
				else if(reinf && state == 0)
					anchored = !anchored
					update_nearby_icons()
					user.text2tab((anchored ? "<span class='notice'>You fasten the frame to the floor.</span>" : "<span class='notice'>You unfasten the frame from the floor.</span>"))
				else if(!reinf)
					anchored = !anchored
					update_nearby_icons()
					user.text2tab((anchored ? "<span class='notice'>You fasten the window to the floor.</span>" : "<span class='notice'>You unfasten the window.</span>"))
			return

		else if (istype(I, /obj/item/weapon/crowbar) && reinf && (state == 0 || state == 1))
			user.text2tab((state == 0 ? "<span class='notice'>You begin to lever the window into the frame...</span>" : "<span class='notice'>You begin to lever the window out of the frame...</span>"))
			playsound(loc, 'sound/items/Crowbar.ogg', 75, 1)
			if(do_after(user, 40/I.toolspeed, target = src))
				//If state was out of frame, put into frame, else do the reverse
				state = (state == 0 ? 1 : 0)
				user.text2tab((state == 1 ? "<span class='notice'>You pry the window into the frame.</span>" : "<span class='notice'>You pry the window out of the frame.</span>"))
			return

		else if(istype(I, /obj/item/weapon/wrench) && !anchored)
			playsound(loc, 'sound/items/Ratchet.ogg', 75, 1)
			user.text2tab("<span class='notice'> You begin to disassemble [src]...</span>")
			if(do_after(user, 40/I.toolspeed, target = src))
				if(qdeleted(src))
					return
				...
				user.text2tab("<span class='notice'>You successfully disassemble [src].</span>")
				qdel(src)
			return
	return ..()
...
/obj/structure/window/verb/rotate()
	...
	if(anchored)
		usr.text2tab("<span class='warning'>[src] cannot be rotated while it is fastened to the floor!</span>")
		return 0
