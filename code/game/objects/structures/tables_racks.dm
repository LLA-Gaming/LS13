...
/obj/structure/table/attack_hand(mob/living/user)
	if(user.a_intent == "grab" && user.pulling && isliving(user.pulling))
		...
		if(pushed_mob.buckled)
			user.text2tab("<span class='warning'>[pushed_mob] is buckled to [pushed_mob.buckled]!</span>")
			return
		if(user.grab_state < GRAB_AGGRESSIVE)
			user.text2tab("<span class='warning'>You need a better grip to do that!</span>")
			return
...
/obj/structure/table/proc/table_deconstruct(mob/user, disassembling = 0)
	...
	if(disassembling)
		...
		user.text2tab("<span class='notice'>You start deconstructing [src]...</span>")
	...
	if(istype(W, /obj/item/weapon/weldingtool))
		...
		if(WT.remove_fuel(0, user))
			...
			if(deconstruction_ready)
				user.text2tab("<span class='notice'>You start strengthening the reinforced table...</span>")
				if (do_after(user, 50/W.toolspeed, target = src))
					if(!src || !WT.isOn()) return
					user.text2tab("<span class='notice'>You strengthen the table.</span>")
					deconstruction_ready = 0
			else
				user.text2tab("<span class='notice'>You start weakening the reinforced table...</span>")
				if (do_after(user, 50/W.toolspeed, target = src))
					if(!src || !WT.isOn()) return
					user.text2tab("<span class='notice'>You weaken the table.</span>")
...
/obj/item/weapon/rack_parts/attack_self(mob/user)
	user.text2tab("<span class='notice'>You start constructing rack...</span>")
