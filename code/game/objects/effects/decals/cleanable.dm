...
/obj/effect/decal/cleanable/attackby(obj/item/weapon/W, mob/user, params)
	if(istype(W, /obj/item/weapon/reagent_containers/glass) || istype(W, /obj/item/weapon/reagent_containers/food/drinks))
		if(src.reagents && W.reagents)
			...
			if(!src.reagents.total_volume)
				user.text2tab("<span class='notice'>[src] isn't thick enough to scoop up!</span>")
				return
			if(W.reagents.total_volume >= W.reagents.maximum_volume)
				user.text2tab("<span class='notice'>[W] is full!</span>")
				return
			user.text2tab("<span class='notice'>You scoop up [src] into [W]!</span>")
	...
	if(W.is_hot()) //todo: make heating a reagent holder proc
		if(istype(W, /obj/item/clothing/mask/cigarette))
			...
			user.text2tab("<span class='notice'>You heat [src] with [W]!</span>")
