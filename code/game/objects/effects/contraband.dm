...
/*/obj/item/weapon/contraband/poster/attack(atom/A, mob/user as mob) //This shit is handled through the wall's attackby()
	if(istype(A, /turf/closed/wall))
		if(resulting_poster == null)
			...
			if(check)
				user.text2tab("<span class='notice'>The wall is far too cluttered to place a poster!</span>")
				return
...
/obj/structure/sign/poster/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/wirecutters))
		...
		if(ruined)
			user.text2tab("<span class='notice'>You remove the remnants of the poster.</span>")
			qdel(src)
		else
			user.text2tab("<span class='notice'>You carefully remove the poster from the wall.</span>")
...
/turf/closed/wall/proc/place_poster(obj/item/weapon/poster/P, mob/user)
	...
	for(var/obj/O in contents) //Let's see if it already has a poster on it or too much stuff
		if(istype(O,/obj/structure/sign/poster))
			user.text2tab("<span class='warning'>The wall is far too cluttered to place a poster!</span>")
			return
		stuff_on_wall++
		if(stuff_on_wall == 3)
			user.text2tab("<span class='warning'>The wall is far too cluttered to place a poster!</span>")
	...
	user.text2tab("<span class='notice'>You start placing the poster on the wall...</span>") //Looks like it's uncluttered enough. Place the poster.
	...
	if(do_after(user,D.placespeed,target=src))
		...
		if(istype(src,/turf/closed/wall) && user && user.loc == temp_loc)	//Let's check if everything is still there
			user.text2tab("<span class='notice'>You place the poster!</span>")
		else
