...
/obj/structure/foamedmetal/attack_animal(mob/living/simple_animal/user)
	...
	if(user.environment_smash >= 1)
		user.do_attack_animation(src)
		user.text2tab("<span class='notice'>You smash apart the foam wall.</span>")
		qdel(src)
...
/obj/structure/foamedmetal/attack_hand(mob/user)
	user.changeNext_move(CLICK_CD_MELEE)
	user.do_attack_animation(src)
	user.text2tab("<span class='warning'>You hit the metal foam but bounce off it!</span>")
	playsound(src.loc, 'sound/weapons/tap.ogg', 100, 1)
...
/obj/structure/foamedmetal/attacked_by(obj/item/I, mob/living/user)
	...
	if(prob(I.force*20 - metal*25))
		...
		user.text2tab("<span class='warning'>You hit the metal foam to no effect!</span>")
