...
/atom/movable/proc/buckle_mob(mob/living/M, force = 0)
	...
	if(!M.can_buckle() && !force)
		if(M == usr)
			M.text2tab("<span class='warning'>You are unable to buckle yourself to the [src]!</span>")
		else
			usr.text2tab("<span class='warning'>You are unable to buckle [M] to the [src]!</span>")
		return 0
