...
/obj/item/stack/sheet/glass/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/stack/cable_coil))
		...
		if (get_amount() < 1 || CC.get_amount() < 5)
			user.text2tab("<span class='warning>You need five lengths of coil and one sheet of glass to make wired glass!</span>")
			return
		CC.use(5)
		use(1)
		user.text2tab("<span class='notice'>You attach wire to the [name].</span>")
		...
		if (V.get_amount() >= 1 && src.get_amount() >= 1)
			...
			user.text2tab("<span class='warning'>You need one rod and one sheet of glass to make reinforced glass!</span>")
			return
	else
		return ..()
	...
	switch(alert(title, "Would you like full tile glass or one direction?", "One Direction", "Full Window", "Cancel", null))
		if("One Direction")
			...
			for (var/obj/structure/window/win in user.loc)
				i++
				if(i >= 4)
					user.text2tab("<span class='warning'>There are too many windows in this location.</span>")
					return 1
				directions-=win.dir
				if(!(win.ini_dir in cardinal))
					user.text2tab("<span class='danger'>Can't let you do that.</span>")
					return 1
		...
		if("Full Window")
			...
			if(src.loc != user)
				return 1
			if(src.get_amount() < 2)
				user.text2tab("<span class='warning'>You need more glass to do that!</span>")
				return 1
			if(locate(/obj/structure/window) in user.loc)
				user.text2tab("<span class='warning'>There is a window in the way!</span>")
				return 1
			var/obj/structure/window/W
	...
	switch(input(title, "Would you like full tile glass a one direction glass pane or a windoor?") in list("One Direction", "Full Window", "Windoor", "Cancel"))
		if("One Direction")
			...
			for (var/obj/structure/window/win in user.loc)
				i++
				if(i >= 4)
					user.text2tab("<span class='danger'>There are too many windows in this location.</span>")
					return 1
				directions-=win.dir
				if(!(win.ini_dir in cardinal))
					user.text2tab("<span class='danger'>Can't let you do that.</span>")
		...
		if("Windoor")
			...
			for(var/obj/structure/windoor_assembly/WA in user.loc)
				if(WA.dir == user.dir)
					user.text2tab("<span class='warning'>There is already a windoor assembly in that location!</span>")
					return 1
...
/obj/item/weapon/shard/afterattack(atom/A as mob|obj, mob/user, proximity)
	...
	if(ishuman(user))
		...
		if(!H.gloves && !(PIERCEIMMUNE in H.dna.species.specflags)) // golems, etc
			H.text2tab("<span class='warning'>[src] cuts into your hand!</span>")
		...
		M.text2tab("<span class='warning'>[src] cuts into your hand!</span>")
		...
		if(WT.remove_fuel(0, user))
			...
			for(var/obj/item/stack/sheet/glass/G in user.loc)
				...
				if(G.amount >= G.max_amount)
					continue
				G.attackby(NG, user)
			user.text2tab("<span class='notice'>You add the newly-formed glass to the stack. It now contains [NG.amount] sheet\s.</span>")
			qdel(src)
	else
		return ..()
