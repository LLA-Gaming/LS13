...
/obj/item/nuke_core_container/proc/load(obj/item/nuke_core/ncore, mob/user)
	...
	user.text2tab("<span class='warning'>Container is sealing...</span>")
...
/obj/item/nuke_core_container/proc/seal()
	if(istype(core))
		...
		if(ismob(loc))
			loc:text2tab("<span class='warning'>[src] is permanently sealed, [core]'s radiation is contained.</span>")

/obj/item/nuke_core_container/attackby(obj/item/nuke_core/core, mob/user)
	if(istype(core))
		if(!user.unEquip(core))
			user.text2tab("<span class='warning'>The [core] is stuck to your hand!</span>")
			return
		else
			load(core, user)
	else
