...
/obj/item/toy/balloon/afterattack(atom/A as mob|obj, mob/user, proximity)
	...
	if (istype(A, /obj/structure/reagent_dispensers))
		...
		if(RD.reagents.total_volume <= 0)
			user.text2tab("<span class='warning'>[RD] is empty.</span>")
		else if(reagents.total_volume >= 10)
			user.text2tab("<span class='warning'>[src] is full.</span>")
		else
			A.reagents.trans_to(src, 10)
			user.text2tab("<span class='notice'>You fill the balloon with the contents of [A].</span>")
	...
	if(istype(I, /obj/item/weapon/reagent_containers/glass))
		if(I.reagents)
			if(I.reagents.total_volume <= 0)
				user.text2tab("<span class='warning'>[I] is empty.</span>")
			else if(reagents.total_volume >= 10)
				user.text2tab("<span class='warning'>[src] is full.</span>")
			else
				desc = "A translucent balloon with some form of liquid sloshing around in it."
				user.text2tab("<span class='notice'>You fill the balloon with the contents of [I].</span>")
...
/obj/item/toy/gun/examine(mob/user)
	..()
	user.text2tab("There [bullets == 1 ? "is" : "are"] [bullets] cap\s left.")

/obj/item/toy/gun/attackby(obj/item/toy/ammo/gun/A, mob/user, params)

	if(istype(A, /obj/item/toy/ammo/gun))
		if (src.bullets >= 7)
			user.text2tab("<span class='warning'>It's already fully loaded!</span>")
			return 1
		if (A.amount_left <= 0)
			user.text2tab("<span class='warning'>There are no more caps!</span>")
			return 1
		if (A.amount_left < (7 - src.bullets))
			src.bullets += A.amount_left
			user.text2tab(text("<span class='notice'>You reload [] cap\s.</span>", A.amount_left))
			A.amount_left = 0
		else
			user.text2tab(text("<span class='notice'>You reload [] cap\s.</span>", 7 - src.bullets))
...
/obj/item/toy/gun/afterattack(atom/target as mob|obj|turf|area, mob/user, flag)
	...
	if (!(istype(usr, /mob/living/carbon/human) || ticker) && ticker.mode.name != "monkey")
		usr.text2tab("<span class='warning'>You don't have the dexterity to do this!</span>")
		return
...
/obj/item/toy/ammo/gun/examine(mob/user)
	..()
	user.text2tab("There [amount_left == 1 ? "is" : "are"] [amount_left] cap\s left.")
...
/obj/item/toy/sword/attack_self(mob/user)
	active = !( active )
	if (active)
		user.text2tab("<span class='notice'>You extend the plastic blade with a quick flick of your wrist.</span>")
		...
		user.text2tab("<span class='notice'>You push the plastic blade back down into the handle.</span>")
...
/obj/item/toy/sword/attackby(obj/item/weapon/W, mob/living/user, params)
	if(istype(W, /obj/item/toy/sword))
		if((W.flags & NODROP) || (flags & NODROP))
			user.text2tab("<span class='warning'>\the [flags & NODROP ? src : W] is stuck to your hand, you can't attach it to \the [flags & NODROP ? W : src]!</span>")
			return
		else
			user.text2tab("<span class='notice'>You attach the ends of the two plastic swords, making a single double-bladed toy! You're fake-cool.</span>")
...
/obj/item/toy/snappop/Crossed(H as mob|obj)
	if((ishuman(H))) //i guess carp and shit shouldn't set them off
		...
		if(M.m_intent == "run")
			M.text2tab("<span class='danger'>You step on the snap pop!</span>")
...
/obj/item/toy/prize/attack_self(mob/user)
	if(!cooldown)
		user.text2tab("<span class='notice'>You play with [src].</span>")
		cooldown = 1
		spawn(30) cooldown = 0
		if (!quiet)
...
/obj/item/toy/cards/deck/attack_hand(mob/user)
	...
	var/choice = null
	if(cards.len == 0)
		user.text2tab("<span class='warning'>There are no more cards to draw!</span>")
		return
...
/obj/item/toy/cards/deck/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/toy/cards/singlecard))
		...
		if(SC.parentdeck == src)
			if(!user.unEquip(SC))
				user.text2tab("<span class='warning'>The card is stuck to your hand, you can't add it to the deck!</span>")
				return
			cards += SC.cardname
			...
			user.text2tab("<span class='warning'>You can't mix cards from other decks!</span>")
		update_icon()
	else if(istype(I, /obj/item/toy/cards/cardhand))
		var/obj/item/toy/cards/cardhand/CH = I
		if(CH.parentdeck == src)
			if(!user.unEquip(CH))
				user.text2tab("<span class='warning'>The hand of cards is stuck to your hand, you can't add it to the deck!</span>")
				return
...
/obj/item/toy/cards/deck/MouseDrop(atom/over_object)
	...
	if(Adjacent(usr))
		if(over_object == M && loc != M)
			M.put_in_hands(src)
			usr.text2tab("<span class='notice'>You pick up the deck.</span>")
		...
		usr.text2tab("<span class='warning'>You can't reach it from here!</span>")
	...
	if(href_list["pick"])
		if (cardUser.get_item_by_slot(slot_l_hand) == src || cardUser.get_item_by_slot(slot_r_hand) == src)
			...
			if(src.currenthand.len == 1)
				...
				cardUser.text2tab("<span class='notice'>You also take [currenthand[1]] and hold it.</span>")
...
/obj/item/toy/cards/singlecard/examine(mob/user)
	if(ishuman(user))
		...
		if(cardUser.get_item_by_slot(slot_l_hand) == src || cardUser.get_item_by_slot(slot_r_hand) == src)
			...
			cardUser.text2tab("<span class='warning'>You need to have the card in your hand to check it!</span>")
...
/obj/item/toy/cards/singlecard/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/toy/cards/singlecard/))
		...
		if(C.parentdeck == src.parentdeck)
			...
			user.text2tab("<span class='notice'>You combine the [C.cardname] and the [src.cardname] into a hand.</span>")
			qdel(C)
			qdel(src)
		else
			user.text2tab("<span class='warning'>You can't mix cards from other decks!</span>")
...
/obj/item/toy/nuke/attack_self(mob/user)
	if (cooldown < world.time)
		...
		user.text2tab("<span class='alert'>Nothing happens, and '</span>[round(timeleft/10)]<span class='alert'>' appears on a small display.</span>")
...
/obj/item/toy/carpplushie/attack_self(mob/user)
	...
	user.text2tab("<span class='notice'>You pet [src]. D'awww.</span>")
	return ..()
...
/obj/item/toy/redbutton/attack_self(mob/user)
	if (cooldown < world.time)
		...
		user.text2tab("<span class='alert'>Nothing happens.</span>")
...
/obj/item/toy/toy_xeno/attack_self(mob/user)
	if(cooldown <= world.time)
		...
		user.text2tab("<span class='warning'>The string on [src] hasn't rewound all the way!</span>")
		return
...
/obj/item/toy/figure/attack_self(mob/user as mob)
	if(cooldown <= world.time)
		...
		user.text2tab("<span class='notice'>The [src] says \"[toysay]\"</span>")
