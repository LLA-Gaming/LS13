...
/obj/item/weapon/match/attack(mob/living/carbon/M, mob/living/carbon/user)
	...
	if(lit && cig && user.a_intent == "help")
		if(cig.lit)
			user.text2tab("<span class='notice'>The [cig.name] is already lit.</span>")
		if(M == user)
...
/obj/item/clothing/mask/cigarette/afterattack(obj/item/weapon/reagent_containers/glass/glass, mob/user, proximity)
	...
	if(istype(glass))	//you can dip cigarettes into beakers
		...
		if(transfered)	//if reagents were transfered, show the message
			user.text2tab("<span class='notice'>You dip \the [src] into \the [glass].</span>")
		else			//if not, either the beaker was empty, or the cigarette was full
			if(!glass.reagents.total_volume)
				user.text2tab("<span class='notice'>[glass] is empty.</span>")
			else
				user.text2tab("<span class='notice'>[src] is full.</span>")
...
/obj/item/clothing/mask/cigarette/process()
	...
	if(smoketime < 1)
		...
		if(ismob(loc))
			M.text2tab("<span class='notice'>Your [name] goes out.</span>")
...
/obj/item/clothing/mask/cigarette/pipe/attackby(obj/item/O, mob/user, params)
	if(istype(O, /obj/item/weapon/reagent_containers/food/snacks/grown))
		...
		if(!packeditem)
			if(G.dry == 1)
				user.text2tab("<span class='notice'>You stuff [O] into [src].</span>")
				smoketime = 400
				packeditem = 1
				...
				user.text2tab("<span class='warning'>It has to be dried first!</span>")
		else
			user.text2tab("<span class='warning'>It is already packed!</span>")
	else
		...
		if(lighting_text)
			if(smoketime > 0)
				...
				user.text2tab("<span class='warning'>There is nothing to smoke!</span>")
		else
			return ..()
...
/obj/item/clothing/mask/cigarette/pipe/attack_self(mob/user)
	...
	if(lit)
		...
		return
	if(!lit && smoketime > 0)
		user.text2tab("<span class='notice'>You empty [src] onto [location].</span>")
		new /obj/effect/decal/cleanable/ash(location)
		packeditem = 0
		smoketime = 0
...
/obj/item/weapon/rollingpaper/afterattack(atom/target, mob/user, proximity)
	...
	if(istype(target, /obj/item/weapon/reagent_containers/food/snacks/grown))
		...
		if(O.dry)
			...
			user.text2tab("<span class='notice'>You roll the [target.name] into a rolling paper.</span>")
			R.desc = "Dried [target.name] rolled up in a thin piece of paper."
			qdel(target)
			qdel(src)
		else
			user.text2tab("<span class='warning'>You need to dry this first!</span>")
	else
		..()
