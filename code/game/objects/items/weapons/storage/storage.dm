...
/obj/item/weapon/storage/proc/standard_orient_objs(rows, cols, list/obj/item/display_contents)
	...
	if(display_contents_with_number)
		for(var/datum/numbered_display/ND in display_contents)
			...
			ND.sample_object.plane = HUDPLANE
			cx++
			if(cx > (4+cols))
				cx = 4
				cy--
...
/obj/item/weapon/storage/proc/can_be_inserted(obj/item/W, stop_messages = 0, mob/user)
	...
	if(contents.len >= storage_slots)
		if(!stop_messages)
			usr.text2tab("<span class='warning'>[src] is full, make some space!</span>")
	...
	if(can_hold.len)
		...
		if(!ok)
			if(!stop_messages)
				usr.text2tab("<span class='warning'>[src] cannot hold [W]!</span>")
			return 0
	...
	if(W.w_class > max_w_class)
		if(!stop_messages)
			usr.text2tab("<span class='warning'>[W] is too big for [src]!</span>")
		return 0
	...
	if(sum_w_class > max_combined_w_class)
		if(!stop_messages)
			usr.text2tab("<span class='warning'>[W] won't fit in [src], make some space!</span>")
		return 0

	if(W.w_class >= w_class && (istype(W, /obj/item/weapon/storage)))
		if(!istype(src, /obj/item/weapon/storage/backpack/holding))	//bohs should be able to hold backpacks again. The override for putting a boh in a boh is in backpack.dm.
			if(!stop_messages)
				usr.text2tab("<span class='warning'>[src] cannot hold [W] as it's a storage item of the same size!</span>")
			return 0 //To prevent the stacking of same sized storage items.

	if(W.flags & NODROP) //SHOULD be handled in unEquip, but better safe than sorry.
		usr.text2tab("<span class='warning'>\the [W] is stuck to your hand, you can't put it in \the [src]!</span>")
		return 0
	...
	if(usr)
		...
		if(!prevent_warning)
			for(var/mob/M in viewers(usr, null))
				if(M == usr)
					usr.text2tab("<span class='notice'>You put [W] [preposition]to [src].</span>")
...
/obj/item/weapon/storage/verb/toggle_gathering_mode()
	...
	switch (collection_mode)
		if(2)
			usr.text2tab("[src] now picks up all items of a single type at once.")
		if(1)
			usr.text2tab("[src] now picks up all items in a tile at once.")
		if(0)
			usr.text2tab("[src] now picks up one item at a time.")
