...
/obj/item/weapon/storage/box/attack_self(mob/user)
	...
	if(!foldable)
		return
	if(contents.len)
		user.text2tab("<span class='warning'>You can't fold this box with items still inside!</span>")
		return
	if(!ispath(foldable))
		return
	...
	user.text2tab("<span class='notice'>You fold [src] flat.</span>")
...
/obj/item/weapon/storage/box/papersack/attackby(obj/item/weapon/W, mob/user, params)
	if(istype(W, /obj/item/weapon/pen))
		...
		if(contents.len)
			user.text2tab("<span class='warning'>You can't modify this [src] with items still inside!</span>")
			return
		...
		if(design == designs[choice] || designs[choice] == "Cancel")
			return 0
		usr.text2tab("<span class='notice'>You make some modifications to the [src] using your pen.</span>")
