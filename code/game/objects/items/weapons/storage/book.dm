...
/obj/item/weapon/storage/book/bible/attack(mob/living/M, mob/living/carbon/human/user)
	...
	if (!user.IsAdvancedToolUser())
		...
		return
	if(!chaplain)
		user.text2tab("<span class='danger'>The book sizzles in your hands.</span>")
		user.take_organ_damage(0,10)
		return

	if (user.disabilities & CLUMSY && prob(50))
		user.text2tab("<span class='danger'>The [src] slips out of your hand and hits your head.</span>")
	...
	if (M.stat !=2)
		if(M.mind && (M.mind.assigned_role == "Chaplain"))
			user.text2tab("<span class='warning'>You can't heal yourself!</span>")
			return
				...
				for(var/obj/item/bodypart/affecting in H.bodyparts)
					if(affecting.status == ORGAN_ORGANIC)
						if(message_halt == 0)
							...
							M.text2tab("<span class='boldnotice'>May the power of [src.deity_name] compel you to be healed!</span>")
							playsound(src.loc, "punch", 25, 1, -1)
							message_halt = 1
					else
						user.text2tab("<span class='warning'>[src.deity_name] refuses to heal this metallic taint!</span>")
						return
...
/obj/item/weapon/storage/book/bible/afterattack(atom/A, mob/user, proximity)
	...
	if (istype(A, /turf/open/floor))
		user.text2tab("<span class='notice'>You hit the floor with the bible.</span>")
		if(user.mind && (user.mind.assigned_role == "Chaplain"))
			for(var/obj/effect/rune/R in orange(2,user))
				R.invisibility = 0
	if(user.mind && (user.mind.assigned_role == "Chaplain"))
		if(A.reagents && A.reagents.has_reagent("water")) //blesses all the water in the holder
			user.text2tab("<span class='notice'>You bless [A].</span>")
			var/water2holy = A.reagents.get_reagent_amount("water")
			A.reagents.del_reagent("water")
			A.reagents.add_reagent("holywater",water2holy)
		if(A.reagents && A.reagents.has_reagent("unholywater")) //yeah yeah, copy pasted code - sue me
			user.text2tab("<span class='notice'>You purify [A].</span>")
