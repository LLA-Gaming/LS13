...
/obj/item/weapon/soap/afterattack(atom/target, mob/user, proximity)
	...
	if(user.client && (target in user.client.screen))
		user.text2tab("<span class='warning'>You need to take that [target.name] off before cleaning it!</span>")
	else if(istype(target,/obj/effect/decal/cleanable))
		user.visible_message("[user] begins to scrub \the [target.name] out with [src].", "<span class='warning'>You begin to scrub \the [target.name] out with [src]...</span>")
		if(do_after(user, src.cleanspeed, target = target))
			user.text2tab("<span class='notice'>You scrub \the [target.name] out.</span>")
			qdel(target)
		...
		if(do_after(user, src.cleanspeed, target = target))
			user.text2tab("<span class='notice'>You clean \the [target.name].</span>")
