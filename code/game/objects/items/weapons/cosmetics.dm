...
/obj/item/weapon/lipstick/attack_self(mob/user)
	overlays.Cut()
	user.text2tab("<span class='notice'>You twist \the [src] [open ? "closed" : "open"].</span>")
	open = !open
	if(open)
...
/obj/item/weapon/lipstick/attack(mob/M, mob/user)
	...
	if(ishuman(M))
		...
		if(H.is_mouth_covered())
			user.text2tab("<span class='warning'>Remove [ H == user ? "your" : "their" ] mask!</span>")
			return
		if(H.lip_style)	//if they already have lipstick on
			user.text2tab("<span class='warning'>You need to wipe off the old lipstick first!</span>")
			return
		if(H == user)
		...
		user.text2tab("<span class='warning'>Where are the lips on that?</span>")
...
/obj/item/weapon/paper/attack(mob/M, mob/user)
	if(user.zone_selected == "mouth")
		...
		if(ishuman(M))
			var/mob/living/carbon/human/H = M
			if(H == user)
				user.text2tab("<span class='notice'>You wipe off the lipstick with [src].</span>")
				H.lip_style = null
				H.update_body()
			else
	...
	if(ishuman(M))
		...
		if(location == "mouth")
			if(!(FACEHAIR in H.dna.species.specflags))
				user.text2tab("<span class='warning'>There is no facial hair to shave!</span>")
				return
			if(!get_location_accessible(H, location))
				user.text2tab("<span class='warning'>The mask is in the way!</span>")
				return
			if(H.facial_hair_style == "Shaved")
				user.text2tab("<span class='warning'>Already clean-shaven!</span>")
				return
			...
			if(!(HAIR in H.dna.species.specflags))
				user.text2tab("<span class='warning'>There is no hair to shave!</span>")
				return
			if(!get_location_accessible(H, location))
				user.text2tab("<span class='warning'>The headgear is in the way!</span>")
				return
			if(H.hair_style == "Bald" || H.hair_style == "Balding Hair" || H.hair_style == "Skinhead")
				user.text2tab("<span class='warning'>There is not enough hair left to shave!</span>")
				return
