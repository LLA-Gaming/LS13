...
/obj/item/weapon/dnainjector/proc/inject(mob/living/carbon/M, mob/user)
	...
	if(M.has_dna() && !(M.disabilities & NOCLONE))
		...
		user.text2tab("<span class='notice'>It appears that [M] does not have compatible DNA.</span>")
		return
...
/obj/item/weapon/dnainjector/attack(mob/target, mob/user)
	if(!user.IsAdvancedToolUser())
		...
		return
	if(used)
		user.text2tab("<span class='warning'>This injector is used up!</span>")
		return
	if(ishuman(target))
	...
	if(target != user)
		...
		user.text2tab("<span class='notice'>You inject yourself with [src].</span>")
...
/obj/item/weapon/dnainjector/timed/inject(mob/living/carbon/M, mob/user)
	...
	if(M.has_dna() && !(M.disabilities & NOCLONE))
		if(M.stat == DEAD)	//prevents dead people from having their DNA changed
			user.text2tab("<span class='notice'>You can't modify [M]'s DNA while \he's dead.</span>")
			return
		M.radiation += rand(20/(damage_coeff  ** 2),50/(damage_coeff  ** 2))
		var/log_msg = "[key_name(user)] injected [key_name(M)] with the [name]"
