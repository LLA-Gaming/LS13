	...
	if(loc == user)
		if(slot_flags == SLOT_BACK)
			if(user.get_item_by_slot(slot_back) == src)
				...
				user.text2tab("<span class='warning'>Put the defibrillator on your back first!</span>")

		else if(slot_flags == SLOT_BELT)
			if(user.get_item_by_slot(slot_belt) == src)
				ui_action_click()
			else
				user.text2tab("<span class='warning'>Strap the defibrillator's belt on first!</span>")
		return
	..()
...
/obj/item/weapon/defibrillator/attackby(obj/item/weapon/W, mob/user, params)
	if(W == paddles)
		...
		if(bcell)
			user.text2tab("<span class='notice'>[src] already has a cell.</span>")
		else
			if(C.maxcharge < paddles.revivecost)
				user.text2tab("<span class='notice'>[src] requires a higher capacity cell.</span>")
				return
			if(!user.unEquip(W))
				return
			W.loc = src
			bcell = W
			user.text2tab("<span class='notice'>You install a cell in [src].</span>")
			update_icon()
		...
		if(bcell)
			...
			bcell = null
			user.text2tab("<span class='notice'>You remove the cell from [src].</span>")
			update_icon()
	else
		return ..()
...
/obj/item/weapon/defibrillator/emag_act(mob/user)
	if(safety)
		safety = 0
		user.text2tab("<span class='warning'>You silently disable [src]'s safety protocols with the cryptographic sequencer.")
	else
		safety = 1
		user.text2tab("<span class='notice'>You silently enable [src]'s safety protocols with the cryptographic sequencer.")
...
/obj/item/weapon/defibrillator/proc/toggle_paddles()
	...
	if(on)
		...
		if(!usr.put_in_hands(paddles))
			on = 0
			user.text2tab("<span class='warning'>You need a free hand to hold the paddles!</span>")
			update_icon()
			return
		paddles.loc = user
	else
...
/obj/item/weapon/twohanded/shockpaddles/dropped(mob/user)
	...
	if(user)
		...
		if(istype(O))
			O.unwield()
		user.text2tab("<span class='notice'>The paddles snap back into the main unit.</span>")
		defib.on = 0
		loc = defib
		defib.update_icon()
	return unwield(user)
...
/obj/item/weapon/twohanded/shockpaddles/attack(mob/M, mob/user)
	...
	if(!wielded)
		if(isrobot(user))
			user.text2tab("<span class='warning'>You must activate the paddles in your active module before you can use them on someone!</span>")
		else
			user.text2tab("<span class='warning'>You need to wield the paddles in both hands before you can use them on someone!</span>")
		return
	if(cooldown)
		if(req_defib)
			user.text2tab("<span class='warning'>[defib] is recharging!</span>")
		else
			user.text2tab("<span class='warning'>[src] are recharging!</span>")
		return
	if(!ishuman(M))
		if(req_defib)
			user.text2tab("<span class='warning'>The instructions on [defib] don't mention how to revive that...</span>")
		else
			user.text2tab("<span class='warning'>You aren't sure how to revive that...</span>")
		return
	...
	if(user.zone_selected != "chest")
		user.text2tab("<span class='warning'>You need to target your patient's \
			chest with [src]!</span>")
		return
