...
/obj/item/weapon/holosign_creator/afterattack(atom/target, mob/user, flag)
	if(flag)
		...
		if(H)
			user.text2tab("<span class='notice'>You use [src] to deactivate [H].</span>")
			qdel(H)
		else
			if(!is_blocked_turf(T)) //can't put holograms on a tile that has dense stuff
				if(holocreator_busy)
					user.text2tab("<span class='notice'>[src] is busy creating a hologram.</span>")
					return
				if(signs.len < max_signs)
					...
					user.text2tab("<span class='notice'>You create \a [H] with [src].</span>")
				else
					user.text2tab("<span class='notice'>[src] is projecting at max capacity!</span>")
...
/obj/item/weapon/holosign_creator/attack_self(mob/user)
	if(signs.len)
		for(var/H in signs)
			qdel(H)
		user.text2tab("<span class='notice'>You clear all active holograms.</span>")
...
/obj/item/weapon/holosign_creator/cyborg/attack_self(mob/user)
	if(isrobot(user))
		...
		if(shock)
			user.text2tab("<span class='notice'>You clear all active holograms, and reset your projector to normal.</span>")
			holosign_type = /obj/effect/overlay/holograph/barrier/cyborg
			creation_time = 5
			if(signs.len)
				for(var/H in signs)
			...
			user.text2tab("<span class='warning'>You clear all active holograms, and overload your energy projector!</span>")
