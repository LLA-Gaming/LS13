	...
	switch(p_class)
		if(PAINT_MODE) // Paint pipes
			if(!istype(A,/obj/machinery/atmospherics/pipe))
				...
				user.text2tab("<span class='warning'>\The [src]'s error light flickers!  Perhaps you need to only use it on pipes and pipe meters?</span>")
				return 0
		...
		if(EATING_MODE) // Eating pipes
			...
			if(istype(A,/obj/item/pipe) || istype(A,/obj/item/pipe_meter) || istype(A,/obj/structure/disposalconstruct))
				user.text2tab("<span class='notice'>You start destroying pipe...</span>")
				playsound(get_turf(src), 'sound/machines/click.ogg', 50, 1)
		...
		if(ATMOS_MODE)
			if(!(istype(A, /turf)))
				user.text2tab("<span class='warning'>The [src]'s error light flickers!</span>")
				return 0
			user.text2tab("<span class='notice'>You start building pipes...</span>")
			playsound(get_turf(src), 'sound/machines/click.ogg', 50, 1)
			if(do_after(user, 2, target = A))
				activate()
		...
		if(METER_MODE)
			if(!(istype(A, /turf)))
				user.text2tab("<span class='warning'>The [src]'s error light flickers!</span>")
				return 0
			user.text2tab("<span class='notice'>You start building meter...</span>")
			playsound(get_turf(src), 'sound/machines/click.ogg', 50, 1)
			if(do_after(user, 2, target = A))
				activate()
		...
		if(DISPOSALS_MODE)
			...
			if(do_after(user, 20, target = A))
				...
				if(!C.can_place())
					user.text2tab("<span class='warning'>There's not enough room to build that here!</span>")
					qdel(C)
					return 0
