...
/obj/item/weapon/flamethrower/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/weapon/wrench) && !status)//Taking this apart
		...
		status = !status
		user.text2tab("<span class='notice'>[igniter] is now [status ? "secured" : "unsecured"]!</span>")
		update_icon()
		return
		...
		if(ptank)
			user.text2tab("<span class='notice'>There appears to already be a plasma tank loaded in [src]!</span>")
			return
		if(!user.unEquip(W))
			return
		ptank = W
...
/obj/item/weapon/flamethrower/attack_self(mob/user)
	...
	if(!ptank)
		user.text2tab("<span class='notice'>Attach a plasma tank first!</span>")
		return
