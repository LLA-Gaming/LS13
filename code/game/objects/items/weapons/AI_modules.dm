...
/obj/item/weapon/aiModule/proc/show_laws(var/mob/user as mob)
	if(laws.len)
		user.text2tab("<B>Programmed Law[(laws.len > 1) ? "s" : ""]:</B>")
		for(var/law in laws)
			user.text2tab("\"[law]\"")
...
/obj/item/weapon/aiModule/proc/install(datum/ai_laws/law_datum, mob/user)
	if(!bypass_law_amt_check && (!laws.len || laws[1] == "")) //So we don't loop trough an empty list and end up with runtimes.
		user.text2tab("<span class='warning'>ERROR: No laws found on board.</span>")
		return
	...
	if(law_datum)
		...
		if(tot_laws > config.silicon_max_law_amount && !bypass_law_amt_check)//allows certain boards to avoid this check, eg: reset
			user.text2tab("<span class='caution'>Not enough memory allocated to [law_datum.owner ? law_datum.owner : "the AI core"]'s law processor to handle this amount of laws.")
	...
	if(law_datum.owner)
		user.text2tab("<span class='notice'>Upload complete. [law_datum.owner]'s laws have been modified.</span>")
		law_datum.owner.show_laws()
		law_datum.owner.law_change_counter++
	else
		user.text2tab("<span class='notice'>Upload complete.</span>")
...
/obj/item/weapon/aiModule/proc/transmitInstructions(datum/ai_laws/law_datum, mob/sender)
	if(law_datum.owner)
		law_datum.owner.text2tab("<span class='userdanger'>[sender] has uploaded a change to the laws you must follow using a [name]. From now on, these are your laws: </span>")
...
/obj/item/weapon/aiModule/zeroth/transmitInstructions(datum/ai_laws/law_datum, mob/sender)
	if(law_datum.owner)
		if(law_datum.owner.laws.zeroth)
			law_datum.owner.text2tab("[sender.real_name] attempted to modify your zeroth law.")
			law_datum.owner.text2tab("It would be in your best interest to play along with [sender.real_name] that:")
			for(var/failedlaw in laws)
				law_datum.owner.text2tab("[failedlaw]")
			return 1
...
/obj/item/weapon/aiModule/supplied/safeguard/install(datum/ai_laws/law_datum, mob/user)
	if(!targetName)
		user.text2tab("No name detected on module, please enter one.")
		return 0
	..()
...
/obj/item/weapon/aiModule/supplied/freeform/install(datum/ai_laws/law_datum, mob/user)
	if(laws[1] == "")
		user.text2tab("No law detected on module, please create one.")
		return 0
	..()
...
//	..()    //We don't want this module reporting to the AI who dun it. --NEO
	if(law_datum.owner)
		law_datum.owner.text2tab("<span class='warning'>BZZZZT</span>")
...
/obj/item/weapon/aiModule/toyAI/transmitInstructions(datum/ai_laws/law_datum, mob/sender)
	//..()
	if(law_datum.owner)
		law_datum.owner.text2tab("<span class='warning'>KRZZZT</span>")
		law_datum.owner.add_ion_law(laws[1])
	else
		law_datum.add_ion_law(laws[1])
	return laws[1]
...
/obj/item/weapon/aiModule/toyAI/attack_self(mob/user)
	...
	user.text2tab("<span class='notice'>You press the button on [src].</span>")
