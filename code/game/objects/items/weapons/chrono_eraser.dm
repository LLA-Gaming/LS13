...
/obj/item/weapon/gun/energy/chrono_gun/proc/field_connect(obj/effect/chrono_field/F)
	...
	if(F.gun)
		if(isliving(user) && F.captured)
			user.text2tab("<span class='alert'><b>FAIL: <i>[F.captured]</i> already has an existing connection.</b></span>")
		...
		F.gun = src
		if(isliving(user) && F.captured)
			user.text2tab("<span class='notice'>Connection established with target: <b>[F.captured]</b></span>")
...
/obj/item/weapon/gun/energy/chrono_gun/proc/field_disconnect(obj/effect/chrono_field/F)
	if(F && field == F)
		...
		if(F.gun == src)
			F.gun = null
		if(isliving(user) && F.captured)
			user.text2tab("<span class='alert'>Disconnected from target: <b>[F.captured]</b></span>")
	field = null
	startpos = null
...
/obj/effect/chrono_field/process()
	if(captured)
		if(tickstokill > initial(tickstokill))
			...
			captured.text2tab("<span class='boldnotice'>As the last essence of your being is erased from time, you begin to re-experience your most enjoyable memory. You feel happy...</span>")
