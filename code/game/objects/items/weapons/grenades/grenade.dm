...
/obj/item/weapon/grenade/proc/clown_check(mob/living/carbon/human/user)
	if(user.disabilities & CLUMSY && prob(50))
		user.text2tab("<span class='warning'>Huh? How does this thing work?</span>")
		active = 1
...
/obj/item/weapon/grenade/examine(mob/user)
	...
	if(display_timer)
		if(det_time > 1)
			user.text2tab("The timer is set to [det_time/10] second\s.")
		else
			user.text2tab("\The [src] is set for instant detonation.")


/obj/item/weapon/grenade/attack_self(mob/user)
	if(!active)
		if(clown_check(user))
			user.text2tab("<span class='warning'>You prime the [name]! [det_time/10] seconds!</span>")
