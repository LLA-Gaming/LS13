...
/obj/item/weapon/grenade/chem_grenade/attack_self(mob/user)
	if(stage == READY &&  !active)
		if(nadeassembly)
			...
			user.text2tab("<span class='warning'>You prime the [name]! [det_time / 10] second\s!</span>")
...
/obj/item/weapon/grenade/chem_grenade/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/screwdriver))
		if(stage == WIRED)
			if(beakers.len)
				stage_change(READY)
				user.text2tab("<span class='notice'>You lock the [initial(name)] assembly.</span>")
				playsound(loc, 'sound/items/Screwdriver.ogg', 25, -3)
			else
				user.text2tab("<span class='warning'>You need to add at least one beaker before locking the [initial(name)] assembly!</span>")
		else if(stage == READY && !nadeassembly)
			det_time = det_time == 50 ? 30 : 50	//toggle between 30 and 50
			user.text2tab("<span class='notice'>You modify the time delay. It's set for [det_time / 10] second\s.</span>")
		else if(stage == EMPTY)
			user.text2tab("<span class='warning'>You need to add an activation mechanism!</span>")

	else if(stage == WIRED && is_type_in_list(I, allowed_containers))
		. = 1 //no afterattack
		if(beakers.len == 2)
			user.text2tab("<span class='warning'>[src] can not hold more containers!</span>")
			return
		else
			if(I.reagents.total_volume)
				if(!user.unEquip(I))
					return
				user.text2tab("<span class='notice'>You add [I] to the [initial(name)] assembly.</span>")
				I.loc = src
				beakers += I
			else
		...
		stage_change(WIRED)
		user.text2tab("<span class='notice'>You add [A] to the [initial(name)] assembly.</span>")
		...
		if (C.use(1))
			...
			stage_change(WIRED)
			user.text2tab("<span class='notice'>You rig the [initial(name)] assembly.</span>")
		else
			user.text2tab("<span class='warning'>You need one length of coil to wire the assembly!</span>")
			return

	else if(stage == READY && istype(I, /obj/item/weapon/wirecutters))
		stage_change(WIRED)
		user.text2tab("<span class='notice'>You unlock the [initial(name)] assembly.</span>")
		...
		if(beakers.len)
			for(var/obj/O in beakers)
				O.loc = get_turf(src)
			beakers = list()
			user.text2tab("<span class='notice'>You open the [initial(name)] assembly and remove the payload.</span>")
		...
		stage_change(EMPTY)
		user.text2tab("<span class='notice'>You remove the activation mechanism from the [initial(name)] assembly.</span>")
	else
		return ..()
	...
	if(istype(I, /obj/item/device/multitool))
		switch(unit_spread)
			...
			if(25 to 99)
				...
				unit_spread = 5
		user.text2tab("<span class='notice'> You set the time release to [unit_spread] units per detonation.</span>")
		return
	..()
