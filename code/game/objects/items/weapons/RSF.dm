...
/obj/item/weapon/rsf/attackby(obj/item/weapon/W, mob/user, params)
	if(istype(W, /obj/item/weapon/rcd_ammo))
		if ((matter + 10) > 30)
			user.text2tab("The RSF can't hold any more matter.")
			return
		qdel(W)
		matter += 10
		playsound(src.loc, 'sound/machines/click.ogg', 10, 1)
		user.text2tab("The RSF now holds [matter]/30 fabrication-units.")
		desc = "A RSF. It currently holds [matter]/30 fabrication-units."
	else
		return ..()
...
/obj/item/weapon/rsf/attack_self(mob/user)
	...
	if (mode == 1)
		mode = 2
		user.text2tab("Changed dispensing mode to 'Drinking Glass'")
		return
	if (mode == 2)
		mode = 3
		user.text2tab("Changed dispensing mode to 'Paper'")
		return
	if (mode == 3)
		mode = 4
		user.text2tab("Changed dispensing mode to 'Pen'")
		return
	if (mode == 4)
		mode = 5
		user.text2tab("Changed dispensing mode to 'Dice Pack'")
		return
	if (mode == 5)
		mode = 6
		user.text2tab("Changed dispensing mode to 'Cigarette'")
		return
	if (mode == 6)
		mode = 1
		user.text2tab("Changed dispensing mode to 'Dosh'")
		return
	// Change mode
...
/obj/item/weapon/rsf/afterattack(atom/A, mob/user, proximity)
	...
	if(matter < 1)
		user.text2tab("<span class='warning'>\The [src] doesn't have enough matter left.</span>")
		return
	if(isrobot(user))
		var/mob/living/silicon/robot/R = user
		if(!R.cell || R.cell.charge < 200)
			user.text2tab("<span class='warning'>You do not have enough power to use [src].</span>")
			return
	...
	switch(mode)
		if(1)
			user.text2tab("Dispensing Dosh...")
			new /obj/item/stack/spacecash/c10(T)
			use_matter(200, user)
		if(2)
			user.text2tab("Dispensing Drinking Glass...")
			new /obj/item/weapon/reagent_containers/food/drinks/drinkingglass(T)
			use_matter(20, user)
		if(3)
			user.text2tab("Dispensing Paper Sheet...")
			new /obj/item/weapon/paper(T)
			use_matter(10, user)
		if(4)
			user.text2tab("Dispensing Pen...")
			new /obj/item/weapon/pen(T)
			use_matter(50, user)
		if(5)
			user.text2tab("Dispensing Dice Pack...")
			new /obj/item/weapon/storage/pill_bottle/dice(T)
			use_matter(200, user)
		if(6)
			user.text2tab("Dispensing Cigarette...")
...
/obj/item/weapon/cookiesynth/emag_act(mob/user)
	emagged = !emagged
	if(emagged)
		user.text2tab("<span class='warning'>You short out the [src]'s reagent safety checker!</span>")
	else
		user.text2tab("<span class='warning'>You reset the [src]'s reagent safety checker!</span>")
		toxin = 0
...
/obj/item/weapon/cookiesynth/attack_self(mob/user)
	...
	if(emagged&&!toxin)
		toxin = 1
		user.text2tab("Cookie Synthesizer Hacked")
	else if(P.emagged&&!toxin)
		toxin = 1
		user.text2tab("Cookie Synthesizer Hacked")
	else
		toxin = 0
		user.text2tab("Cookie Synthesizer Reset")
...
/obj/item/weapon/cookiesynth/afterattack(atom/A, mob/user, proximity)
	...
	if (!(istype(A, /obj/structure/table) || istype(A, /turf/open/floor)))
		return
	if(matter < 1)
		user.text2tab("<span class='warning'>The [src] doesn't have enough matter left. Wait for it to recharge!</span>")
		return
	if(isrobot(user))
		var/mob/living/silicon/robot/R = user
	...
	var/turf/T = get_turf(A)
	playsound(src.loc, 'sound/machines/click.ogg', 10, 1)
	user.text2tab("Fabricating Cookie..")
