...
/obj/item/weapon/melee/classic_baton/attack(mob/target, mob/living/user)
	if(on)
		...
		if((CLUMSY in user.disabilities) && prob(50))
			user.text2tab("<span class ='danger'>You club yourself over the head.</span>")
			user.Weaken(3 * force)
			if(ishuman(user))
...
/obj/item/weapon/melee/classic_baton/telescopic/attack_self(mob/user)
	on = !on
	if(on)
		user.text2tab("<span class ='warning'>You extend the baton.</span>")
		...
		user.text2tab("<span class ='notice'>You collapse the baton.</span>")
		icon_state = "telebaton_0"
