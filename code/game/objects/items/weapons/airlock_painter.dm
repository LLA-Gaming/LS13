...
/obj/item/weapon/airlock_painter/proc/can_use(mob/user)
	if(!ink)
		user.text2tab("<span class='notice'>There is no toner cartridge installed installed in \the [name]!</span>")
		return 0
	else if(ink.charges < 1)
		user.text2tab("<span class='notice'>\The [name] is out of ink!</span>")
		return 0
	else
		return 1
...
/obj/item/weapon/airlock_painter/examine(mob/user)
	..()
	if(!ink)
		user.text2tab("<span class='notice'>It doesn't have a toner cardridge installed.</span>")
		return
	var/ink_level = "high"
	if(ink.charges < 1)
		ink_level = "empty"
	...
	user.text2tab("<span class='notice'>Its ink levels look [ink_level].</span>")


/obj/item/weapon/airlock_painter/attackby(obj/item/weapon/W, mob/user, params)
	if(istype(W, /obj/item/device/toner))
		if(ink)
			user.text2tab("<span class='notice'>\the [name] already contains \a [ink].</span>")
			return
		if(!user.unEquip(W))
			return
		W.loc = src
		user.text2tab("<span class='notice'>You install \the [W] into \the [name].</span>")
		ink = W
...
/obj/item/weapon/airlock_painter/attack_self(mob/user)
	if(ink)
		...
		user.text2tab("<span class='notice'>You remove \the [ink] from \the [name].</span>")
		ink = null
