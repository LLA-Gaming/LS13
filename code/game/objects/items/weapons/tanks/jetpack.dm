...
/obj/item/weapon/tank/jetpack/ui_action_click(mob/user, actiontype)
	if(actiontype == /datum/action/item_action/toggle_jetpack)
		...
		if(on)
			...
			user.text2tab("<span class='notice'>You turn the jetpack stabilization [stabilizers ? "on" : "off"].</span>")
	else
...
/obj/item/weapon/tank/jetpack/proc/cycle(mob/user)
	...
	if(!on)
		turn_on()
		user.text2tab("<span class='notice'>You turn the jetpack on.</span>")
	else
		turn_off()
		user.text2tab("<span class='notice'>You turn the jetpack off.</span>")
...
/obj/item/weapon/tank/jetpack/suit/cycle(mob/user)
	...
	if(!istype(H.s_store, /obj/item/weapon/tank/internals))
		user.text2tab("<span class='warning'>You need a tank in your suit storage!</span>")
		return
	..()
