...
/obj/item/weapon/watertank/verb/toggle_mister()
	...
	if (usr.get_item_by_slot(usr.getBackSlot()) != src)
		usr.text2tab("<span class='warning'>The watertank must be worn properly to use!</span>")
		return
	...
	if(on)
		...
		if(!user.put_in_hands(noz))
			on = 0
			user.text2tab("<span class='warning'>You need a free hand to hold the mister!</span>")
			return
		noz.loc = user
	else
...
/obj/item/weapon/reagent_containers/spray/mister/dropped(mob/user)
	..()
	user.text2tab("<span class='notice'>The mister snaps back onto the watertank.</span>")
	tank.on = 0
	loc = tank
...
/obj/item/weapon/reagent_containers/spray/mister/janitor/attack_self(var/mob/user)
	...
	user.text2tab("<span class='notice'>You [amount_per_transfer_from_this == 10 ? "remove" : "fix"] the nozzle. You'll now use [amount_per_transfer_from_this] units per spray.</span>")
...
/obj/item/weapon/extinguisher/mini/nozzle/attack_self(mob/user)
	switch(nozzle_mode)
		if(EXTINGUISHER)
			...
			user.text2tab("Swapped to nanofrost launcher")
			return
		if(NANOFROST)
			nozzle_mode = METAL_FOAM
			tank.icon_state = "waterbackpackatmos_2"
			user.text2tab("Swapped to metal foam synthesizer")
			return
		if(METAL_FOAM)
			nozzle_mode = EXTINGUISHER
			tank.icon_state = "waterbackpackatmos_0"
			user.text2tab("Swapped to water extinguisher")
			return
	return

/obj/item/weapon/extinguisher/mini/nozzle/dropped(mob/user)
	..()
	user.text2tab("<span class='notice'>The nozzle snaps back onto the tank!</span>")
	tank.on = 0
	loc = tank
...
/obj/item/weapon/extinguisher/mini/nozzle/afterattack(atom/target, mob/user)
	...
	if(nozzle_mode == NANOFROST)
		...
		if(R.total_volume < 100)
			user.text2tab("<span class='warning'>You need at least 100 units of water to use the nanofrost launcher!</span>")
			return
		if(nanofrost_cooldown)
			user.text2tab("<span class='warning'>Nanofrost launcher is still recharging...</span>")
			return
	...
	if(nozzle_mode == METAL_FOAM)
		...
		if(metal_synthesis_cooldown < 5)
			...
			user.text2tab("<span class='warning'>Metal foam mix is still being synthesized...</span>")
			return
...
/obj/item/weapon/reagent_containers/chemtank/proc/toggle_injection()
	...
	if (user.get_item_by_slot(slot_back) != src)
		user.text2tab("<span class='warning'>The chemtank needs to be on your back before you can activate it!</span>")
		return
	if(on)
		turn_off()
	else
...
/obj/item/weapon/reagent_containers/chemtank/proc/turn_on()
	...
	if(ismob(loc))
		loc:text2tab("<span class='notice'>[src] turns on.</span>")

/obj/item/weapon/reagent_containers/chemtank/proc/turn_off()
	on = 0
	SSobj.processing.Remove(src)
	if(ismob(loc))
		loc:text2tab("<span class='notice'>[src] turns off.</span>")
