...
/obj/item/weapon/implant/mindshield/implant(mob/target)
	if(..())
		...
		if((target.mind in ticker.mode.cult) || (target.mind in ticker.mode.blue_deity_prophets|ticker.mode.red_deity_prophets|ticker.mode.red_deity_followers|ticker.mode.blue_deity_followers))
			target.text2tab("<span class='warning'>You feel something interfering with your mental conditioning, but you resist it!</span>")
		else
			target.text2tab("<span class='notice'>You feel a sense of peace and security. You are now protected from brainwashing.</span>")
		return 1
	return 0
...
/obj/item/weapon/implant/mindshield/removed(mob/target, var/silent = 0)
	if(..())
		if(target.stat != DEAD && !silent)
			target.text2tab("<span class='boldnotice'>Your mind suddenly feels terribly vulnerable. You are no longer safe from brainwashing.</span>")
		return 1
	return 0
