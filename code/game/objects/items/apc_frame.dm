...
/obj/item/wallframe/proc/try_build(turf/on_wall)
	...
	if(!istype(loc, /turf/open/floor))
		usr.text2tab("<span class='warning'>You cannot place [src] on this spot!</span>")
		return
	if(A.requires_power == 0 || istype(A, /area/space))
		usr.text2tab("<span class='warning'>You cannot place [src] in this area!</span>")
		return
	if(gotwallitem(loc, ndir, inverse*2))
		usr.text2tab("<span class='warning'>There's already an item on this wall!</span>")
		return
...
/obj/item/wallframe/attackby(obj/item/weapon/W, mob/user, params)
	...
	if(istype(W, /obj/item/weapon/wrench) && (metal_amt || glass_amt))
		user.text2tab("<span class='notice'>You dismantle [src].</span>")
		if(metal_amt)
...
/obj/item/wallframe/apc/try_build(turf/on_wall)
	...
	var/area/A = loc.loc
	if (A.get_apc())
		usr.text2tab("<span class='warning'>This area already has APC!</span>")
		return //only one APC per area
	for(var/obj/machinery/power/terminal/T in loc)
		if (T.master)
			usr.text2tab("<span class='warning'>There is another network terminal here!</span>")
			return
		else
			var/obj/item/stack/cable_coil/C = new /obj/item/stack/cable_coil(loc)
			C.amount = 10
			usr.text2tab("<span class='notice'>You cut the cables and disassemble the unused power terminal.</span>")
			qdel(T)
	return 1
