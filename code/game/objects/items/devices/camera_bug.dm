...
/obj/item/device/camera_bug/check_eye(mob/user)
	...
	if(T.z != current.z || !current.can_use())
		user.text2tab("<span class='danger'>[src] has lost the signal.</span>")
		current = null
		user.unset_machine()
...
/obj/item/device/camera_bug/Topic(href,list/href_list)
	...
	if("view" in href_list)
		...
		if(istype(C))
			if(!C.can_use())
				usr.text2tab("<span class='warning'>Something's wrong with that camera!  You can't get a feed.</span>")
				return
			var/turf/T = get_turf(loc)
			if(!T || C.z != T.z)
				usr.text2tab("<span class='warning'>You can't get a signal!</span>")
				return
			current = C
			spawn(6)
