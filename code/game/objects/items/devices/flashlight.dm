...
/obj/item/device/flashlight/attack(mob/living/carbon/human/M, mob/living/carbon/human/user)
	...
	if(on && user.zone_selected == "eyes")
		...
		if(istype(M, /mob/living/carbon/human) && ((H.head && H.head.flags_cover & HEADCOVERSEYES) || (H.wear_mask && H.wear_mask.flags_cover & MASKCOVERSEYES) || (H.glasses && H.glasses.flags_cover & GLASSESCOVERSEYES)))
			user.text2tab("<span class='notice'>You're going to need to remove that [(H.head && H.head.flags_cover & HEADCOVERSEYES) ? "helmet" : (H.wear_mask && H.wear_mask.flags_cover & MASKCOVERSEYES) ? "mask": "glasses"] first.</span>")
			return
		...
		if(M == user)	//they're using it on themselves
			...
			if(istype(C))
				if(C.stat == DEAD || (C.disabilities & BLIND)) //mob is dead or fully blind
					user.text2tab("<span class='warning'>[C] pupils don't react to the light!</span>")
				else if(C.dna.check_mutation(XRAY))	//mob has X-RAY vision
					user.text2tab("<span class='danger'>[C] pupils give an eerie glow!</span>")
				else //they're okay!
					if(C.flash_eyes(visual = 1))
						user.text2tab("<span class='notice'>[C]'s pupils narrow.</span>")
	else
		return ..()
...
/obj/item/device/flashlight/pen/afterattack(atom/target, mob/user, proximity_flag)
	if(!proximity_flag)
		if(holo_cooldown > world.time)
			user.text2tab("<span class='warning'>[src] is not ready yet!</span>")
			return
...
/obj/item/device/flashlight/flare/attack_self(mob/user)
	...
	// Usual checks
	if(!fuel)
		user.text2tab("<span class='warning'>It's out of fuel!</span>")
		return
	if(on)
		return
...
/obj/item/device/flashlight/emp/afterattack(atom/movable/A, mob/user, proximity)
	...
	if(emp_cur_charges > 0)
		...
		user.text2tab("\The [src] now has [emp_cur_charges] charge\s.")
		A.emp_act(1)
	else
		user.text2tab("<span class='warning'>\The [src] needs time to recharge!</span>")
	return
