...
/obj/item/device/radio/examine(mob/user)
	..()
	if (b_stat)
		user.text2tab("<span class='notice'>[name] can be attached and modified.</span>")
	else
		user.text2tab("<span class='notice'>[name] can not be modified or attached.</span>")
...
/obj/item/device/radio/attackby(obj/item/weapon/W, mob/user, params)
	...
	if(istype(W, /obj/item/weapon/screwdriver))
		b_stat = !b_stat
		if(b_stat)
			user.text2tab("<span class='notice'>The radio can now be attached and modified!</span>")
		else
			user.text2tab("<span class='notice'>The radio can no longer be modified or attached!</span>")
	else
		return ..()
...
/obj/item/device/radio/emp_act(severity)
	...
	if (listening && ismob(loc))	// if the radio is turned on and on someone's person they notice
		loc:text2tab("<span class='warning'>\The [src] overloads.</span>")
	broadcasting = 0
	listening = 0
	for (var/ch_name in channels)
		channels[ch_name] = 0
...
/obj/item/device/radio/borg/attackby(obj/item/weapon/W, mob/user, params)
	...
	if(istype(W, /obj/item/weapon/screwdriver))
		if(keyslot)
			...
			recalculateChannels()
			user.text2tab("<span class='notice'>You pop out the encryption key in the radio.</span>")

		else
			user.text2tab("<span class='warning'>This radio doesn't have any encryption keys!</span>")

	else if(istype(W, /obj/item/device/encryptionkey/))
		if(keyslot)
			user.text2tab("<span class='warning'>The radio can't hold another key!</span>")
			return
