...
/obj/item/device/lightreplacer/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/stack/sheet/glass))
		...
		if(uses >= max_uses)
			user.text2tab("<span class='warning'>[src.name] is full.</span>")
			return
		else if(G.use(decrement))
			AddUses(increment)
			user.text2tab("<span class='notice'>You insert a piece of glass into the [src.name]. You have [uses] light\s remaining.</span>")
			return
		else
			user.text2tab("<span class='warning'>You need one sheet of glass to replace lights!</span>")
	...
	if(istype(W, /obj/item/weapon/light))
		...
		user.text2tab("<span class='notice'>You insert the [L.name] into the [src.name]. " + status_string() + "</span>")
		return
	...
	if(istype(W, /obj/item/weapon/storage))
		...
		if(!found_lightbulbs)
			user.text2tab("<span class='warning'>\The [S] contains no bulbs.</span>")
			return

		if(!replaced_something && src.uses == max_uses)
			user.text2tab("<span class='warning'>\The [src] is full!</span>")
			return

		user.text2tab("<span class='notice'>You fill \the [src] with lights from \the [S]. " + status_string() + "</span>")
...
/obj/item/device/lightreplacer/proc/ReplaceLight(obj/machinery/light/target, mob/living/U)
	...
	if(target.status != LIGHT_OK)
		if(CanUse(U))
			if(!Use(U)) return
			U.text2tab("<span class='notice'>You replace the [target.fitting] with \the [src].</span>")

			if(target.status != LIGHT_EMPTY)
				var/new_bulbs = AddShards(1)
				if(new_bulbs != 0)
					U.text2tab("<span class='notice'>\The [src] has fabricated a new bulb from the broken bulbs it has stored. It now has [uses] uses.</span>")
					playsound(src.loc, 'sound/machines/ding.ogg', 50, 1)
		...
		U.text2tab("<span class='warning'>There is a working [target.fitting] already inserted!</span>")
		return
