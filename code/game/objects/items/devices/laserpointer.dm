...
/obj/item/device/laser_pointer/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/weapon/stock_parts/micro_laser))
		if(!diode)
			...
			W.loc = src
			diode = W
			user.text2tab("<span class='notice'>You install a [diode.name] in [src].</span>")
		else
			user.text2tab("<span class='notice'>[src] already has a diode installed.</span>")

	else if(istype(W, /obj/item/weapon/screwdriver))
		if(diode)
			user.text2tab("<span class='notice'>You remove the [diode.name] from \the [src].</span>")
...
/obj/item/device/laser_pointer/proc/laser_act(atom/target, mob/living/user, params)
	if( !(user in (viewers(7,target))) )
		return
	if (!diode)
		user.text2tab("<span class='notice'>You point [src] at [target], but nothing happens!</span>")
		return
	...
	if(ishuman(user))
		...
		if(H.dna.check_mutation(HULK) || (NOGUNS in H.dna.species.specflags))
			user.text2tab("<span class='warning'>Your fingers can't press the button!</span>")
			return
	...
	if(recharge_locked)
		user.text2tab("<span class='notice'>You point [src] at [target], but it's still charging.</span>")
		return
	...
	//robots
		...
		if(prob(effectchance * diode.rating))
			...
			S.Weaken(rand(5,10))
			S.text2tab("<span class='danger'>Your sensors were overloaded by a laser!</span>")
	...
	if(outmsg)
		user.text2tab(outmsg)
	else
		user.text2tab("<span class='info'>You point [src] at [target].</span>")
	...
	if(energy <= max_energy)
		...
		if(energy <= 0)
			user.text2tab("<span class='warning'>[src]'s battery is overused, it needs time to recharge!</span>")
			recharge_locked = 1
