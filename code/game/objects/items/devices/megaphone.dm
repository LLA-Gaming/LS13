...
/obj/item/device/megaphone/attack_self(mob/living/carbon/human/user)
	if(user.client)
		if(user.client.prefs.muted & MUTE_IC)
			user.text2tab("<span class='warning'>You cannot speak in IC (muted).</span>")
			return

	if(!ishuman(user))
		user.text2tab("<span class='warning'>You don't know how to use this!</span>")
		return

	if(spamcheck > world.time)
		user.text2tab("<span class='warning'>\The [src] needs to recharge!</span>")
		return
	...
	if(!user.can_speak(message))
		user.text2tab("<span class='warning'>You find yourself unable to speak at all!</span>")
		return
	...
	if ((src.loc == user && user.stat == 0))
		if(emagged)
			if(insults)
				...
				user.text2tab("<span class='warning'>*BZZZZzzzzzt*</span>")
		else
...
/obj/item/device/megaphone/emag_act(mob/user)
	user.text2tab("<span class='warning'>You overload \the [src]'s voice synthesizer.</span>")
	emagged = 1
