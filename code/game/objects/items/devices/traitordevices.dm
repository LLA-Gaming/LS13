...
/obj/item/device/batterer/attack_self(mob/living/carbon/user, flag = 0, emp = 0)
	...
	if(times_used >= max_uses)
		user.text2tab("<span class='danger'>The mind batterer has been burnt out!</span>")
		return
	...
	for(var/mob/living/carbon/human/M in urange(10, user, 1))
		if(prob(50))
			...
			if(prob(25))
				M.Stun(rand(5,10))
			M.text2tab("<span class='userdanger'>You feel a tremendous, paralyzing wave flood your mind.</span>")

		else
			M.text2tab("<span class='userdanger'>You feel a sudden, electric jolt travel through your head.</span>")

	playsound(src.loc, 'sound/misc/interference.ogg', 50, 1)
	user.text2tab("<span class='notice'>You trigger [src].</span>")
	times_used += 1
	if(times_used >= max_uses)
...
/obj/item/device/healthanalyzer/rad_laser/attack(mob/living/M, mob/living/user)
	...
	if(!used)
		...
		user.text2tab("<span class='warning'>The radioactive microlaser is still recharging.</span>")
...
/obj/item/device/shadowcloak/proc/Activate(mob/living/carbon/human/user)
	if(!user)
		return
	user.text2tab("<span class='notice'>You activate [src].</span>")
	src.user = user
...
/obj/item/device/shadowcloak/proc/Deactivate()
	user.text2tab("<span class='notice'>You deactivate [src].</span>")
