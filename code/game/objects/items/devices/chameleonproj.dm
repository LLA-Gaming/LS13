...
/obj/item/device/chameleon/afterattack(atom/target, mob/user , proximity)
	...
	if(!active_dummy)
		if(istype(target,/obj/item) && !istype(target, /obj/item/weapon/disk/nuclear))
			...
			user.text2tab("<span class='notice'>Scanned [target].</span>")
...
/obj/item/device/chameleon/proc/toggle()
	...
	if(active_dummy)
		...
		qdel(active_dummy)
		active_dummy = null
		usr.text2tab("<span class='notice'>You deactivate \the [src].</span>")
		PoolOrNew(/obj/effect/overlay/temp/emp/pulse, get_turf(src))
	else
		playsound(get_turf(src), 'sound/effects/pop.ogg', 100, 1, -6)
		var/obj/effect/dummy/chameleon/C = new/obj/effect/dummy/chameleon(usr.loc)
		C.activate(usr, saved_appearance, src)
		usr.text2tab("<span class='notice'>You activate \the [src].</span>")
		PoolOrNew(/obj/effect/overlay/temp/emp/pulse, get_turf(src))

/obj/item/device/chameleon/proc/disrupt(delete_dummy = 1)
	if(active_dummy)
		for(var/mob/M in active_dummy)
			M.text2tab("<span class='danger'>Your chameleon-projector deactivates.</span>")
