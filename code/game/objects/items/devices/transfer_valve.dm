...
/obj/item/device/transfer_valve/attackby(obj/item/item, mob/user, params)
	if(istype(item, /obj/item/weapon/tank))
		if(tank_one && tank_two)
			user.text2tab("<span class='warning'>There are already two tanks attached, remove one first!</span>")
			return
		...
		if(!tank_one)
			...
			tank_one = item
			item.loc = src
			user.text2tab("<span class='notice'>You attach the tank to the transfer valve.</span>")
...
//TODO: Have this take an assemblyholder
		...
		if(A.secured)
			user.text2tab("<span class='notice'>The device is secured.</span>")
			return
		if(attached_device)
			user.text2tab("<span class='warning'>There is already a device attached to the valve, remove it first!</span>")
			return
		user.remove_from_mob(item)
		attached_device = A
		A.loc = src
		user.text2tab("<span class='notice'>You attach the [item] to the valve controls and secure it.</span>")
		A.holder = src
