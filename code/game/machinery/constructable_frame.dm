...
/obj/structure/frame/examine(mob/user)
	..()
	if(circuit)
		user.text2tab("It has \a [circuit] installed.")
...
/obj/structure/frame/machine/examine(mob/user)
	...
	if(state == 3 && req_components && req_component_names)
		...
		if(hasContent)
			user.text2tab(requires + ".")
		else
			user.text2tab("It does not require any more components.")
	...
	switch(state)
		if(1)
			if(istype(P, /obj/item/weapon/circuitboard/machine))
				user.text2tab("<span class='warning'>The frame needs wiring first!</span>")
			...
			if(istype(P, /obj/item/stack/cable_coil))
				...
				if(C.get_amount() >= 5)
					...
					user.text2tab("<span class='warning'>You need five length of cable to wire the frame!</span>")
					return
			if(istype(P, /obj/item/weapon/screwdriver) && !anchored)
				...
				if(do_after(user, 40/P.toolspeed, target = src))
					if(state == 1)
						user.text2tab("<span class='notice'>You disassemble the frame.</span>")
		...
		if(2)
			...
			if(istype(P, /obj/item/weapon/circuitboard/machine))
				if(!anchored)
					user.text2tab("<span class='warning'>The frame needs to be secured first!</span>")
					return
				var/obj/item/weapon/circuitboard/machine/B = P
				if(!user.drop_item())
					return
				playsound(src.loc, 'sound/items/Deconstruct.ogg', 50, 1)
				user.text2tab("<span class='notice'>You add the circuit board to the frame.</span>")
				circuit = B
				B.loc = src
				icon_state = "box_2"
				state = 3
		...
		if(3)
			if(istype(P, /obj/item/weapon/crowbar))
				...
				if(components.len == 0)
					...
					user.text2tab("<span class='notice'>You remove the circuit board and other components.</span>")
			...
			if(istype(P, /obj/item/weapon/storage/part_replacer) && P.contents.len && get_req_components_amt())
				...
				for(var/obj/item/weapon/stock_parts/part in added_components)
					components += part
					user.text2tab("<span class='notice'>[part.name] applied.</span>")
					...
					if(istype(P, I) && (req_components[I] > 0))
						if(istype(P, /obj/item/stack))
							...
							if(used_amt && S.use(used_amt))
								...
								user.text2tab("<span class='notice'>You add [P] to [src].</span>")
							return
						if(!user.drop_item())
							break
						user.text2tab("<span class='notice'>You add [P] to [src].</span>")
						...
						req_components[I]--
						return 1
				user.text2tab("<span class='warning'>You cannot add that to the machine!</span>")
				return 0
