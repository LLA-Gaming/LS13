...
/obj/machinery/computer/slot_machine/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/weapon/coin))
		...
		if(prob(2))
			...
			user.text2tab("<span class='warning'>[src] spits your coin back out!</span>")

		else
			if(!user.drop_item())
				return
			user.text2tab("<span class='notice'>You insert a [C.cmineral] coin into [src]'s slot!</span>")
			balance += C.value
			qdel(C)
	else
		return ..()
...
/obj/machinery/computer/slot_machine/proc/can_spin(mob/user)
	if(stat & NOPOWER)
		user.text2tab("<span class='warning'>The slot machine has no power!</span>")
	if(stat & BROKEN)
		user.text2tab("<span class='warning'>The slot machine is broken!</span>")
	if(working)
		user.text2tab("<span class='warning'>You need to wait until the machine stops spinning before you can play again!</span>")
		return 0
	if(balance < SPIN_PRICE)
		user.text2tab("<span class='warning'>Insufficient money to play!</span>")
		return 0
	return 1
...
/obj/machinery/computer/slot_machine/proc/give_prizes(usrname, mob/user)
	...
	if(reels[1][2] + reels[2][2] + reels[3][2] + reels[4][2] + reels[5][2] == "[SEVEN][SEVEN][SEVEN][SEVEN][SEVEN]")
		...
		user.text2tab("<span class='notice'>You win three free games!</span>")
		balance += SPIN_PRICE * 4
		money = max(money - SPIN_PRICE * 4, money)

	else
		user.text2tab("<span class='warning'>No luck!</span>")
