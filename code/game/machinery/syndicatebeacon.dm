...
/obj/machinery/syndicate_beacon/Topic(href, href_list)
	...
	if(href_list["betraitor"])
		...
		if(istype(M, /mob/living/carbon/human))
			...
			M.text2tab("<B>You have joined the ranks of the Syndicate and become a traitor to the station!</B>")

			var/obj_count = 1
			for(var/datum/objective/OBJ in M.mind.objectives)
				M.text2tab("<B>Objective #[obj_count]</B>: [OBJ.explanation_text]")
				obj_count++
...
/obj/machinery/power/singularity_beacon/proc/Activate(mob/user = null)
	if(surplus() < 1500)
		if(user) user.text2tab("<span class='notice'>The connected wire doesn't have enough current.</span>")
		return
	...
	machines |= src
	if(user)
		user.text2tab("<span class='notice'>You activate the beacon.</span>")
...
/obj/machinery/power/singularity_beacon/proc/Deactivate(mob/user = null)
	...
	active = 0
	if(user)
		user.text2tab("<span class='notice'>You deactivate the beacon.</span>")
...
/obj/machinery/power/singularity_beacon/attack_hand(mob/user)
	if(anchored)
		...
		user.text2tab("<span class='warning'>You need to screw the beacon to the floor first!</span>")
		return
...
/obj/machinery/power/singularity_beacon/attackby(obj/item/weapon/W, mob/user, params)
	if(istype(W,/obj/item/weapon/screwdriver))
		if(active)
			user.text2tab("<span class='warning'>You need to deactivate the beacon first!</span>")
			return

		if(anchored)
			anchored = 0
			user.text2tab("<span class='notice'>You unscrew the beacon from the floor.</span>")
			disconnect_from_network()
			return
		else
			if(!connect_to_network())
				user.text2tab("<span class='warning'>This device must be placed over an exposed, powered cable node!</span>")
				return
			anchored = 1
			user.text2tab("<span class='notice'>You screw the beacon to the floor and attach the cable.</span>")
			return
	else
		return ..()
...
/obj/item/device/sbeacondrop/attack_self(mob/user)
	if(user)
		user.text2tab("<span class='notice'>Locked In.</span>")
