...
/mob/living/silicon/ai/proc/ai_actual_track(mob/living/target)
	...
	if(!target || !target.can_track(usr))
		U.text2tab("<span class='warning'>Target is not near any active cameras.</span>")
		U.cameraFollow = null
		return

	U.text2tab("<span class='notice'>Now tracking [target.get_visible_name()] on camera.</span>")
	...
	spawn(0)
		while(U.cameraFollow == target)
			...
			if(!target.can_track(usr))
				U.tracking = 1
				if(!cameraticks)
					U.text2tab("<span class='warning'>Target is not near any active cameras. Attempting to reacquire...</span>")
				cameraticks++
				if(cameraticks > 9)
					U.cameraFollow = null
					U.text2tab("<span class='warning'>Unable to reacquire, cancelling track...</span>")
					tracking = 0
					return
				else
					sleep(10)
