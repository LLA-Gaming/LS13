...
/obj/machinery/computer/upload/attackby(obj/item/O, mob/user, params)
	if(istype(O, /obj/item/weapon/aiModule))
		...
		if(src.stat & (NOPOWER|BROKEN|MAINT))
			return
		if(!current)
			user.text2tab("<span class='caution'>You haven't selected anything to transmit laws to!</span>")
			return
		if(!can_upload_to(current))
			user.text2tab("<span class='caution'>Upload failed!</span> Check to make sure [current.name] is functioning properly.")
			current = null
			return
		var/turf/currentloc = get_turf(current)
		if(currentloc && user.z != currentloc.z)
			user.text2tab("<span class='caution'>Upload failed!</span> Unable to establish a connection to [current.name]. You're too far away!")
			current = null
			return
...
/obj/machinery/computer/upload/ai/attack_hand(mob/user)
	...
	if (!src.current)
		user.text2tab("<span class='caution'>No active AIs detected!</span>")
	else
		user.text2tab("[src.current.name] selected for law changes.")
...
/obj/machinery/computer/upload/borg/attack_hand(mob/user)
	...
	if(!src.current)
		user.text2tab("<span class='caution'>No active unslaved cyborgs detected!</span>")
	else
		user.text2tab("[src.current.name] selected for law changes.")
