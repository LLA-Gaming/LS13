		...
		if(settlers.len == 0)
			...
			if(food <= 0)
				...
				if(emagged)
					...
					user.text2tab("<span class='userdanger'><font size=3>Your body instantly contracts to that of one who has not eaten in months. Agonizing cramps seize you as you fall to the floor.</span>")
			if(fuel <= 0)
				...
				if(emagged)
					...
					user.text2tab("<span class='userdanger'><font size=3>You feel an immense wave of heat emanate from the arcade machine. Your skin bursts into flames.</span>")
		dat += "<br><P ALIGN=Right><a href='byond://?src=\ref[src];menu=1'>OK...</a></P>"

		if(emagged)
			user.text2tab("<span class='userdanger'><font size=3>You're never going to make it to Orion...</span></font>")
			user.death()
			...
			if(emagged)
				...
				switch(event)
					if(ORION_TRAIL_RAIDERS)
						if(prob(50))
							usr.text2tab("<span class='userdanger'>You hear battle shouts. The tramping of boots on cold metal. Screams of agony. The rush of venting air. Are you going insane?</span>")
							M.hallucination += 30
						else
							usr.text2tab("<span class='userdanger'>Something strikes you from behind! It hurts like hell and feel like a blunt weapon, but nothing is there...</span>")
							M.take_organ_damage(30)
							playsound(loc, 'sound/weapons/genhit2.ogg', 100, 1)
					if(ORION_TRAIL_ILLNESS)
						var/severity = rand(1,3) //pray to RNGesus. PRAY, PIGS
						if(severity == 1)
							M.text2tab("<span class='userdanger'>You suddenly feel slightly nauseous.</span>") //got off lucky
						if(severity == 2)
							usr.text2tab("<span class='userdanger'>You suddenly feel extremely nauseous and hunch over until it passes.</span>")
							M.Stun(3)
						if(severity >= 3) //you didn't pray hard enough
							M.text2tab("<span class='warning'>An overpowering wave of nausea consumes over you. You hunch over, your stomach's contents preparing for a spectacular exit.</span>")
							M.Stun(5)
							sleep(30)
							M.vomit(50)
					if(ORION_TRAIL_FLUX)
						if(prob(75))
							...
							M.text2tab("<span class='userdanger'>A violent gale blows past you, and you barely manage to stay standing!</span>")
...
/obj/machinery/computer/arcade/orion_trail/emag_act(mob/user)
	if(!emagged)
		user.text2tab("<span class='notice'>You override the cheat code menu and skip to Cheat #[rand(1, 50)]: Realism Mode.</span>")
...
/obj/item/weapon/orion_ship/examine(mob/user)
	...
	if(!(in_range(user, src)))
		return
	if(!active)
		user.text2tab("<span class='notice'>There's a little switch on the bottom. It's flipped down.</span>")
	else
		user.text2tab("<span class='notice'>There's a little switch on the bottom. It's flipped up.</span>")
...
/obj/item/weapon/orion_ship/attack_self(mob/user) //Minibomb-level explosion. Should probably be more because of how hard it is to survive the machine! Also, just over a 5-second fuse
	...
	user.text2tab("<span class='warning'>You flip the switch on the underside of [src].</span>")
	active = 1
