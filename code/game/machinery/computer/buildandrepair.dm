	...
	switch(state)
		if(0)
			if(istype(P, /obj/item/weapon/wrench))
				...
				user.text2tab("<span class='notice'>You start wrenching the frame into place...</span>")
				if(do_after(user, 20/P.toolspeed, target = src))
					user.text2tab("<span class='notice'>You wrench the frame into place.</span>")
					anchored = 1
					state = 1
			if(istype(P, /obj/item/weapon/weldingtool))
				...
				if(!WT.remove_fuel(0, user))
					if(!WT.isOn())
						user.text2tab("<span class='warning'>The welding tool must be on to complete this task!</span>")
					return
				playsound(src.loc, 'sound/items/Welder.ogg', 50, 1)
				user.text2tab("<span class='notice'>You start deconstructing the frame...</span>")
				if(do_after(user, 20/P.toolspeed, target = src))
					if(!src || !WT.isOn()) return
					user.text2tab("<span class='notice'>You deconstruct the frame.</span>")
		...
		if(1)
			if(istype(P, /obj/item/weapon/wrench))
				playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
				user.text2tab("<span class='notice'>You start to unfasten the frame...</span>")
				if(do_after(user, 20/P.toolspeed, target = src))
					user.text2tab("<span class='notice'>You unfasten the frame.</span>")
					anchored = 0
					state = 0
			if(istype(P, /obj/item/weapon/circuitboard/computer) && !circuit)
				...
				user.text2tab("<span class='notice'>You place the circuit board inside the frame.</span>")
				icon_state = "1"
				circuit = P
				...
				user.text2tab("<span class='warning'>This frame does not accept circuit boards of this type!</span>")

			if(istype(P, /obj/item/weapon/screwdriver) && circuit)
				playsound(src.loc, 'sound/items/Screwdriver.ogg', 50, 1)
				user.text2tab("<span class='notice'>You screw the circuit board into place.</span>")
				state = 2
				icon_state = "2"
			if(istype(P, /obj/item/weapon/crowbar) && circuit)
				playsound(src.loc, 'sound/items/Crowbar.ogg', 50, 1)
				user.text2tab("<span class='notice'>You remove the circuit board.</span>")
				state = 1
				icon_state = "0"
				circuit.loc = src.loc
				circuit.add_fingerprint(user)
		...
		if(2)
			if(istype(P, /obj/item/weapon/screwdriver) && circuit)
				playsound(src.loc, 'sound/items/Screwdriver.ogg', 50, 1)
				user.text2tab("<span class='notice'>You unfasten the circuit board.</span>")
				state = 1
				icon_state = "1"
			if(istype(P, /obj/item/stack/cable_coil))
				...
				if(C.get_amount() >= 5)
					playsound(src.loc, 'sound/items/Deconstruct.ogg', 50, 1)
					user.text2tab("<span class='notice'>You start adding cables to the frame...</span>")
					if(do_after(user, 20/P.toolspeed, target = src))
						if(C.get_amount() >= 5 && state == 2)
							C.use(5)
							user.text2tab("<span class='notice'>You add cables to the frame.</span>")
							state = 3
							icon_state = "3"
				else
					user.text2tab("<span class='warning'>You need five lengths of cable to wire the frame!</span>")
		if(3)
			if(istype(P, /obj/item/weapon/wirecutters))
				playsound(src.loc, 'sound/items/Wirecutter.ogg', 50, 1)
				user.text2tab("<span class='notice'>You remove the cables.</span>")
				state = 2
				icon_state = "2"
			...
			if(istype(P, /obj/item/stack/sheet/glass))
				...
				if(G.get_amount() < 2)
					user.text2tab("<span class='warning'>You need two glass sheets to continue construction!</span>")
					return
				else
					playsound(src.loc, 'sound/items/Deconstruct.ogg', 50, 1)
					user.text2tab("<span class='notice'>You start to put in the glass panel...</span>")
					if(do_after(user, 20, target = src))
						if(G.get_amount() >= 2 && state == 3)
							G.use(2)
							user.text2tab("<span class='notice'>You put in the glass panel.</span>")
							state = 4
							src.icon_state = "4"
		if(4)
			if(istype(P, /obj/item/weapon/crowbar))
				playsound(src.loc, 'sound/items/Crowbar.ogg', 50, 1)
				user.text2tab("<span class='notice'>You remove the glass panel.</span>")
				state = 3
				icon_state = "3"
			...
			if(istype(P, /obj/item/weapon/screwdriver))
				playsound(src.loc, 'sound/items/Screwdriver.ogg', 50, 1)
				user.text2tab("<span class='notice'>You connect the monitor.</span>")
...
/obj/item/weapon/circuitboard/computer/card/minor/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/screwdriver))
		...
		user.text2tab("<span class='notice'>You set the board to \"[dept_list[target_dept]]\".</span>")
	else
		return ..()

/obj/item/weapon/circuitboard/computer/card/minor/examine(mob/user)
	..()
	user.text2tab("Currently set to \"[dept_list[target_dept]]\".")
...
/obj/item/weapon/circuitboard/computer/rdconsole/attackby(obj/item/I, mob/user, params)
	if(istype(I,/obj/item/weapon/screwdriver))
		if(build_path == /obj/machinery/computer/rdconsole/core)
			...
			user.text2tab("<span class='notice'>Access protocols successfully updated.</span>")
		else
			name = "circuit board (RD Console)"
			build_path = /obj/machinery/computer/rdconsole/core
			user.text2tab("<span class='notice'>Defaulting access protocols.</span>")
	else
		return ..()
...
/obj/item/weapon/circuitboard/computer/cargo/attackby(obj/item/I, mob/user, params)
	if(istype(I,/obj/item/device/multitool))
		if(!emagged)
			...
			user.text2tab("<span class='notice'>Receiver spectrum set to [contraband ? "Broad" : "Standard"].</span>")
		else
			user.text2tab("<span class='notice'>The spectrum chip is unresponsive.</span>")
	else if(istype(I,/obj/item/weapon/card/emag))
		if(!emagged)
			contraband = TRUE
			emagged = TRUE
			user.text2tab("<span class='notice'>You adjust [src]'s routing and receiver spectrum, unlocking special supplies and contraband.</span>")
	else
		return ..()
