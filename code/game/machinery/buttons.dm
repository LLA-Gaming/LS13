...
/obj/machinery/button/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/weapon/screwdriver))
		if(panel_open || allowed(user))
			...
			user.text2tab("<span class='danger'>Maintenance Access Denied</span>")
	...
	if(panel_open)
		if(!device && istype(W, /obj/item/device/assembly))
			if(!user.unEquip(W))
				user.text2tab("<span class='warning'>\The [W] is stuck to you!</span>")
				return
			W.loc = src
			device = W
			user.text2tab("<span class='notice'>You add [W] to the button.</span>")
		...
		if(!device && !board && istype(W, /obj/item/weapon/wrench))
			user.text2tab("<span class='notice'>You start unsecuring the button frame...</span>")
			playsound(loc, 'sound/items/Ratchet.ogg', 50, 1)
			if(do_after(user, 40/W.toolspeed, target = src))
				user.text2tab("<span class='notice'>You unsecure the button frame.</span>")
	...
	if(panel_open)
		if(device || board)
			...
			if(board)
				...
				board = null
			update_icon()
			user.text2tab("<span class='notice'>You remove electronics from the button frame.</span>")
			...
			if(skin == "doorctrl")
				...
				skin = "doorctrl"
			user.text2tab("<span class='notice'>You change the button frame's front panel.</span>")
		return
