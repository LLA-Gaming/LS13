...
/obj/item/pipe/attackby(obj/item/weapon/W, mob/user, params)
	...
	for(var/obj/machinery/atmospherics/M in src.loc)
		...
		if(M.GetInitDirections() & A.GetInitDirections())	// matches at least one direction on either type of pipe
			user.text2tab("<span class='warning'>There is already a pipe at that location!</span>")
			qdel(A)
			return 1
	// no conflicts found
...
/obj/item/pipe_meter/attackby(obj/item/weapon/W, mob/user, params)
	...
	if(!locate(/obj/machinery/atmospherics/pipe, src.loc))
		user.text2tab("<span class='warning'>You need to fasten it to a pipe!</span>")
		return 1
	new/obj/machinery/meter( src.loc )
	playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
	user.text2tab("<span class='notice'>You fasten the meter to the pipe.</span>")
	qdel(src)
