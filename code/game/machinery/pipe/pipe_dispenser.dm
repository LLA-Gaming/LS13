...
/obj/machinery/pipedispenser/attackby(obj/item/W, mob/user, params)
	...
	if (istype(W, /obj/item/pipe) || istype(W, /obj/item/pipe_meter))
		usr.text2tab("<span class='notice'>You put [W] back into [src].</span>")
		if(!user.drop_item())
			return
		qdel(W)
		return
		...
		if (!anchored && !isinspace())
			...
			user.text2tab("<span class='notice'>You begin to fasten \the [src] to the floor...</span>")
			...
			playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
			user.text2tab("<span class='notice'>You begin to unfasten \the [src] from the floor...</span>")
...
/obj/machinery/pipedispenser/disposal/Topic(href, href_list)
	...
	if(href_list["dmake"])
		if(!wait)
			...
			if(!C.can_place())
				usr.text2tab("<span class='warning'>There's not enough room to build that here!</span>")
				qdel(C)
				return
