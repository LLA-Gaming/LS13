...
/obj/machinery/recharger/attackby(obj/item/weapon/G, mob/user, params)
	if(istype(G, /obj/item/weapon/wrench))
		if(charging)
			user.text2tab("<span class='notice'>Remove the charging item first!</span>")
			return
		anchored = !anchored
		power_change()
		user.text2tab("<span class='notice'>You [anchored ? "attached" : "detached"] [src].</span>")
		playsound(loc, 'sound/items/Ratchet.ogg', 75, 1)
		return

	if(istype(G, /obj/item/weapon/gun/energy) || istype(G, /obj/item/weapon/melee/baton) || istype(G, /obj/item/ammo_box/magazine/recharge) || istype(G, /obj/item/weapon/stock_parts/cell/magazine))
		if(anchored)
			...
			if(!isarea(a) || a.power_equip == 0)
				user.text2tab("<span class='notice'>[src] blinks red as you try to insert [G].</span>")
				return 1

			if (istype(G, /obj/item/weapon/gun/energy))
				var/obj/item/weapon/gun/energy/gun = G
				if(!gun.can_charge)
					user.text2tab("<span class='notice'>Your gun has no external power connector.</span>")
					return 1
			...
			user.text2tab("<span class='notice'>[src] isn't connected to anything!</span>")
		return 1
