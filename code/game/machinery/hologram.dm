	...
	if (href_list["AIrequest"])
		if(last_request + 200 < world.time)
			...
			for(var/mob/living/silicon/ai/AI in living_mob_list)
				if(!AI.client)
					continue
				AI.text2tab("<span class='info'>Your presence is requested at <a href='?src=\ref[AI];jumptoholopad=\ref[src]'>\the [area]</a>.</span>")
		else
...
/obj/machinery/hologram/holopad/proc/activate_holo(mob/living/silicon/ai/user)
	if(!(stat & NOPOWER) && user.eyeobj.loc == src.loc)//If the projector has power and client eye is on it
		if (istype(user.current, /obj/machinery/hologram/holopad))
			user.text2tab("<span class='danger'>ERROR:</span> \black Image feed in progress.")
			return
		create_holo(user)//Create one.
		src.visible_message("A holographic image of [user] flicks to life right before your eyes!")
	else
		user.text2tab("<span class='danger'>ERROR:</span> \black Unable to project hologram.")
	return
