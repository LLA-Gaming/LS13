...
/obj/machinery/shieldgen/attack_hand(mob/user)
	if(locked)
		user.text2tab("<span class='warning'>The machine is locked, you are unable to use it!</span>")
		return
	if(panel_open)
		user.text2tab("<span class='warning'>The panel must be closed before operating this machine!</span>")
		return
	...
	if (active)
		...
		if(anchored)
			...
			user.text2tab("<span class='warning'>The device must first be secured to the floor!</span>")
	return
...
/obj/machinery/shieldgen/attackby(obj/item/weapon/W, mob/user, params)
	if(istype(W, /obj/item/weapon/screwdriver))
		...
		if(panel_open)
			user.text2tab("<span class='notice'>You open the panel and expose the wiring.</span>")
		else
			user.text2tab("<span class='notice'>You close the panel.</span>")
	else if(istype(W, /obj/item/stack/cable_coil) && (stat & BROKEN) && panel_open)
		var/obj/item/stack/cable_coil/coil = W
		if (coil.get_amount() < 1)
			user.text2tab("<span class='warning'>You need one length of cable to repair [src]!</span>")
			return
		user.text2tab("<span class='notice'>You begin to replace the wires...</span>")
		if(do_after(user, 30, target = src))
			...
			health = max_health
			stat &= ~BROKEN
			user.text2tab("<span class='notice'>You repair \the [src].</span>")
			update_icon()

	else if(istype(W, /obj/item/weapon/wrench))
		if(locked)
			user.text2tab("<span class='warning'>The bolts are covered! Unlocking this would retract the covers.</span>")
			return
		if(!anchored && !isinspace())
			playsound(src.loc, 'sound/items/Ratchet.ogg', 100, 1)
			user.text2tab("<span class='notice'>You secure \the [src] to the floor!</span>")
			anchored = 1
		else if(anchored)
			playsound(src.loc, 'sound/items/Ratchet.ogg', 100, 1)
			user.text2tab("<span class='notice'>You unsecure \the [src] from the floor!</span>")
			if(active)
				user.text2tab("<span class='notice'>\The [src] shuts off!</span>")
				shields_down()
			anchored = 0
...
/obj/machinery/shieldwallgen/attack_hand(mob/user)
	...
	if(locked && !istype(user, /mob/living/silicon))
		...
		return 1
	if(power != 1)
		user.text2tab("<span class='warning'>\The [src] needs to be powered by wire underneath!</span>")
		return 1
...
/obj/machinery/shieldwallgen/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/weapon/wrench))
		if(active)
			user.text2tab("<span class='warning'>Turn off the field generator first!</span>")
			return

		else if(!anchored && !isinspace()) //Can't fasten this thing in space
			playsound(src.loc, 'sound/items/Ratchet.ogg', 75, 1)
			user.text2tab("<span class='notice'>You secure the external reinforcing bolts to the floor.</span>")
			anchored = 1
			return

		else //You can unfasten it tough, if you somehow manage to fasten it.
			playsound(src.loc, 'sound/items/Ratchet.ogg', 75, 1)
			user.text2tab("<span class='notice'>You undo the external reinforcing bolts.</span>")
			anchored = 0
			return
