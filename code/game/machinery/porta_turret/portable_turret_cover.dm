...
/obj/machinery/porta_turret_cover/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/weapon/wrench) && !parent_turret.on)
		...
		if(parent_turret.allowed(user))
			...
			user.text2tab("<span class='notice'>Controls are now [parent_turret.locked ? "locked" : "unlocked"].</span>")
			updateUsrDialog()
		else
		...
		user.text2tab("<span class='notice'>You add [parent_turret] to multitool buffer.</span>")
	else
		return ..()
...
/obj/machinery/porta_turret_cover/emag_act(mob/user)
	if(!parent_turret.emagged)
		user.text2tab("<span class='notice'>You short out [parent_turret]'s threat assessment circuits.</span>")
