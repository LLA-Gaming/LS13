...
/obj/machinery/porta_turret/Topic(href, href_list)
	...
	if(href_list["power"] && !locked)
		if(anchored)	//you can't turn a turret on/off if it's not anchored/secured
			...
			usr.text2tab("<span class='notice'>It has to be secured first!</span>")
...
/obj/machinery/porta_turret/attackby(obj/item/I, mob/user, params)
	if(stat & BROKEN)
		if(istype(I, /obj/item/weapon/crowbar))
			...
			user.text2tab("<span class='notice'>You begin prying the metal coverings off...</span>")
			if(do_after(user, 20/I.toolspeed, target = src))
				if(prob(70))
					user.text2tab("<span class='notice'>You remove the turret and salvage some components.</span>")
					if(installation)
					...
					user.text2tab("<span class='notice'>You remove the turret but did not manage to salvage anything.</span>")
				qdel(src)
		...
		if(!anchored && !isinspace())
			...
			icon_state = "[base_icon_state][off_state]"
			user.text2tab("<span class='notice'>You secure the exterior bolts on the turret.</span>")
			if(has_cover)
				cover = new /obj/machinery/porta_turret_cover(loc) //create a new turret. While this is handled in process(), this is to workaround a bug where the turret becomes invisible for a split second
				cover.parent_turret = src //make the cover's parent src
		else if(anchored)
			anchored = 0
			user.text2tab("<span class='notice'>You unsecure the exterior bolts on the turret.</span>")
			icon_state = "turretCover"
			invisibility = 0
		...
		if(allowed(user))
			locked = !locked
			user.text2tab("<span class='notice'>Controls are now [locked ? "locked" : "unlocked"].</span>")
		else
			user.text2tab("<span class='notice'>Access denied.</span>")
	else if(istype(I,/obj/item/device/multitool) && !locked)
		var/obj/item/device/multitool/M = I
		M.buffer = src
		user.text2tab("<span class='notice'>You add [src] to multitool buffer.</span>")
	else
		return ..()
...
/obj/machinery/porta_turret/emag_act(mob/user)
	if(!emagged)
		user.text2tab("<span class='warning'>You short out [src]'s threat assessment circuits.</span>")
...
/obj/machinery/turretid/attackby(obj/item/I, mob/user, params)
	...
	if (istype(I,/obj/item/device/multitool))
		...
		if(M.buffer && istype(M.buffer,/obj/machinery/porta_turret))
			turrets |= M.buffer
			user.text2tab("You link \the [M.buffer] with \the [src]")
			return
	...
	if( get_dist(src, user) == 0 )		// trying to unlock the interface
		if (src.allowed(usr))
			if(emagged)
				user.text2tab("<span class='notice'>The turret control is unresponsive.</span>")
				return

			locked = !locked
			user.text2tab("<span class='notice'>You [ locked ? "lock" : "unlock"] the panel.</span>")
			if (locked)
				if (user.machine==src)
					user.unset_machine()
...
/obj/machinery/turretid/emag_act(mob/user)
	if(!emagged)
		user.text2tab("<span class='danger'>You short out the turret controls' access analysis module.</span>")
		emagged = 1
		locked = 0
...
/obj/machinery/turretid/attack_ai(mob/user)
	if(!ailock || IsAdminGhost(user))
		...
		user.text2tab("<span class='notice'>There seems to be a firewall preventing you from accessing this device.</span>")

/obj/machinery/turretid/attack_hand(mob/user as mob)
	if ( get_dist(src, user) > 0 )
		if ( !(issilicon(user) || IsAdminGhost(user)) )
			user.text2tab("<span class='notice'>You are too far away.</span>")
			user.unset_machine()
			user << browse(null, "window=turretid")
			return
	...
	if (istype(loc, /turf))
		loc = loc:loc
	if (!istype(loc, /area))
		user.text2tab(text("Turret badly positioned - loc.loc is [].", loc))
		return
	var/area/area = loc
	var/t = ""
...
/obj/machinery/turretid/Topic(href, href_list)
	...
	if (src.locked)
		if (!(istype(usr, /mob/living/silicon) || IsAdminGhost(usr)))
			usr.text2tab("Control panel is locked!")
			return
